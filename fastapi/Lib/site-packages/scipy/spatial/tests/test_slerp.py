...
class TestGeometricSlerp:
    ...
    def test_shape_property(self, n_dims, n_pts):
        ...
        actual = geometric_slerp(
            start=start, end=end, t=np.linspace(0, 1, n_pts)
        )
    ...
    def test_handle_antipodes(self, start, end, expected):
        ...
        if expected == "warning":
            with pytest.warns(UserWarning, match="antipodes"):
                res = geometric_slerp(
                    start=start, end=end, t=np.linspace(0, 1, 10)
                )
        else:
            res = geometric_slerp(
                start=start, end=end, t=np.linspace(0, 1, 10)
    ...
    def test_t_values_limits(self, t):
        ...
        with pytest.raises(ValueError, match="interpolation parameter"):
            _ = geometric_slerp(
                start=np.array([1, 0]), end=np.array([0, 1]), t=t
            )
    ...
    def test_order_handling(self, start, end, t_func):
        ...
        forward_results = geometric_slerp(
            start=start, end=end, t=forward_t_vals
        )
        reverse_results = geometric_slerp(
            start=start, end=end, t=reverse_t_vals
        )
        scrambled_results = geometric_slerp(
            start=start, end=end, t=scramble_t_vals
        )
