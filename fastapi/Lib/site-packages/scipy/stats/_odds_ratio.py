...
def _conditional_oddsratio_ci(
    table, confidence_level=0.95, alternative="two-sided"
):
    """
...
def odds_ratio(table, *, kind="conditional"):
    ...
    if c.shape != (2, 2):
        raise ValueError(
            f"Invalid shape {c.shape}. The input `table` must be "
            "of shape (2, 2)."
        )
