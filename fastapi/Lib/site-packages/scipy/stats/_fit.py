...
class FitResult:
    ...
    def _hist_plot(self, ax, fit_params):
        ...
        if self.discrete:
            ...
            ax.vlines(
                x[:-1], 0, y[:-1], label="Fitted Distribution PMF", color="C0"
            )
    ...
    def _qp_plot(self, ax, fit_params, qq):
        ...
        ax.plot(
            ...
            y,
            ".",
            label=f"Fitted Distribution {plot_type}",
            color="C0",
            zorder=1,
        )
        xlim = ax.get_xlim()
...
def fit(
    ...
    *,
    ...
    if hasattr(dist, "pdf"):  # can't use isinstance for types
        ...
        message = (
            "`dist` must be an instance of `rv_continuous` "
            "or `rv_discrete.`"
        )
        raise ValueError(message)
    ...
    if data.ndim != 1:
        ...
        raise ValueError(message)
    if not (
        np.issubdtype(data.dtype, np.number) and np.all(np.isfinite(data))
    ):
    ...
    if user_guess is None:
        ...
        try:
            guess_array = np.asarray(
                [default_guess[param.name] for param in param_info],
                dtype=float,
            )
        except ValueError as e:
            raise ValueError(message) from e
...
GoodnessOfFitResult = namedtuple(
    "GoodnessOfFitResult",
    ("fit_result", "statistic", "pvalue", "null_distribution"),
)
...
def goodness_of_fit(
    ...
    *observed data*. This tends to reduce the value of the statistic for the
    ...
    nhd_fit_fun = _get_fit_fun(
        dist, data, guessed_nhd_params, fixed_nhd_params
    )
...
def _get_fit_fun(dist, data, guessed_params, fixed_params):
    ...
    # Define statistic, including fitting distribution to data
        ...
        def fit_fun_1d(data):
            return dist.fit(
                data, *guessed_shapes, **guessed_params, **fixed_params
            )
...
def _corr(X, M):
    ...
    den = np.sqrt(
        np.sum((X - Xm) ** 2, axis=-1) * np.sum((M - Mm) ** 2, axis=-1)
    )
    return num / den
...
def _gof_iv(
    ...
    if not isinstance(dist, stats.rv_continuous):
        message = (
            "`dist` must be a (non-frozen) instance of "
            "`stats.rv_continuous`."
        )
