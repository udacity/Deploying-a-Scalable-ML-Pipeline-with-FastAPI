...
class multivariate_normal_gen(multi_rv_generic):
    ...
    def _process_parameters_psd(self, dim, mean, cov):
        ...
        if mean.ndim != 1 or mean.shape[0] != dim:
            raise ValueError(
                "Array 'mean' must be a vector of length %d." % dim
            )
        if cov.ndim == 0:
            cov = cov * np.eye(dim)
        elif cov.ndim == 1:
...
for name in ["logpdf", "pdf", "logcdf", "cdf", "rvs"]:
    ...
    method_frozen.__doc__ = doccer.docformat(
        method.__doc__, mvn_docdict_noparams
    )
...
class matrix_normal_gen(multi_rv_generic):
    ...
    def _process_parameters(self, mean, rowcov, colcov):
        ...
        if mean is not None:
            if meanshape[0] != numrows:
                raise ValueError(
                    "Arrays `mean` and `rowcov` must have the "
                    "same number of rows."
                )
    ...
    def _logpdf(
        ...
        scale_dev = np.tensordot(
            col_prec_rt.T, np.dot(roll_dev, row_prec_rt), 1
        )
    ...
    def entropy(self, rowcov=1, colcov=1):
        ...
        dims, _, rowcov, colcov = self._process_parameters(
            dummy_mean, rowcov, colcov
        )
        rowpsd = _PSD(rowcov, allow_singular=False)
        colpsd = _PSD(colcov, allow_singular=False)
...
class matrix_normal_frozen(multi_rv_frozen):
    ...
    def rvs(self, size=1, random_state=None):
        return self._dist.rvs(
            self.mean, self.rowcov, self.colcov, size, random_state
        )
...
for name in ["logpdf", "pdf", "rvs", "entropy"]:
    ...
    method_frozen.__doc__ = doccer.docformat(
        method.__doc__, matnorm_docdict_noparams
    )
...
def _dirichlet_check_input(alpha, x):
    ...
    if np.min(x) < 0:
        raise ValueError(
            "Each entry in 'x' must be greater than or equal " "to zero."
        )
...
class wishart_gen(multi_rv_generic):
    ...
    def _standard_rvs(self, n, shape, dim, df, random_state):
        ...
        n_tril = dim * (dim - 1) // 2
        covariances = random_state.normal(size=n * n_tril).reshape(
            shape + (n_tril,)
        )
...
class wishart_frozen(multi_rv_frozen):
    ...
    def rvs(self, size=1, random_state=None):
        ...
        out = self._dist._rvs(
            n, shape, self.dim, self.df, self.C, random_state
        )
        return _squeeze_output(out)
...
class invwishart_frozen(multi_rv_frozen):
    ...
    def logpdf(self, x):
        x = self._dist._process_quantiles(x, self.dim)
        out = self._dist._logpdf(
            x, self.dim, self.df, self.log_det_scale, self.C
        )
        return _squeeze_output(out)
...
class multinomial_gen(multi_rv_generic):
    ...
    def __init__(self, seed=None):
        super().__init__(seed)
        self.__doc__ = doccer.docformat(
            self.__doc__, multinomial_docdict_params
        )
...
for name in ["logpmf", "pmf", "mean", "cov", "rvs"]:
    ...
    method.__doc__ = doccer.docformat(
        method.__doc__, multinomial_docdict_params
    )
...
class random_correlation_gen(multi_rv_generic):
    ...
    def __call__(self, eigs, seed=None, tol=1e-13, diag_tol=1e-7):
        ...
        """
        return random_correlation_frozen(
            eigs, seed=seed, tol=tol, diag_tol=diag_tol
        )
...
class multivariate_t_gen(multi_rv_generic):
    ...
    def __call__(
        self, loc=None, shape=1, df=1, allow_singular=False, seed=None
    ):
    ...
    def cdf(
        ...
        *,
        ...
        return self._cdf(
            x, loc, shape, df, dim, maxpts, lower_limit, random_state
        )
    ...
    def _process_parameters(self, loc, shape, df):
        ...
        if loc.ndim != 1 or loc.shape[0] != dim:
            raise ValueError(
                "Array 'loc' must be a vector of length %d." % dim
            )
        if shape.ndim == 0:
...
for name in ["logpdf", "pdf", "rvs", "cdf", "entropy"]:
    ...
    method_frozen.__doc__ = doccer.docformat(
        method.__doc__, mvt_docdict_noparams
    )
...
class multivariate_hypergeom_gen(multi_rv_generic):
    ...
    def _process_parameters(self, m, n):
        ...
        if m.ndim == 0:
            raise ValueError(
                "'m' must be an array with" " at least one dimension."
            )
    ...
    def _process_quantiles(self, x, M, m, n):
        ...
        if x.ndim == 0:
            raise ValueError(
                "'x' must be an array with" " at least one dimension."
            )
    ...
    def logpmf(self, x, m, n):
        ...
        (x, M, m, n, xcond, xcond_reduced) = self._process_quantiles(
            x, M, m, n
        )
        mxcond = mcond | xcond
    ...
    def mean(self, m, n):
        ...
        mu = n * (m / M)
        if m.size != 0:
            mncond = mncond[..., np.newaxis] | np.zeros(
                mu.shape, dtype=np.bool_
            )
...
class multivariate_hypergeom_frozen(multi_rv_frozen):
    ...
    def rvs(self, size=1, random_state=None):
        return self._dist.rvs(
            self.m, self.n, size=size, random_state=random_state
        )
...
class dirichlet_multinomial_gen(multi_rv_generic):
    ...
    def __init__(self, seed=None):
        super().__init__(seed)
        self.__doc__ = doccer.docformat(
            self.__doc__, dirichlet_mn_docdict_params
        )
...
for name in ["logpmf", "pmf", "mean", "var", "cov"]:
    ...
    method.__doc__ = doccer.docformat(
        method.__doc__, dirichlet_mn_docdict_params
    )
...
class vonmises_fisher_gen(multi_rv_generic):
    ...
    **Fitting the distribution parameters**
    ...
    def _rvs_2d(self, mu, kappa, size, random_state):
        ...
        samples = np.stack(
            [np.cos(angle_samples), np.sin(angle_samples)], axis=-1
        )
        return samples
    ...
    def _rvs_3d(self, kappa, size, random_state):
        ...
        samples = np.stack(
            [x, temp * uniformcircle[..., 0], temp * uniformcircle[..., 1]],
            axis=-1,
        )
        if size is None:
    ...
    def _rejection_sampling(self, dim, kappa, size, random_state):
        ...
        coord_rest = _sample_uniform_direction(
            dim_minus_one, n_accepted, random_state
        )
    ...
    def _entropy(self, dim, kappa):
        halfdim = 0.5 * dim
        return -self._log_norm_factor(dim, kappa) - kappa * ive(
            halfdim, kappa
        ) / ive(halfdim - 1, kappa)
    ...
    def fit(self, x):
        ...
        root_res = root_scalar(
            solve_for_kappa, method="brentq", bracket=(1e-8, 1e9)
        )
        kappa = root_res.root
        return mu, kappa
...
class vonmises_fisher_frozen(multi_rv_frozen):
    def __init__(self, mu=None, kappa=1, seed=None):
        ...
        self.dim, self.mu, self.kappa = self._dist._process_parameters(
            mu, kappa
        )
    ...
    def rvs(self, size=1, random_state=None):
        ...
        return self._dist._rvs(
            self.dim, self.mu, self.kappa, size, random_state
        )
