...
def _wilcoxon_nd(
    ...
    if method == "approx":
        ...
        if alternative == "less":
            ...
            p = 2 * np.minimum(
                dist.sf(np.floor(r_plus)), dist.cdf(np.ceil(r_plus))
            )
            p = np.clip(p, 0, 1)
    ...
    statistic = (
        np.minimum(r_plus, r_minus) if alternative == "two-sided" else r_plus
    )
    z = (
        -np.abs(z)
        if (alternative == "two-sided" and method == "approx")
        else z
    )
