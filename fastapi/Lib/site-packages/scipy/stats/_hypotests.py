...
def _psi1_mod(x):
    ...
    def _Ak(k, x):
        ...
        e3 = (
            2 * (m + 2) * gamma(k + 3 / 2) * _ed3((4 * k + 5) / sx) / (12 * y2)
        )
...
def _cdf_cvm_inf(x):
    ...
    def term(x, k):
        ...
        u = np.exp(gammaln(k + 0.5) - gammaln(k + 1)) / (
            np.pi**1.5 * np.sqrt(x)
        )
        y = 4 * k + 1
        q = y**2 / (16 * x)
...
def _cdf_cvm(x, n=None):
    ...
    if n is None:
        ...
        y[sup] = (
            _cdf_cvm_inf(x[sup]) * (1 + 1.0 / (12 * n)) + _psi1_mod(x[sup]) / n
        )
        y[x >= n / 3] = 1
...
def somersd(x, y=None, alternative="two-sided"):
    ...
    *p*-value returned by `kendalltau` with the statistic returned by
    ...
    if x.ndim == 1:
        ...
        if np.any(x < 0):
            raise ValueError(
                "All elements of the contingency table must be "
                "non-negative."
            )
        ...
        if x.nonzero()[0].size < 2:
            raise ValueError(
                "At least two elements of the contingency table "
                "must be nonzero."
            )
        table = x
    else:
...
def tukey_hsd(*args):
    ...
    mse = np.sum(
        [np.var(arg, ddof=1) for arg in args] * (nsamples_treatments - 1)
    ) / (nobs - ntreatments)
    ...
    return TukeyHSDResult(
        mean_differences, pvalues, ntreatments, nobs, stand_err
    )
