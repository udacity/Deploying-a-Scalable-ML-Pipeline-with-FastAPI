...
def _broadcast_shapes(shapes, axis=None):
    ...
    for row, shape in zip(new_shapes, shapes):
        row[len(row) - len(shape) :] = (
            shape  # can't use negative indices (-0:)
        )
    ...
    if axis is not None:
        ...
        if axis[-1] >= n_dims or axis[0] < 0:
            message = (
                f"`axis` is out of bounds " f"for array of dimension {n_dims}"
            )
...
def _axis_nan_policy_factory(
    ...
    def axis_nan_policy_decorator(hypotest_fun_in):
        ...
        def axis_nan_policy_wrapper(*args, _no_deco=False, **kwds):
            ...
            if (
                len(args) == 0
            ):  # extract sample from `kwds` if there are no `args`
            ...
            if axis is None:
                ...
                new_shapes = [
                    shape[:-n_axes] + (np.prod(shape[-n_axes:]),)
                    for shape in shapes
                ]
                samples = [
            ...
            if np.all(ndims <= 1):
                ...
                if nan_policy != "propagate" or override["nan_propagation"]:
                    contains_nan = [
                        _contains_nan(sample, nan_policy)[0]
                        for sample in samples
                    ]
                else:
