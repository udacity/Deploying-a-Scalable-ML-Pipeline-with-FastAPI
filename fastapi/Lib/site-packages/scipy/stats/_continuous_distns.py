...
class kstwo_gen(rv_continuous):
    ...
    def _get_support(self, n):
        return (
            0.5 / (n if not isinstance(n, Iterable) else np.asanyarray(n)),
            1.0,
        )
...
class alpha_gen(rv_continuous):
    ...
    def _logpdf(self, x, a):
        return (
            -2 * np.log(x) + _norm_logpdf(a - 1.0 / x) - np.log(_norm_cdf(a))
        )
...
class beta_gen(rv_continuous):
    ...
    def _stats(self, a, b):
        ...
        _beta_kurtosis_excess = (
            _beta_kurtosis_excess_n / _beta_kurtosis_excess_d
        )
        return (
            _beta_mean,
            _beta_variance,
    ...
    def _fitstart(self, data):
        ...
        def func(x):
            a, b = x
            sk = (
                2 * (b - a) * np.sqrt(a + b + 1) / (a + b + 2) / np.sqrt(a * b)
            )
            ku = (
                a**3
                - a**2 * (2 * b - 1)
    ...
    def _entropy(self, a, b):
        ...
        def asymptotic_b_large(a, b):
            ...
            log_term = (
                sum_ab * np.log1p(a / b) + np.log(b) - 2 * np.log(sum_ab)
            )
...
class burr12_gen(rv_continuous):
    ...
    def _munp(self, n, c, d):
        ...
        return _lazywhere(
            c * d > n, (n, c, d), moment_if_exists, fillvalue=np.nan
        )
...
class chi_gen(rv_continuous):
    ...
    def _entropy(self, df):
        ...
        return _lazywhere(
            df < 3e2, (df,), regular_formula, f2=asymptotic_formula
        )
...
class dgamma_gen(rv_continuous):
    ...
    def _cdf(self, x, a):
        return np.where(
            x > 0, 0.5 + 0.5 * sc.gammainc(a, x), 0.5 * sc.gammaincc(a, -x)
        )

    def _sf(self, x, a):
        return np.where(
            x > 0, 0.5 * sc.gammaincc(a, x), 0.5 + 0.5 * sc.gammainc(a, -x)
        )
...
class foldcauchy_gen(rv_continuous):
    ...
    def _pdf(self, x, c):
        ...
        return (
            1.0 / np.pi * (1.0 / (1 + (x - c) ** 2) + 1.0 / (1 + (x + c) ** 2))
        )
...
class weibull_min_gen(rv_continuous):
    ...
    def fit(self, data, *args, **kwds):
        ...
        data, fc, floc, fscale = _check_fit_input_parameters(
            self, data, args, kwds
        )
        method = kwds.get("method", "mle").lower()
        ...
        if fscale is None and scale is None:
            v = np.var(data)
            scale = np.sqrt(
                v / (sc.gamma(1 + 2 / c) - sc.gamma(1 + 1 / c) ** 2)
            )
...
class truncweibull_min_gen(rv_continuous):
    ...
    def _isf(self, q, c, a, b):
        return pow(
            -np.log((1 - q) * np.exp(-pow(b, c)) + q * np.exp(-pow(a, c))),
            1 / c,
        )

    def _ppf(self, q, c, a, b):
        return pow(
            -np.log((1 - q) * np.exp(-pow(a, c)) + q * np.exp(-pow(b, c))),
            1 / c,
        )

    def _munp(self, n, c, a, b):
        gamma_fun = sc.gamma(n / c + 1.0) * (
            sc.gammainc(n / c + 1.0, pow(b, c))
            - sc.gammainc(n / c + 1.0, pow(a, c))
        )
        denum = np.exp(-pow(a, c)) - np.exp(-pow(b, c))
...
class genpareto_gen(rv_continuous):
    ...
    def _munp(self, n, c):
        ...
        return _lazywhere(
            c != 0, (c,), lambda c: __munp(n, c), sc.gamma(n + 1)
        )
...
class genextreme_gen(rv_continuous):
    ...
    def _stats(self, c):
        ...
        g4 = g(4)
        g2mg12 = np.where(
            abs(c) < 1e-7, (c * np.pi) ** 2.0 / 6.0, g2 - g1**2.0
        )
        gam2k = np.where(
            abs(c) < 1e-7,
            np.pi**2.0 / 6.0,
            sc.expm1(sc.gammaln(2.0 * c + 1.0) - 2 * sc.gammaln(c + 1.0))
            / c**2.0,
        )
        eps = 1e-14
        ...
        sk = np.where(
            abs(c) <= eps**0.29, 12 * np.sqrt(6) * _ZETA3 / np.pi**3, sk1
        )
...
def _digammainv(y):
    ...
    value, info, ier, mesg = optimize.fsolve(
        func, x0, xtol=1e-11, full_output=True
    )
    if ier != 1:
...
class gamma_gen(rv_continuous):
    ...
    def _entropy(self, a):
        ...
        return _lazywhere(
            a < 250, (a,), regular_formula, f2=asymptotic_formula
        )
...
class genhyperbolic_gen(rv_continuous):
    ...
    def _integrate_pdf(x0, x1, p, a, b):
        ...
        if x0 < mean < x1:
            ...
            intgrl = (
                ...
                + integrate.quad(llc, mean, x1, epsrel=epsrel, epsabs=epsabs)[
                    0
                ]
            )
        else:
            intgrl = integrate.quad(llc, x0, x1, epsrel=epsrel, epsabs=epsabs)[
                0
            ]
        if np.isnan(intgrl):
            msg = (
    ...
    def _rvs(self, p, a, b, size=None, random_state=None):
        ...
        gig = geninvgauss.rvs(
            p=p, b=t2, scale=t3, size=size, random_state=random_state
        )
...
class gumbel_r_gen(rv_continuous):
    ...
    def fit(self, data, *args, **kwds):
        data, floc, fscale = _check_fit_input_parameters(
            self, data, args, kwds
        )
...
class halfcauchy_gen(rv_continuous):
    ...
    def fit(self, data, *args, **kwds):
        ...
        def find_scale(loc, data):
            ...
            res = root_scalar(
                fun_to_solve, bracket=(small, np.max(shifted_data))
            )
            return res.root
...
class invgamma_gen(rv_continuous):
    ...
    def _stats(self, a, moments="mvsk"):
        ...
        m2 = _lazywhere(
            a > 2, (a,), lambda x: 1.0 / (x - 1.0) ** 2 / (x - 2.0), np.inf
        )
...
class geninvgauss_gen(rv_continuous):
    ...
    def _cdf(self, x, *args):
        ...
        def _cdf_single(x, *args):
            ...
            llc = LowLevelCallable.from_cython(
                _stats, "_geninvgauss_pdf", user_data
            )
...
class norminvgauss_gen(rv_continuous):
    ...
    def _sf(self, x, a, b):
        if np.isscalar(x):
            ...
            for x0, a0, b0 in zip(x, a, b):
                result.append(
                    integrate.quad(self._pdf, x0, np.inf, args=(a0, b0))[0]
                )
    ...
    def _isf(self, q, a, b):
        def _isf_scalar(q, a, b):
            ...
            result = optimize.brentq(
                eq, left, right, args=(a, b, q), xtol=self.xtol
            )
            return result
    ...
    def _rvs(self, a, b, size=None, random_state=None):
        ...
        return b * ig + np.sqrt(ig) * norm.rvs(
...
class laplace_asymmetric_gen(rv_continuous):
    ...
    def _stats(self, kappa):
        ...
        g1 = (
            2.0
            * (1 - np.power(kappa, 6))
            / np.power(1 + np.power(kappa, 4), 1.5)
        )
        g2 = (
            6.0
            * (1 + np.power(kappa, 8))
            / np.power(1 + np.power(kappa, 4), 2)
        )
        return mn, var, g1, g2
...
class logistic_gen(rv_continuous):
    ...
    def fit(self, data, *args, **kwds):
        ...
        scale = abs(scale)
        return (
            (loc, scale) if res.success else super().fit(data, *args, **kwds)
        )
...
class loggamma_gen(rv_continuous):
    ...
    def _entropy(self, c):
        ...
        def asymptotic(c):
            # using asymptotic expansions for gammaln and psi (see gh-18093)
            term = (
                -0.5 * np.log(c) + c**-1.0 / 6 - c**-3.0 / 90 + c**-5.0 / 210
            )
...
class loglaplace_gen(rv_continuous):
    ...
    def _isf(self, q, c):
        return np.where(
            q > 0.5, (2.0 * (1.0 - q)) ** (1.0 / c), (2 * q) ** (-1.0 / c)
        )
...
class mielke_gen(rv_continuous):
    ...
    def _logpdf(self, x, k, s):
        ...
        with np.errstate(divide="ignore"):
            return (
                np.log(k) + np.log(x) * (k - 1) - np.log1p(x**s) * (1 + k / s)
            )
    ...
    def _munp(self, n, k, s):
        def nth_moment(n, k, s):
            ...
            return (
                sc.gamma((k + n) / s) * sc.gamma(1 - n / s) / sc.gamma(k / s)
            )
...
class kappa4_gen(rv_continuous):
    ...
    def _get_support(self, h, k):
        ...
        _a = _lazyselect(
            condlist, [f0, f1, f0, f3, f3, f5], [h, k], default=np.nan
        )
        ...
        _b = _lazyselect(
            condlist, [f0, f1, f1, f0, f1, f1], [h, k], default=np.nan
        )
        return _a, _b
    ...
    def _logpdf(self, x, h, k):
        ...
        def f1(x, h, k):
            ...
            logpdf = ...
            """
            return sc.xlog1py(1.0 / k - 1.0, -k * x) - (1.0 - k * x) ** (
                1.0 / k
            )
        ...
        return _lazyselect(
            condlist, [f0, f1, f2, f3], [x, h, k], default=np.nan
        )
    ...
    def _ppf(self, q, h, k):
        ...
        return _lazyselect(
            condlist, [f0, f1, f2, f3], [q, h, k], default=np.nan
        )
...
class pareto_gen(rv_continuous):
    ...
    def _stats(self, b, moments="mv"):
        ...
        if "s" in moments:
            ...
            vals = (
                2 * (bt + 1.0) * np.sqrt(bt - 2.0) / ((bt - 3.0) * np.sqrt(bt))
            )
    ...
    def fit(self, data, *args, **kwds):
        ...
        if floc is fscale is None:
            ...
            def interval_contains_root(lbrack, rbrack):
                # return true if the signs disagree.
                return np.sign(fun_to_solve(lbrack)) != np.sign(
                    fun_to_solve(rbrack)
                )
...
class pearson3_gen(rv_continuous):
    ...
    def _logpdf(self, x, skew):
        ...
        ans, x, transx, mask, invmask, beta, alpha, _ = self._preprocess(
            x, skew
        )
    ...
    def _rvs(self, skew, size=None, random_state=None):
        ...
        ans, _, _, mask, invmask, beta, alpha, zeta = self._preprocess(
            [0], skew
        )
...
class powerlaw_gen(rv_continuous):
    ...
    def fit(self, data, *args, **kwds):
        ...
        data, fshape, floc, fscale = _check_fit_input_parameters(
            self, data, args, kwds
        )
        ...
        def fun_to_solve(loc):
            ...
            scale = np.nextafter(get_scale(data, loc), -np.inf)
            shape = fshape or get_shape(data, loc, scale)
            return dL_dScale(data, shape, scale) - dL_dLocation(
                data, shape, loc
            )
        ...
        def fit_loc_scale_w_shape_gt_1():
            ...
            i = 1.0
            while (
                not interval_contains_root(lbrack, rbrack)
                and lbrack != -np.inf
            ):
                lbrack = data.min() - i
                i *= 2
...
class recipinvgauss_gen(rv_continuous):
    ...
    def _cdf(self, x, mu):
        ...
        trm2 = 1.0 / mu + x
        isqx = 1.0 / np.sqrt(x)
        return _norm_cdf(-isqx * trm1) - np.exp(2.0 / mu) * _norm_cdf(
            -isqx * trm2
        )
...
class skewnorm_gen(rv_continuous):
    ...
    def _skewnorm_odd_moments(self):
        skewnorm_odd_moments = {
            ...
            13: Polynomial(
                [135135, -270270, 405405, -386100, 225225, -73710, 10395]
            ),
            15: Polynomial(
                [
                    2027025,
    ...
    def _munp(self, order, a):
        if order & 1:
            ...
            return (
                delta
                * self._skewnorm_odd_moments[order](delta**2)
                * _SQRT_2_OVER_PI
            )
        else:
    ...
    def fit(self, data, *args, **kwds):
        ...
        # this extracts fixed shape, location, and scale however they
        # are specified, and also leaves them in `kwds`
        data, fa, floc, fscale = _check_fit_input_parameters(
            self, data, args, kwds
        )
        method = kwds.get("method", "mle").lower()
...
class trapezoid_gen(rv_continuous):
    ...
    def _entropy(self, c, d):
        ...
        return 0.5 * (1.0 - d + c) / (1.0 + d - c) + np.log(
            0.5 * (1.0 + d - c)
        )
...
class truncnorm_gen(rv_continuous):
    ...
    def _ppf(self, q, a, b):
        ...
        def ppf_left(q, a, b):
            log_Phi_x = _log_sum(
                _norm_logcdf(a), np.log(q) + _log_gauss_mass(a, b)
            )
            return sc.ndtri_exp(log_Phi_x)

        def ppf_right(q, a, b):
            log_Phi_x = _log_sum(
                _norm_logcdf(-b), np.log1p(-q) + _log_gauss_mass(a, b)
            )
            return -sc.ndtri_exp(log_Phi_x)
    ...
    def _isf(self, q, a, b):
        ...
        def isf_left(q, a, b):
            log_Phi_x = _log_diff(
                _norm_logcdf(b), np.log(q) + _log_gauss_mass(a, b)
            )
    ...
    def _stats(self, a, b, moments="mv"):
        ...
        def _truncnorm_stats_scalar(a, b, pA, pB, moments):
            ...
            probs = [pA, -pB]
            vals = _lazywhere(
                probs, [probs, [a, b]], lambda x, y: x * y, fillvalue=0
            )
            m2 = 1 + np.sum(vals)
            vals = _lazywhere(
                probs,
        ...
        _truncnorm_stats = np.vectorize(
            _truncnorm_stats_scalar, excluded=("moments",)
        )
...
class truncpareto_gen(rv_continuous):
    ...
    def _logpdf(self, x, b, c):
        return (
            np.log(b) - np.log(-np.expm1(-b * np.log(c))) - (b + 1) * np.log(x)
        )
    ...
    def fit(self, data, *args, **kwds):
        ...
        def get_b(c, loc, scale):
            ...
            quot = (harm_m - 1) / log_m
            return (
                1 - (quot - 1) / (quot - (1 - 1 / c) * harm_m / np.log(c))
            ) / log_m
        ...
        if (
            ...
            if fb is None:
                ...
                i = 0
                while (lbrack > -np.inf) and (
                    dL_dLoc(lbrack) * dL_dLoc(rbrack) >= 0
                ):
                    i += 1
                    lbrack = rbrack - np.power(2.0, i)
                if not lbrack > -np.inf:
                ...
                up_bound_b = min(
                    1 / log_mean(std_data), 1 / (harm_mean(std_data) - 1)
                )
                if not (b < up_bound_b):
                    return fallback(data, *args, **kwds)
            else:
            ...
            if fc and (floc is not None) and fscale:
                if data.max() > fc * fscale + floc:
                    raise FitDataError(
                        "truncpareto", lower=1, upper=get_c(loc, scale)
                    )
            ...
            if fb is None:
                ...
                try:
                    res = root_scalar(
                        dL_dB, (logc, logm), bracket=(lbrack, rbrack)
                    )
...
class vonmises_gen(rv_continuous):
    ...
    def expect(
        ...
        **kwds,
        ...
        return super().expect(
            func, args, loc, scale, lb, ub, conditional, **kwds
        )
...
class gennorm_gen(rv_continuous):
    ...
    def _ppf(self, x, beta):
        ...
        return c * sc.gammainccinv(1.0 / beta, (1.0 + c) - 2.0 * c * x) ** (
            1.0 / beta
        )
...
class crystalball_gen(rv_continuous):
    ...
    def _cdf(self, x, beta, m):
        ...
        def rhs(x, beta, m):
            return (m / beta) * np.exp(-(beta**2) / 2.0) / (
                m - 1
    ...
    def _ppf(self, p, beta, m):
        ...
        def ppf_greater(p, beta, m):
            ...
            N = 1 / (C + _norm_pdf_C * _norm_cdf(beta))
            return _norm_ppf(
                _norm_cdf(-beta) + (1 / _norm_pdf_C) * (p / N - C)
            )
...
class argus_gen(rv_continuous):
    ...
    def _rvs(self, chi, size=None, random_state=None):
        chi = np.asarray(chi)
        if chi.size == 1:
            out = self._rvs_scalar(
                chi, numsamples=size, random_state=random_state
            )
        else:
            ...
            out = np.empty(size)
            it = np.nditer(
                [chi], flags=["multi_index"], op_flags=[["readonly"]]
            )
            while not it.finished:
                idx = tuple(
                    (it.multi_index[j] if not bc[j] else slice(None))
...
class rv_histogram(rv_continuous):
    ...
    def _munp(self, n):
        ...
        integrals = (
            self._hbins[1:] ** (n + 1) - self._hbins[:-1] ** (n + 1)
        ) / (n + 1)
    ...
    def _entropy(self):
        ...
        res = _lazywhere(
            self._hpdf[1:-1] > 0.0, (self._hpdf[1:-1],), np.log, 0.0
        )
...
studentized_range = studentized_range_gen(
    name="studentized_range", a=0, b=np.inf
)
...
class rel_breitwigner_gen(rv_continuous):
    ...
    def _cdf(self, x, rho):
        ...
        result = np.sqrt(-1 + 1j / rho) * np.arctan(
