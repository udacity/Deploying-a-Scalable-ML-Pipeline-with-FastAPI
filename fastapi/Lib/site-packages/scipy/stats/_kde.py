...
class gaussian_kde:
    ...
    def integrate_kde(self, other):
        ...
        for i in range(small.n):
            ...
            energies = sum(diff * tdiff, axis=0) / 2.0
            result += (
                sum(exp(-energies) * large.weights, axis=0) * small.weights[i]
            )
    ...
    def set_bandwidth(self, bw_method=None):
        ...
        if bw_method is None:
            ...
            msg = (
                "`bw_method` should be 'scott', 'silverman', a scalar "
                "or a callable."
            )
            raise ValueError(msg)
    ...
    def _compute_covariance(self):
        ...
        if not hasattr(self, "_data_cho_cov"):
            ...
            self._data_cho_cov = linalg.cholesky(
                self._data_covariance, lower=True
            )

        self.covariance = self._data_covariance * self.factor**2
        self.cho_cov = (self._data_cho_cov * self.factor).astype(np.float64)
        self.log_det = (
            2 * np.log(np.diag(self.cho_cov * np.sqrt(2 * pi))).sum()
        )
