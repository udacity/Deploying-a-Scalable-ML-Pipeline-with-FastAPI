...
def _chk_size(a, b):
    ...
    if na != nb:
        raise ValueError(
            "The size of the input array should match!" f" ({na} <> {nb})"
        )
    return (a, b, na)
...
def rankdata(data, axis=None, use_missing=False):
    ...
    if axis is None:
        ...
        return ma.apply_along_axis(_rank1d, axis, data, use_missing).view(
            ndarray
        )
...
def spearmanr(
    ...
    def _spearmanr_2cols(x):
        ...
        if rs.shape == (2, 2):
            res = scipy.stats._stats_py.SignificanceResult(
                rs[1, 0], prob[1, 0]
            )
...
def kendalltau(
    ...
    if use_ties:
        ...
        denom = ma.sqrt(
            (n * (n - 1) - corr_x) / 2.0 * (n * (n - 1) - corr_y) / 2.0
        )
    else:
        denom = n * (n - 1) / 2.0
    tau = (C - D) / denom
    ...
    if not xties and not yties and method == "exact":
        ...
        if use_ties:
            ...
            if n > 2:
                v2 = np.sum(
                    [v * k * (k - 1) * (k - 2) for (k, v) in xties.items()],
                    dtype=float,
                ) * np.sum(
                    [v * k * (k - 1) * (k - 2) for (k, v) in yties.items()],
                    dtype=float,
                )
                v2 /= 9.0 * n * (n - 1) * (n - 2)
            else:
...
def kendalltau_seasonal(x):
    ...
    denom_tot = (
        ma.sqrt(1.0 * n_tot * (n_tot - 1) * (n_tot * (n_tot - 1) - corr_ties))
        / 2.0
    )
...
PointbiserialrResult = namedtuple(
...
def kruskal(*args):
    ...
    # Tie correction
    ties = count_tied_groups(ranks)
    T = 1.0 - sum(v * (k**3 - k) for (k, v) in ties.items()) / float(
        ntot**3 - ntot
    )
    if T == 0:
...
def trimr(a, limits=None, inclusive=(True, True), axis=None):
    ...
    if axis is None:
        ...
        return ma.apply_along_axis(
            _trimr1D, axis, a, lolim, uplim, loinc, upinc
        )
...
def trimboth(data, proportiontocut=0.2, inclusive=(True, True), axis=None):
    ...
    return trimr(
        data,
        limits=(proportiontocut, proportiontocut),
        inclusive=inclusive,
        axis=axis,
    )
...
def trimmed_mean(
    a, limits=(0.1, 0.1), inclusive=(1, 1), relative=True, axis=None
):
    ...
    if relative:
        return trimr(a, limits=limits, inclusive=inclusive, axis=axis).mean(
            axis=axis
        )
    else:
