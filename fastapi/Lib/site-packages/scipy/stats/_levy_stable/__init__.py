...
def _fitstart_S1(data):
    ...
    phi_3 = RectBivariateSpline(
        beta_range, alpha_range, nu_c_table, kx=1, ky=1, s=0
    )

    def phi_3_1(beta, alpha):
        return phi_3(beta, alpha) if beta > 0 else phi_3(-beta, alpha)

    phi_5 = RectBivariateSpline(
        beta_range, alpha_range, nu_zeta_table, kx=1, ky=1, s=0
    )
...
def pdf_from_cf_with_fft(cf, h=0.01, q=9, level=3):
    ...
    if level > 1:
        ...
        s1 = (
            (-1) ** steps
            * Cotes[n, 0]
            * np.fft.fft((-1) ** steps * cf(-L + h * steps))
        )
