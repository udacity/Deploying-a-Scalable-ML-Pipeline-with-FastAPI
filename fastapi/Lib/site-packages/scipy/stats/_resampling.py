...
def _bca_interval(data, statistic, axis, alpha, theta_hat_b, batch):
    ...
    theta_hat_j_dot = [
        theta_hat_i.mean(axis=-1, keepdims=True)
        for theta_hat_i in theta_hat_ji
    ]

    U_ji = [
        (n - 1) * (theta_hat_dot - theta_hat_i)
        for theta_hat_dot, theta_hat_i, n in zip(
            theta_hat_j_dot, theta_hat_ji, n_j
        )
    ]
...
def _bootstrap_iv(
    ...
    if (
        ...
        or not bootstrap_result.bootstrap_distribution.size
...
def monte_carlo_test(
    ...
    *,
    ...
    null_distribution = xp.reshape(
        null_distribution, [-1] + [1] * observed.ndim
    )
    ...
    def less(null_distribution, observed):
        ...
        pvalues = (xp.sum(cmps, axis=0, dtype=dtype) + 1.0) / (
            n_resamples + 1.0
        )
        return pvalues
...
def _all_partitions_concatenated(ns):
    ...
    for partitioning in all_partitions_n(z, ns[:]):
        x = np.concatenate(
            [list(partition) for partition in partitioning]
        ).astype(int)
        yield x
...
def _calculate_null_both(
    data, statistic, n_permutations, batch, random_state=None
):
    """
...
class MonteCarloMethod(ResamplingMethod):
    ...
    def _asdict(self):
        ...
        return dict(
            n_resamples=self.n_resamples, batch=self.batch, rvs=self.rvs
        )
