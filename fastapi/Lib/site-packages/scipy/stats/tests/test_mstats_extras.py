...
def test_trimmed_mean_ci():
    ...
    assert_equal(
        np.round(ms.trimmed_mean_ci(data, (0.2, 0.2)), 1), [561.8, 630.6]
    )
...
def test_idealfourths():
    ...
    test = np.arange(100)
    assert_almost_equal(
        np.asarray(ms.idealfourths(test)), [24.416667, 74.583333], 6
    )
    ...
    assert_almost_equal(
        ms.idealfourths(test_2D, axis=1), test.repeat(2).reshape(-1, 2)
    )
    test = [0, 0]
