...
class TestVariation:
    ...
    def test_ddof(self, xp):
        x = xp.arange(9.0)
        xp_assert_close(
            variation(x, ddof=1), xp.asarray(math.sqrt(60 / 8) / 4)
        )
    ...
    @pytest.mark.parametrize(
        "nan_policy, expected",
        [("propagate", np.nan), ("omit", np.sqrt(20 / 3) / 4)],
    )
    @skip_xp_backends(
    ...
    @skip_xp_backends(
        np_only=True, reasons=["`keepdims` only supports NumPy backend"]
    )
    ...
    def test_2d_size_zero_with_axis(self, axis, expected, xp):
        ...
        with suppress_warnings() as sup:
            ...
            if axis != 0:
                if is_numpy(xp):
                    with pytest.warns(
                        SmallSampleWarning, match="See documentation..."
                    ):
    ...
    def test_neg_inf_nan(self, xp):
        ...
        xp_assert_equal(
            variation(x2, axis=1, ddof=2, nan_policy="omit"),
            [-xp.inf, -xp.inf],
        )
    ...
    def test_combined_edge_cases(self, nan_policy, xp):
        x = xp.array(
            [[0, 10, xp.nan, 1], [0, -5, xp.nan, 2], [0, -5, xp.nan, 3]]
        )
        if nan_policy == "omit":
