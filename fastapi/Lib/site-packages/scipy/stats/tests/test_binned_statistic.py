...
class TestBinnedStatistic:
    ...
    def test_non_finite_inputs_and_int_bins(self):
        ...
        assert_raises(
            ValueError, binned_statistic, u, x, "std", bins=np.int64(10)
        )
        u[0] = np.nan
    ...
    def test_1d_bincode(self):
        ...
        bc2 = np.array(
            [3, 2, 1, 3, 2, 3, 3, 3, 3, 1, 1, 3, 3, 1, 2, 3, 1, 1, 2, 1]
        )
    ...
    def test_dd_binned_statistic_result(self):
        ...
        result = binned_statistic_dd(
            x, v, "mean", binned_statistic_result=result
        )
    ...
    def test_dd_range_errors(self):
        ...
        with assert_raises(
            ValueError, match="In range, start must be <= stop"
        ):
        ...
        with assert_raises(
            ...
            binned_statistic_dd(
                [self.x, self.y], self.v, range=[[1, 0], [0, 1]]
            )
        with assert_raises(
            ValueError, match="In dimension 2 of range, start must be <= stop"
        ):
            binned_statistic_dd(
                [self.x, self.y], self.v, range=[[0, 1], [1, 0]]
            )
        with assert_raises(
