...
failing_fits = {
    "MM": mm_failing_fits + mm_XXslow_fits,
    "MLE": mle_failing_fits,
}
...
def test_cont_fit(distname, arg, method):
    ...
    if distname in failing_fits[method] and not run_xfail:
        ...
        msg = (
            "Failure expected; set environment variable SCIPY_XFAIL=1 to run."
        )
        pytest.xfail(msg)
    ...
    diffthreshold = np.max(
        np.vstack(
            [truearg * thresh_percent, np.full(distfn.numargs + 2, thresh_min)]
        ),
        0,
    )
...
class TestFit:
    ...
    def basic_fit_test(self, dist_name, method):
        ...
        with npt.suppress_warnings() as sup:
            ...
            res = stats.fit(
                dist, data, bounds, method=method, optimizer=self.opt
            )
    ...
    def test_argus(self, method):
        ...
        res = stats.fit(
            dist, data, shape_bounds, optimizer=self.opt, method=method
        )
...
class TestGoodnessOfFit:
    ...
    def test_against_anderson_gumbel_r(self):
        ...
        res = goodness_of_fit(
            stats.gumbel_r, x, statistic="ad", random_state=rng
        )
