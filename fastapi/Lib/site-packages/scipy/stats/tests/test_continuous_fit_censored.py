...
def test_cauchy_mixed():
    ...
    * one interval-censored [[4, 8]].
    ...
    """
    data = CensoredData(
        uncensored=[1, 10], left=[1], right=[30], interval=[[4, 8]]
    )
    loc, scale = cauchy.fit(data, optimizer=optimizer)
...
def test_invweibull():
    ...
    data = CensoredData(
        uncensored=[2, 3, 9], right=[10, 10], interval=[[0, 1]]
    )
...
def test_nct():
    ...
    with np.errstate(over="ignore"):  # remove context when gh-14901 is closed
        df, nc, loc, scale = nct.fit(
            data, floc=0, fscale=1, optimizer=optimizer
        )
...
def test_ncx2():
    ...
    * 2 right-censored values [8, 8].
    ...
    """
    data = CensoredData(
        uncensored=[2.7, 0.2, 6.5, 0.4, 0.1],
        right=[8, 8],
        interval=[[0.6, 1.0]],
    )
    with np.errstate(over="ignore"):  # remove context when gh-14901 is closed
        df, ncp, loc, scale = ncx2.fit(
            data, floc=0, fscale=1, optimizer=optimizer
        )
...
def test_weibull_min_sas2():
    ...
    c, loc, scale = weibull_min.fit(
        data, 1, loc=100, scale=100, optimizer=optimizer
    )
