...
class TestCensoredData:
    ...
    def test_basic(self):
        ...
        interval = [[2, 3]]
        data = CensoredData(
            uncensored, left=left, right=right, interval=interval
        )
        ...
        assert_equal(
            udata,
            np.concatenate(
                (uncensored, left, right, np.mean(interval, axis=1))
            ),
        )
    ...
    def test_invalid_constructor_args(self):
        ...
        with pytest.raises(
            ValueError, match="both values must not be infinite"
        ):
            CensoredData(interval=[[1, 3], [2, 9], [np.inf, np.inf]])

        with pytest.raises(
            ValueError, match="left value must not exceed the right"
        ):
    ...
    def test_invalid_left_right_censored_args(self, func):
        with pytest.raises(ValueError, match="`x` must be one-dimensional"):
            func([[1, 2, 3]], [0, 1, 1])
        with pytest.raises(
            ValueError, match="`censored` must be one-dimensional"
        ):
            func([1, 2, 3], [[0, 1, 1]])
    ...
    def test_invalid_censored_args(self):
        with pytest.raises(
            ValueError, match="`low` must be a one-dimensional"
        ):
            CensoredData.interval_censored(low=[[3]], high=[4, 5])
        with pytest.raises(
            ValueError, match="`high` must be a one-dimensional"
        ):
            CensoredData.interval_censored(low=[3], high=[[4, 5]])
