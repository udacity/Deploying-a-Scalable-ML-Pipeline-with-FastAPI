...
@pytest.mark.parametrize(
    "distname,arg,first_case", cases_test_discrete_basic()
)
...
def check_cdf_ppf(distfn, arg, supp, msg):
    ...
    npt.assert_array_equal(
        distfn.ppf(cdf_supp0, *arg), supp, msg + "-roundtrip"
    )
    # Repeat the same calculation, but with the CDF values decreased by 1e-8.
    npt.assert_array_equal(
        distfn.ppf(distfn.cdf(supp, *arg) - 1e-8, *arg),
        supp,
        msg + "-roundtrip",
    )
...
def check_pmf_cdf(distfn, arg, distname):
    ...
    if distname == "skellam":  # ncx2 accuracy
        atol, rtol = 1e-5, 1e-5
    npt.assert_allclose(
        cdfs - cdfs[0], pmfs_cum - pmfs_cum[0], atol=atol, rtol=rtol
    )
...
def test_rv_sample():
    ...
    with np.errstate(divide="ignore"):
        ...
        assert_allclose(
            dist.logcdf(xk_right), np.log(dist.cdf(xk_right)), atol=1e-15
        )
        assert_allclose(
            dist.logcdf(xk_left), np.log(dist.cdf(xk_left)), atol=1e-15
        )
    ...
    with np.errstate(divide="ignore"):
        ...
        assert_allclose(
            dist.logsf(xk_right), np.log(dist.sf(xk_right)), atol=1e-15
        )
        assert_allclose(
            dist.logsf(xk_left), np.log(dist.sf(xk_left)), atol=1e-15
        )
