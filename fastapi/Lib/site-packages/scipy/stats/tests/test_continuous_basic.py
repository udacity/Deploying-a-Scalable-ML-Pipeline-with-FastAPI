        ...
        for method in ["MLE", "MM"]:
            for fix_args in [True, False]:
                if method == "MLE" and distname in slow_fit_mle:
                    yield pytest.param(
                        distname, arg, method, fix_args, marks=slow
                    )
                    continue
                if method == "MLE" and distname in xslow_fit_mle:
                    yield pytest.param(
                        distname, arg, method, fix_args, marks=xslow
                    )
                    continue
                if method == "MLE" and distname in xfail_fit_mle:
                    yield pytest.param(
                        distname, arg, method, fix_args, marks=fail
                    )
                    continue
                if method == "MLE" and distname in skip_fit_mle:
                    yield pytest.param(
                        distname, arg, method, fix_args, marks=skip
                    )
                    continue
...
def cases_test_moments():
    ...
    for distname, arg in distcont[:] + histogram_test_instances:
        ...
        yield pytest.param(
            distname, arg, cond1, cond2, cond3, False, marks=marks
        )
...
def test_rvs_gh2069_regression():
    ...
    d = np.diff(vals)
    npt.assert_(
        np.all(d != 0), "All the values are equal, but they shouldn't be!"
    )
    ...
    vals = stats.norm.rvs(
        loc=np.array([[0], [0]]), scale=np.ones(5), random_state=rng
    )
    ...
    assert_raises(
        ValueError, stats.norm.rvs, [[0, 0], [0, 0]], [[1, 1], [1, 1]], 1
    )
...
def check_pdf(distfn, arg, msg):
    ...
    if (pdfv < 1e-4) or (pdfv > 1e4):
        ...
        pdfv = distfn.pdf(median, *arg)
    cdfdiff = (
        (distfn.cdf(median + eps, *arg) - distfn.cdf(median - eps, *arg))
        / eps
        / 2.0
    )
...
def check_retrieving_support(distfn, args):
    ...
    npt.assert_almost_equal(
        np.array(supp) * scale + loc, np.array(supp_loc_scale)
    )
...
def check_fit_args(distfn, arg, rvs, method):
    with np.errstate(all="ignore"), npt.suppress_warnings() as sup:
        ...
        sup.filter(
            category=RuntimeWarning, message="floating point number truncated"
        )
...
def test_moments_with_array_gh12192_regression():
    ...
    vals2 = stats.norm.moment(
        order=1, loc=np.array([1, 2, 3]), scale=[-3, 1, 0]
    )
...
def test_broadcasting_in_moments_gh12192_regression():
    ...
    vals1 = stats.norm.moment(
        order=1, loc=np.array([[1], [2], [3]]), scale=[1, 2, 3]
    )
