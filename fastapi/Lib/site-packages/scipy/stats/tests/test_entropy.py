...
class TestEntropy:
    ...
    def test_entropy_2d(self, xp):
        ...
        xp_assert_close(
            stats.entropy(pk, qk), xp.asarray([0.1933259, 0.18609809])
        )
    ...
    def test_entropy_2d_zero(self, xp):
        ...
        xp_assert_close(
            stats.entropy(pk, qk), xp.asarray([xp.inf, 0.18609809])
        )

        pk = xp.asarray([[0.0, 0.2], [0.6, 0.3], [0.3, 0.5]])
        xp_assert_close(
            stats.entropy(pk, qk), xp.asarray([0.17403988, 0.18609809])
        )
    ...
    def test_entropy_transposed(self, xp):
        ...
        qk = xp.asarray([[0.2, 0.1], [0.3, 0.6], [0.5, 0.3]])
        xp_assert_close(
            stats.entropy(pk.T, qk.T), stats.entropy(pk, qk, axis=1)
        )
...
class TestDifferentialEntropy:
    ...
    def test_differential_entropy_vasicek(self):
        ...
        entropy = stats.differential_entropy(
            values, window_length=1, method="vasicek"
        )
        assert_allclose(entropy, 1.122044, rtol=1e-6)

        entropy = stats.differential_entropy(
            values, window_length=8, method="vasicek"
        )
    ...
    @pytest.mark.parametrize(
        "method", ["vasicek", "van es", "ebrahimi", "correa"]
    )
    ...
    @pytest.mark.parametrize(
        "method, expected", list(norm_rmse_std_cases.items())
    )
    def test_norm_rmse_std(self, method, expected):
        ...
        res = stats.differential_entropy(
            rvs, window_length=m, method=method, axis=-1
        )
        assert_allclose(
            np.sqrt(np.mean((res - true_entropy) ** 2)),
            rmse_expected,
            atol=0.005,
        )
