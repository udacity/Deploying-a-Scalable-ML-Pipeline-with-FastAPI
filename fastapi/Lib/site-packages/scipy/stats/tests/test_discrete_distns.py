...
class TestZipfian:
    ...
    def test_zipfian_continuity(self):
        ...
        k = np.arange(1, N + 1)
        assert_allclose(
            zipfian.pmf(k, alt1, N), zipfian.pmf(k, agt1, N), rtol=5e-7
        )
        assert_allclose(
            zipfian.cdf(k, alt1, N), zipfian.cdf(k, agt1, N), rtol=5e-7
        )
        assert_allclose(
            zipfian.sf(k, alt1, N), zipfian.sf(k, agt1, N), rtol=5e-7
        )
        assert_allclose(
    ...
    np.random.seed(0)
    naive_tests = np.vstack(
        (np.logspace(-2, 1, 10), np.random.randint(2, 40, 10))
    ).T
...
class TestNCH:
    ...
    def test_nch_hypergeom(self, dist_name):
        ...
        assert_allclose(
            dist.pmf(x, N, m1, n, odds=1), hypergeom.pmf(x, N, m1, n)
        )
    ...
    def test_nchypergeom_fisher_naive(self):
        ...
        assert_allclose(
            nchypergeom_fisher.stats(N, m1, n, odds, moments="m"), mean
        )
        assert_allclose(
            nchypergeom_fisher.stats(N, m1, n, odds, moments="v"), var
        )
    ...
    def test_nchypergeom_wallenius_naive(self):
        ...
        with suppress_warnings() as sup:
            sup.filter(
                RuntimeWarning, message="invalid value encountered in mean"
            )
            assert_allclose(
                nchypergeom_wallenius.mean(N, m1, n, w),
                mean(N, m1, n, w),
                rtol=2e-2,
            )
        ...
        def pmf(x, N, m1, n, w):
            ...
            def f(x):
                ...
                t2 = special_binom(m2, n - x)
                the_integral = quad(
                    integrand, 0, 1, epsrel=1e-16, epsabs=1e-16
                )
    ...
    def test_wallenius_against_mpmath(self):
        ...
        assert_allclose(
            nchypergeom_wallenius.pmf(sup, M, n, N, odds),
            pmf,
            rtol=1e-13,
            atol=1e-13,
        )
        assert_allclose(
            nchypergeom_wallenius.mean(M, n, N, odds), mean, rtol=1e-13
        )
        assert_allclose(
            nchypergeom_wallenius.var(M, n, N, odds), var, rtol=1e-11
        )
...
class TestBetaNBinom:
    ...
    def test_betanbinom_kurtosis(self, n, a, b, ref):
        ...
        assert_allclose(
            betanbinom.stats(n, a, b, moments="k"), ref, rtol=3e-15
        )
