...
def check_moment(distfn, arg, m, v, msg):
    ...
    if not np.isinf(m):
        npt.assert_almost_equal(
            m1, m, decimal=10, err_msg=msg + " - 1st moment"
        )
    else:  # or np.isnan(m1),
        npt.assert_(
            np.isinf(m1), msg + " - 1st moment -infinite, m1=%s" % str(m1)
        )
...
def check_private_entropy(distfn, args, superclass):
    ...
    npt.assert_allclose(
        distfn._entropy(*args), superclass._entropy(distfn, *args)
    )
...
def check_named_args(distfn, x, shape_args, defaults, meths):
    ...
    while names:
        ...
        if "n" not in k.keys():
            ...
            npt.assert_equal(
                distfn.moment(1, *a, **k), distfn.moment(1, *shape_args)
            )
...
def check_meth_dtype(distfn, arg, meths):
    ...
    x_cast = [
        x0.astype(tp) for tp in (np_long, np.float16, np.float32, np.float64)
    ]
...
def check_pickling(distfn, args):
    ...
    npt.assert_equal(
        distfn.cdf(medians[0], *args), unpickled.cdf(medians[1], *args)
    )
    ...
    if hasattr(distfn, "fit"):
        ...
        assert (
            fit_function.__name__ == unpickled_fit_function.__name__ == "fit"
        )
