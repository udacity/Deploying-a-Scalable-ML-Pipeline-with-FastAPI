...
def test_chi2_contingency_g():
    ...
    g, p, dof, e = chi2_contingency(
        c, lambda_="log-likelihood", correction=False
    )
    assert_allclose(g, 2 * xlogy(c, c / e).sum())

    g, p, dof, e = chi2_contingency(
        c, lambda_="log-likelihood", correction=True
    )
...
def test_bad_association_args():
    ...
    assert_raises(
        ValueError,
        association,
        np.array([[1, 2], ["dd", 4]], dtype=object),
        "cramer",
    )
...
def test_assoc(stat, expected):
    # 2d Array
    obs1 = np.array(
        [[12, 13, 14, 15, 16], [17, 16, 18, 19, 11], [9, 15, 14, 12, 11]]
    )
