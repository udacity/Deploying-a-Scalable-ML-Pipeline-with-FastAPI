...
def nan_policy_1d(
    ...
    **kwds
    ...
    if nan_policy == "raise":
        ...
        nan_policy == "propagate" and hypotest not in override_propagate_funcs
    ):
...
def test_axis_nan_policy_fast(
    ...
    if (
        hypotest in {stats.cramervonmises_2samp, stats.kruskal}
        and not SCIPY_XSLOW
    ):
...
if SCIPY_XSLOW:
    ...
    # `kurtosistest` and `normaltest` when sample size < 20
    @pytest.mark.filterwarnings(
        "ignore:`kurtosistest` p-value may be:UserWarning"
    )
    # `foneway`
    @pytest.mark.filterwarnings(
        "ignore:all input arrays have length 1.:RuntimeWarning"
    ...
    # `bartlett`, `tvar`, `tstd`, `tsem`
    @pytest.mark.filterwarnings(
        "ignore:Degrees of freedom <= 0 for:RuntimeWarning"
    )
    # kstat, kstatvar, ttest_1samp, ttest_rel, ttest_ind, ttest_ci, brunnermunzel
    # mood, levene, fligner, bartlett
    @pytest.mark.filterwarnings(
    ...
    @pytest.mark.parametrize(
        ("data_generator"), ("all_nans", "all_finite", "mixed")
    )
...
def _axis_nan_policy_test(
    ...
    for i, _ in np.ndenumerate(np.zeros(output_shape)):
        ...
        try:
            ...
            ea_str = str(ea)
            if any(
                [str(ea_str).startswith(msg) for msg in too_small_messages]
            ):
    ...
    if (
        ...
        with pytest.warns(SmallSampleWarning, match=too_small_nd_omit):
            res = hypotest(
                *data, axis=axis, nan_policy=nan_policy, *args, **kwds
            )
...
def test_axis_nan_policy_axis_is_None(
    ...
    if nan_policy == "raise" and data_generator not in {"all_finite", "empty"}:
        ...
        with pytest.raises(ValueError, match=message):
            hypotest(
                *data_raveled, axis=None, nan_policy=nan_policy, *args, **kwds
            )
    ...
    with warnings.catch_warnings():  # treat warnings as errors
        ...
        try:
            res1db = hypotest(
                *data_raveled, *args, nan_policy=nan_policy, **kwds
            )
        ...
        try:
            res1dc = hypotest(
                *data, *args, axis=None, nan_policy=nan_policy, **kwds
            )
        except SmallSampleWarning as ec:
            ec_str = str(ec)

    if (
        ea_str or eb_str or ec_str
    ):  # *if* there is some sort of error or warning
...
def test_keepdims(
    ...
    for axis in axis_cases:
        ...
        res = unpacker(
            hypotest(*data, *args, axis=axis, keepdims=True, **kwds)
        )
        res_base = unpacker(
            hypotest(*data, *args, axis=axis, keepdims=False, **kwds)
        )
        nan_res = unpacker(
            hypotest(
                *nan_data,
        ...
        for r, r_base, rn, rn_base in zip(
            res, res_base, nan_res, nan_res_base
        ):
...
@pytest.mark.parametrize(
    ("fun", "nsamp"), [(stats.kstat, 1), (stats.kstatvar, 1)]
)
...
def _check_arrays_broadcastable(arrays, axis):
    ...
    for dim in range(
        1, n_dims + 1
    ):  # we'll index from -1 to -n_dims, inclusive
        if -dim == axis:
...
def test_empty(hypotest, args, kwds, n_samples, n_outputs, paired, unpacker):
    ...
    if (
        hypotest in {stats.kruskal, stats.friedmanchisquare}
        and not SCIPY_XSLOW
    ):
        pytest.skip("Too slow.")
...
def test_mixed_mask_nan_1():
    ...
    res4 = stats.wilcoxon(
        a_masked3, b_masked3, nan_policy="propagate", axis=axis
    )
...
@pytest.mark.parametrize(
    ("axis"), list(permutations(range(-3, 3), 2)) + [(-4, 1)]
)
