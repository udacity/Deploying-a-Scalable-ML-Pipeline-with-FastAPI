...
if sys.implementation.name == "pypy" and sys.implementation.version < (
    7,
    3,
    10,
):
...
def test_threading_behaviour():
    ...
    def func1():
        ...
        rng = TransformedDensityRejection(
            dist, domain=(10, 100), random_state=12
        )
        try:
            rng.rvs(100000)
    ...
    def func2():
        ...
        rng = TransformedDensityRejection(
            dist, domain=(10, 100), random_state=2
        )
        try:
            rng.rvs(100000)
        except ValueError as e:
...
class TestQRVS:
    ...
    def test_QRVS_shape_consistency(
        self, qrng, size_in, size_out, d_in, d_out, method
    ):
...
class TestTransformedDensityRejection:
    ...
    def test_bad_construction_points_scalar(self, construction_points):
        with pytest.raises(
            ValueError,
            match=r"`construction_points` must be " r"a positive integer.",
        ):
            TransformedDensityRejection(
    ...
    def test_bad_construction_points_array(self):
        ...
        with pytest.raises(
            UNURANError, match=r"50 : bad construction " r"points."
        ):
            TransformedDensityRejection(
                StandardNormal(), construction_points=construction_points
            )
...
class TestNumericalInversePolynomial:
    ...
    class dist3:
        ...
        def cdf(self, x):
            return (
                ...
                + 0.09
                / (2 * np.pi)
                * (np.cos(10 * np.pi) - np.cos(2 * np.pi * x))
            )
    ...
    def test_basic_all_scipy_dists(self, distname, params):
        ...
        dist = (
            getattr(stats, distname) if isinstance(distname, str) else distname
        )
        dist = dist(*params)
        with suppress_warnings() as sup:
            sup.filter(RuntimeWarning)
    ...
    def test_bad_u_resolution(self, u_resolution):
        ...
        with pytest.raises(ValueError, match=msg):
            NumericalInversePolynomial(
                StandardNormal(), u_resolution=u_resolution
            )
