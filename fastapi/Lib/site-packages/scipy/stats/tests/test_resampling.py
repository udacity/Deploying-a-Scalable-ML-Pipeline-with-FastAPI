...
def test_bootstrap_vectorized(method, axis, paired):
    ...
    assert res1.bootstrap_distribution.shape == res1.standard_error.shape + (
        100,
    )
    ...
    assert_allclose(
        res2.confidence_interval.high, res1.confidence_interval.high
    )
...
def test_bootstrap_against_theory(method):
    ...
    assert_allclose(
        res.confidence_interval.low, dist.ppf(1 - alpha), rtol=5e-4
    )
...
def test_bootstrap_against_R(method, expected):
    ...
    res = bootstrap(
        (x,), np.mean, n_resamples=1000000, method=method, random_state=0
    )
...
@pytest.mark.parametrize(
    "method, expected", tests_against_itself_1samp.items()
)
def test_bootstrap_against_itself_1samp(method, expected):
    ...
    pvalue = stats.binomtest(
        ci_contains_true, n_replications, confidence_level
    ).pvalue
    assert pvalue > 0.1
...
@pytest.mark.slow
@pytest.mark.parametrize(
    "method, expected", tests_against_itself_2samp.items()
)
...
def test_bootstrap_min():
    ...
    assert_allclose(
        -res.confidence_interval.low, res2.confidence_interval.high
    )
    assert_allclose(
        -res.confidence_interval.high, res2.confidence_interval.low
    )
...
def test_re_bootstrap(additional_resamples):
    ...
    assert_allclose(
        res.confidence_interval, ref.confidence_interval, rtol=1e-14
    )
...
def test_jackknife_resample():
    ...
    y2 = np.concatenate(
        list(_resampling._jackknife_resample(x, batch=2)), axis=-2
    )
...
def test_vector_valued_statistic_gh17715():
    ...
    assert_allclose(
        res.confidence_interval.high[0],
        ref.confidence_interval.high,
        atol=1e-15,
    )
...
class TestMonteCarloHypothesisTest:
    ...
    @pytest.mark.xfail_on_32bit(
        "Statistic may not depend on sample order on 32-bit"
    )
    def test_finite_precision_statistic(self):
        ...
        def rvs(size):
            return 1.0 * stats.bernoulli(p=0.333).rvs(
...
class TestPower:
    ...
    def test_ttest_1samp_power(self):
        ...
        ref = [
            [
                [0.0126515, 0.10269751, 0.40415802],
...
class TestPermutationTest:
    ...
    def test_permutation_test_iv(self):
        ...
        with pytest.raises(ValueError, match=message):
            permutation_test(
                ([1, 2, 3], [1, 2, 3]), stat, permutation_type="ekki"
            )
        ...
        with pytest.raises(ValueError, match=message):
            permutation_test(
                ([1, 2, 3], [1, 2, 3]), stat, random_state="herring"
            )
    ...
    @pytest.mark.parametrize(
        "permutation_type", ["pairings", "samples", "independent"]
    )
    ...
    def test_randomized_test_against_exact_both(self):
        ...
        res = permutation_test(
            data, statistic, n_resamples=permutations, **kwds
        )
    ...
    def test_against_ks_2samp(self, alternative):
        ...
        def statistic1d(x, y):
            return stats.ks_2samp(
                x, y, mode="asymp", alternative=alternative
            ).statistic
    ...
    def test_against_cvm(self):
        ...
        def statistic1d(x, y):
            return stats.cramervonmises_2samp(
    ...
    def test_with_ties(self, case):
        ...
        Pr_gte_S_mean = (
            np.sum(np.abs(res.null_distribution - mean) >= np.abs(S - mean))
            / n
        )
    ...
    @pytest.mark.parametrize(
        "iterable, batch, expected", batch_generator_cases
    )
