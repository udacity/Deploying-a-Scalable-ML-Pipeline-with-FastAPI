...
def betaprime_pdf(x, a, b):
    if x > 0:
        logf = (
            (a - 1) * math.log(x) - (a + b) * math.log1p(x) - sc.betaln(a, b)
        )
...
def crystalball_pdf(x, b, m):
    ...
    return math.exp(
        m * math.log(m / b) - 0.5 * b * b - m * math.log(m / b - b - x)
    )
...
def invgauss_pdf(x, mu):
    ...
    if x > 0:
        return math.exp(
            -1.5 * math.log(x) - (x - mu) ** 2 / (2 * x * mu**2) - lfm
        )
    return 0.0
...
PINV_CONFIG = {
    ...
    "burr": {
        ...
        "check_pinv_params": lambda a, b: (min(a, b) >= 0.3)
        and (max(a, b) <= 50),
    },
    "burr12": {
        "pdf": burr12_pdf,
        "center": lambda a, b: (2 ** (1 / b) - 1) ** (1 / a),
        "check_pinv_params": lambda a, b: (min(a, b) >= 0.2)
        and (max(a, b) <= 50),
    },
    "cauchy": {
