...
def _calc_binned_statistic(Vdim, bin_numbers, result, values, stat_func):
    ...
    for vv in builtins.range(Vdim):
        bin_map = _create_binned_data(
            bin_numbers, unique_bin_numbers, values, vv
        )
...
def _bin_edges(sample, bins=None, range=None):
    ...
    edges_dtype = (
        sample.dtype if np.issubdtype(sample.dtype, np.floating) else float
    )
    ...
    for i in builtins.range(Ndim):
        if np.isscalar(bins[i]):
            ...
            edges[i] = np.linspace(
                smin[i], smax[i], nbin[i] - 1, dtype=edges_dtype
            )
        else:
