...
def _log_nfactorial_div_n_pow_n(n):
    ...
    return (
        ...
        - n
        + _LOG_2PI / 2
        + rn * np.polyval(_STIRLING_COEFFS, rn / n)
    )
    ...
    if i == 0:
        ...
        if ip1mod2 == 0:  # i is odd
            if ip1div2 == n + 1:
                ...
                j1, j2 = (
                    ip1div2 - 1 - ll - roundf - 1,
                    ip1div2 + ll - 1 + ceilf - 1,
                )
        else:
...
def _kolmogn_Pomeranz(n, x, cdf=True):
    ...
    onem2gpower[0] = 1.0
    expnt = 0
    g_over_n, two_g_over_n, one_minus_two_g_over_n = (
        g / n,
        2 * g / n,
        (1 - 2 * g) / n,
    )
...
def _kolmogn(n, x, cdf=True):
    ...
    if t <= 1.0:  # Ruben-Gambino: 1/2n <= x <= 1/n
        ...
        if n <= 140:
            ...
            prob = np.exp(
                _log_nfactorial_div_n_pow_n(n) + n * np.log(2 * t - 1)
            )
...
def _kolmogn_p(n, x):
    ...
    if t <= 1.0:
        ...
        if n <= 140:
            ...
            prd = np.exp(
                _log_nfactorial_div_n_pow_n(n) + (n - 1) * np.log(2 * t - 1)
            )
        return prd * 2 * n**2
    if t >= n - 1:
