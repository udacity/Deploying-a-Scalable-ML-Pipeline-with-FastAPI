...
class TestConvertTemperature:
    def test_convert_temperature(self, xp):
        xp_assert_equal(
            sc.convert_temperature(xp.asarray(32.0), "f", "Celsius"),
            xp.asarray(0.0),
        )
        xp_assert_equal(
            sc.convert_temperature(
                xp.asarray([0.0, 0.0]), "celsius", "Kelvin"
            ),
        ...
        xp_assert_equal(
            sc.convert_temperature(
                xp.asarray([273.15, 273.15]), "kelvin", "F"
            ),
        ...
        xp_assert_close(
            sc.convert_temperature(
                xp.asarray([0.0, 0.0], dtype=xp.float64), "c", "r"
            ),
        ...
        xp_assert_close(
            sc.convert_temperature(
                xp.asarray([491.67, 0.0], dtype=xp.float64),
                "rankine",
                "kelvin",
            ),
    ...
    @skip_xp_backends(
        np_only=True, reasons=["Python list input uses NumPy backend"]
    )
    ...
    @skip_xp_backends(
        np_only=True, reasons=["Python int input uses NumPy backend"]
    )
...
class TestLambdaToNu:
    ...
    def test_lambda_to_nu_array_like(self, xp):
        xp_assert_equal(
            sc.lambda2nu([sc.speed_of_light, 1]), [1, sc.speed_of_light]
        )
...
class TestNuToLambda:
    ...
    def test_nu_to_lambda_array_like(self, xp):
        xp_assert_equal(
            sc.nu2lambda([sc.speed_of_light, 1]), [1, sc.speed_of_light]
        )
