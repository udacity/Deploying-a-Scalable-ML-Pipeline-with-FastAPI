...
X = np.array(
    [[1, 0, 0, 1], [2, 1, 2, 0], [0, 2, 1, 0], [0, 0, 1, 2]], dtype=float
)
...
def test_min_max(A):
    ...
    if hasattr(A, "min"):
        assert not isinstance(
            A.min(axis=1), np.matrix
        ), "Expected array, got matrix"
    if hasattr(A, "max"):
        assert not isinstance(
            A.max(axis=1), np.matrix
        ), "Expected array, got matrix"
    if hasattr(A, "argmin"):
        assert not isinstance(
            A.argmin(axis=1), np.matrix
        ), "Expected array, got matrix"
    if hasattr(A, "argmax"):
        assert not isinstance(
            A.argmax(axis=1), np.matrix
        ), "Expected array, got matrix"
...
def test_expm_multiply(B):
    ...
    npt.assert_allclose(
        spla.expm_multiply(B, np.array([1, 2])), spla.expm(B) @ [1, 2]
    )
...
def test_spsolve(B):
    ...
    npt.assert_allclose(
        spla.spsolve(B, [1, 2]), np.linalg.solve(B.todense(), [1, 2])
    )
...
def test_factorized(B):
    ...
    npt.assert_allclose(
        LU(np.array([1, 2])), np.linalg.solve(B.todense(), [1, 2])
    )
