...
class TestInt32Overflow:
    ...
    def test_bsr_1_block(self, op):
        ...
        def get_matrix():
            ...
            m = bsr_matrix(
                (data, indices, indptr), blocksize=(n, n), copy=False
            )
    ...
    def test_bsr_n_block(self, op):
        ...
        def get_matrix():
            ...
            m = bsr_matrix(
                (data, indices, indptr), blocksize=(n, 1), copy=False
            )
            del data, indptr, indices
            return m
    ...
    def _check_bsr_matmat(self, m):  # skip name check
        ...
        # _bsr_matmat
        m2 = bsr_matrix(
            np.ones((n, 2), dtype=np.int8), blocksize=(m.blocksize[1], 2)
        )
        ...
        # _bsr_matmat
        m2 = bsr_matrix(
            np.ones((2, n), dtype=np.int8), blocksize=(2, m.blocksize[0])
        )
