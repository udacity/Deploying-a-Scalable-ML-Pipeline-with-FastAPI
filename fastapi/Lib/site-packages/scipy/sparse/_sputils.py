...
def issequence(t) -> bool:
    return (
        isinstance(t, (list, tuple)) and (len(t) == 0 or np.isscalar(t[0]))
    ) or (isinstance(t, np.ndarray) and (t.ndim == 1))


def ismatrix(t) -> bool:
    return (
        isinstance(t, (list, tuple)) and len(t) > 0 and issequence(t[0])
...
def check_shape(
    args, current_shape=None, *, allow_1d=False
) -> tuple[int, ...]:
    ...
    if current_shape is None:
        ...
        if not negative_indexes:
            ...
            if remainder != 0:
                err_shape = tuple(
                    "newshape" if x < 0 else x for x in new_shape
                )
                raise ValueError(
            ...
            new_shape = (
                new_shape[:skip] + (unspecified,) + new_shape[skip + 1 :]
            )
        else:
