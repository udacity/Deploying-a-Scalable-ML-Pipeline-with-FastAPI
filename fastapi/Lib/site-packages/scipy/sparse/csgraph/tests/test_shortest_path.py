...
def test_directed():
    def check(method):
        SP = shortest_path(
            directed_G, method=method, directed=True, overwrite=False
        )
...
def test_shortest_path_indices():
    ...
    def check(func, indshape):
        outshape = indshape + (5,)
        SP = func(
            directed_G, directed=False, indices=indices.reshape(indshape)
        )
    ...
    assert_raises(
        ValueError, shortest_path, directed_G, method="FW", indices=indices
    )
...
def test_negative_cycles():
    ...
    def check(method, directed):
        assert_raises(
            NegativeCycleError, shortest_path, graph, method, directed
        )
    ...
    for directed in (True, False):
        ...
        assert_raises(
            NegativeCycleError, yen, graph, 0, 1, 1, directed=directed
        )
