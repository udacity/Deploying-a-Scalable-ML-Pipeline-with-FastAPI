...
class _cs_matrix(_data_matrix, _minmax_mixin, IndexMixin):
    ...
    def __init__(self, arg1, shape=None, dtype=None, copy=False):
        ...
        if shape is not None:
            ...
            self._shape = check_shape(
                self._swap((major_d, minor_d)), allow_1d=is_array
            )
    ...
    def _getnnz(self, axis=None):
        if axis is None:
            ...
            if axis == 0:
                return np.bincount(
                    downcast_intp_index(self.indices), minlength=N
                )
            elif axis == 1:
    ...
    def _matmul_vector(self, other):
        ...
        # output array
        result = np.zeros(
            M, dtype=upcast_char(self.dtype.char, other.dtype.char)
        )
    ...
    def _matmul_sparse(self, other):
        ...
        if nnz == 0:
            ...
            return self.__class__(
                new_shape, dtype=upcast(self.dtype, other.dtype)
            )
    ...
    def diagonal(self, k=0):
        ...
        y = np.empty(
            min(rows + min(k, 0), cols - max(k, 0)), dtype=upcast(self.dtype)
        )
        fn(
            k,
            self.shape[0],
    ...
    def _major_index_fancy(self, idx):
        ...
        row_nnz = (self.indptr[indices + 1] - self.indptr[indices]).astype(
            idx_dtype
        )
    ...
    def _major_slice(self, idx, copy=False):
        ...
        row_nnz = (
            self.indptr[start1:stop1:step] - self.indptr[start0:stop0:step]
        )
        idx_dtype = self.indices.dtype
        res_indptr = np.zeros(M + 1, dtype=idx_dtype)
        np.cumsum(row_nnz, out=res_indptr[1:])
    ...
    def _prepare_indices(self, i, j):
        ...
        def check_bounds(indices, bound):
            ...
            if idx >= bound:
                raise IndexError(
                    "index (%d) out of range (>= %d)" % (idx, bound)
                )
            idx = indices.min()
            if idx < -bound:
                raise IndexError(
    ...
    def tocoo(self, copy=True):
        if self.ndim == 1:
            csr = self.tocsr()
            return self._coo_container(
                (csr.data, (csr.indices,)), csr.shape, copy=copy
            )
