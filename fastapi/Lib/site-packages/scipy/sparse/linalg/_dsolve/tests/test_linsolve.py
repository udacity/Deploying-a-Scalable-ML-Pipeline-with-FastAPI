...
def setup_bug_8278():
    N = 2**6
    h = 1 / N
    Ah1D = scipy.sparse.diags(
        [-1, 2, -1], [-1, 0, 1], shape=(N - 1, N - 1)
    ) / (h**2)
...
class TestFactorized:
    ...
    def test_call_with_incorrectly_sized_matrix_without_umfpack(self):
        ...
        with assert_raises(ValueError, match="is of incompatible size"):
            solve(B)
        with assert_raises(
            ValueError, match="object too deep for desired array"
        ):
            solve(BB)
...
class TestLinsolve:
    ...
    def test_twodiags(self, format: str, idx_dtype: np.dtype):
        A = spdiags(
            [[1, 2, 3, 4, 5], [6, 5, 8, 9, 10]], [0, 1], 5, 5, format=format
        )
        b = array([1, 2, 3, 4, 5])
        ...
        for b in bs:
            ...
            for spmattype in [csc_matrix, csr_matrix, dok_matrix, lil_matrix]:
                ...
                if issparse(b) and x.ndim > 1:
                    ...
                    assert_(
                        isinstance(x1, np.ndarray), repr((b, spmattype, 1))
                    )
                    assert_(
                        isinstance(x2, np.ndarray), repr((b, spmattype, 2))
                    )
    ...
    def test_dtype_cast(self):
        ...
        A_complex = scipy.sparse.csr_matrix(
            [[1, 2, 0], [0, 0, 3], [4, 0, 5 + 1j]]
        )
...
class TestSplu:
    ...
    @pytest.mark.skipif(
        not hasattr(sys, "getrefcount"), reason="no sys.getrefcount"
    )
...
class TestGstrsErrors:
    ...
    def test_shape_LU(self):
        ...
        U = scipy.sparse.triu(self.A, k=1, format="csc")
        with assert_raises(
            ValueError, match="L and U must have the same dimension"
        ):
            _superlu.gstrs(
                "N",
                L.shape[0],
    ...
    def test_shape_b(self):
        ...
        U = scipy.sparse.triu(self.A, k=1, format="csc")
        with assert_raises(
            ValueError, match="right hand side array has invalid shape"
        ):
            _superlu.gstrs(
                "N",
                L.shape[0],
    ...
    def test_types_unsupported(self):
        ...
        with assert_raises(
            TypeError, match="nzvals is not of a type supported"
        ):
            _superlu.gstrs(
                "N",
                L.shape[0],
...
class TestSpsolveTriangular:
    ...
    def test_random(
        self, n, m, lower, format, unit_diagonal, choice_of_A, choice_of_b
    ):
        def random_triangle_matrix(
            n, lower=True, format="csr", choice_of_A="real"
        ):
            if choice_of_A == "real":
                dtype = np.float64
