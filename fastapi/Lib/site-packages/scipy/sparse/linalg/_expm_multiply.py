...
def expm_multiply(
    A, B, start=None, stop=None, num=None, endpoint=None, traceA=None
):
    """
...
def _expm_multiply_simple(A, B, t=1.0, traceA=None, balance=False):
    ...
    if A.shape[1] != B.shape[0]:
        raise ValueError(
            "shapes of matrices A {} and B {} are incompatible".format(
                A.shape, B.shape
            )
        )
    ident = _ident_like(A)
...
def _expm_multiply_simple_core(
    A, B, t, mu, m_star, s, tol=None, balance=False
):
    """
    A helper function.
    """
