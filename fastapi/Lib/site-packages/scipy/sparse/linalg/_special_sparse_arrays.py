...
of_the_second_derivative
    ...
    def __init__(
        self, grid_shape, *, boundary_conditions="neumann", dtype=np.int8
    ):
    ...
    def _eigenvalue_ordering(self, m):
        ...
        if m is None:
            ...
            grid_shape_min = min(
                grid_shape, tuple(np.ones_like(grid_shape) * m)
            )
        ...
        for ind, dim in enumerate(grid_shape):
            ...
            if ind > 0:
                ...
                for j in range(1, tiles):
                    L_i[j * dim : (j + 1) * dim, j * dim : (j + 1) * dim] = (
                        L_i[:dim, :dim]
                    )
                    new_dim += dim
            # 2-
            ...
            L_i.reshape((new_dim, tiles, new_dim, tiles))[:, idx, :, idx] = (
                Ltemp[:new_dim, :new_dim]
            )
    ...
    def tosparse(self):
        ...
        for i in range(N):
            ...
            L_i = dia_array(
                (data, [-1, 0, 1]), shape=(dim, dim), dtype=np.int8
            )
...
class Sakurai(LinearOperator):
    ...
    def eigenvalues(self, m=None):
        ...
        return np.flip(
            16.0 * np.power(np.cos(0.5 * k * np.pi / (self.n + 1)), 4)
        )
