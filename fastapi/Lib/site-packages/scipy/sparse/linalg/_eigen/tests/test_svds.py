...
class SVDSCommonTests:
    ...
    def test_svds_input_validation_v0_1(self, transpose, n):
        ...
        required_length = (
            A.shape[0] if self.solver == "propack" else min(A.shape)
        )
    ...
    def test_svds_parameter_k_which(self, k, which):
        ...
        if self.solver == "lobpcg":
            with pytest.warns(UserWarning, match="The problem size"):
                res = svds(
                    A, k=k, which=which, solver=self.solver, random_state=0
                )
        else:
    ...
    @pytest.mark.filterwarnings(
        "ignore:Exited", reason="Ignore LOBPCG early exit."
    )
    ...
    @pytest.mark.parametrize(
        "random_state",
        (0, 1, np.random.RandomState(0), np.random.default_rng(0)),
    )
    ...
    @pytest.mark.parametrize(
        "random_state",
        (None, np.random.RandomState(0), np.random.default_rng(0)),
    )
    @pytest.mark.filterwarnings(
        "ignore:Exited", reason="Ignore LOBPCG early exit."
    )
    def test_svd_random_state_3(self, random_state):
        ...
        res1a = svds(
            A, k, solver=self.solver, random_state=random_state, maxiter=1000
        )
        res2a = svds(
            A, k, solver=self.solver, random_state=random_state, maxiter=1000
        )
    ...
    def test_svd_maxiter(self):
        ...
        ud, sd, vhd = svds(
            A, k, solver=self.solver, maxiter=maxiter, random_state=0
        )
    ...
    @pytest.mark.parametrize(
        "lo_type", (np.asarray, csc_matrix, aslinearoperator)
    )
        ...
        for n, m, k in nmks:
            ...
            if k < min(n, m) - 1:
                ...
                for dt, eps in [(complex, 1e-7), (np.complex64, 3e-3)]:
                    ...
                    if self.solver == "lobpcg":
                        with pytest.warns(
                            UserWarning, match="The problem size"
                        ):
                            U1, s1, VH1 = reorder(
                                svds(
                                    A,
                        ...
                        U1, s1, VH1 = reorder(
                            svds(
                                A, k, which="LM", solver=solver, random_state=0
                            )
                        )
                        U2, s2, VH2 = reorder(
                            svds(
                                L, k, which="LM", solver=solver, random_state=0
                            )
                        )
    ...
    def test_small_sigma_sparse(self, shape, dtype):
        ...
        S = S.astype(dtype)
        u, s, vh = svds(
            S, k, which="SM", solver=solver, maxiter=1000, random_state=0
        )
...
class Test_SVDS_ARPACK(SVDSCommonTests):
    ...
    def test_svds_input_validation_ncv_1(self, ncv):
        ...
        if ncv in {4, 5}:
            u, s, vh = svds(
                A, k=k, ncv=ncv, solver=self.solver, random_state=0
            )
