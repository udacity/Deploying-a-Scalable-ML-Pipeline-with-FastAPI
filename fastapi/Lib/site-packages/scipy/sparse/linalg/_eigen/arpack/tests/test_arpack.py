...
def argsort_which(
    eigenvalues, typ, k, which, sigma=None, OPpart=None, mode=None
):
...
class SymmetricParams:
    def __init__(self):
        ...
        Ac = (
            generate_matrix(
                N, hermitian=True, pos_definite=True, complex_=True
            )
            .astype("F")
            .astype("D")
        )
            ...
            for which in params.which:
                ...
                for mattype in params.mattypes:
                    for sigma in params.sigmas_modes:
                        eval_evec(
                            symmetric, D, typ, k, which, None, sigma, mattype
                        )
...
def test_real_eigs_real_k_subset():
    ...
    for which, sigma, dtype in itertools.product(whichs, [None, 0, 5], dtypes):
        ...
        for k in range(1, 9):
            ...
            assert_allclose(
                np.linalg.norm(A.dot(z) - z * w), 0, atol=np.sqrt(eps)
            )
