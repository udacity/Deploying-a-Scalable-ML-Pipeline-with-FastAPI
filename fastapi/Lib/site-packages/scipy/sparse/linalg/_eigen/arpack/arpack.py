...
SNEUPD_ERRORS[-14] = (
    "SNAUPD did not find any eigenvalues to sufficient " "accuracy."
)
SNEUPD_ERRORS[-15] = (
...
CNEUPD_ERRORS[-14] = (
    "CNAUPD did not find any eigenvalues to sufficient " "accuracy."
)
CNEUPD_ERRORS[-15] = (
    ...
    def __init__(
        ...
        if mode == 1:
            ...
            if M_matvec is None:
                ...
                self.OP = lambda x: Minv_matvec(
                    matvec(x) + sigma * M_matvec(x)
                )
                self.B = M_matvec
                self.bmat = "G"
        else:
        ...
        if which not in _SEUPD_WHICH:
            raise ValueError(
                "which must be one of %s" % " ".join(_SEUPD_WHICH)
            )
        if k >= n:
    ...
    def iterate(self):
        ...
        if self.ido == -1:
            ...
            if self.mode == 1:
                ...
                self.workd[yslice] = self.OPa(
                    Ax + (self.sigma * self.workd[Bxslice])
                )
            else:
                Bxslice = slice(self.ipntr[2] - 1, self.ipntr[2] - 1 + self.n)
...
class _UnsymmetricArpackParams(_ArpackParams):
    def __init__(
        ...
        if mode == 1:
            ...
            if matvec is None:
                raise ValueError(
                    "matvec must be specified " "for mode in (3,4)"
                )
            if Minv_matvec is None:
                raise ValueError(
                    "Minv_matvec must be specified " "for mode in (3,4)"
                )
    ...
    def __init__(self, A, M, sigma, ifunc=gmres_loose, tol=0):
        ...
        if M is None:
            ...
            self.OP = LinearOperator(
                self.A.shape, mult_func_M_None, dtype=dtype
            )
        else:
...
def get_OPinv_matvec(A, M, sigma, hermitian=False, tol=0):
    ...
    if M is None:
        ...
        if isdense(A):
            ...
            return IterOpInv(
                _aslinearoperator_with_dtype(A), M, sigma, tol=tol
            ).matvec
    else:
        if (
            not isdense(A) and not issparse(A) and not is_pydata_spmatrix(A)
        ) or (
...
def eigs(
    ...
    if k >= n - 1:
        ...
        if isinstance(A, LinearOperator):
            raise TypeError(
                "Cannot use scipy.linalg.eig for LinearOperator "
                "A with k >= N - 1."
            )
    ...
    if sigma is None:
        ...
        if OPinv is not None:
            raise ValueError(
                "OPinv should not be specified " "with sigma = None."
            )
        if OPpart is not None:
            raise ValueError(
                "OPpart should not be specified with "
        ...
        if M is None:
            ...
            if Minv is not None:
                raise ValueError(
                    "Minv should not be " "specified with M = None."
                )
        else:
        ...
        if np.issubdtype(A.dtype, np.complexfloating):
            if OPpart is not None:
                raise ValueError(
                    "OPpart should not be specified "
        ...
        if OPinv is None:
            Minv_matvec = get_OPinv_matvec(
                A, M, sigma, hermitian=False, tol=tol
            )
        else:
...
def eigsh(
    ...
    if np.issubdtype(A.dtype, np.complexfloating):
        if mode != "normal":
            raise ValueError(
                "mode=%s cannot be used with " "complex matrix A" % mode
            )
        if which == "BE":
    ...
    if k >= n:
        ...
        if isinstance(A, LinearOperator):
            raise TypeError(
                "Cannot use scipy.linalg.eigh for LinearOperator "
                "A with k >= N."
            )
        if isinstance(M, LinearOperator):
            raise TypeError(
    ...
    if sigma is None:
        ...
        if mode == "normal":
            ...
            if OPinv is None:
                Minv_matvec = get_OPinv_matvec(
                    A, M, sigma, hermitian=True, tol=tol
                )
            else:
                OPinv = _aslinearoperator_with_dtype(OPinv)
                Minv_matvec = OPinv.matvec
