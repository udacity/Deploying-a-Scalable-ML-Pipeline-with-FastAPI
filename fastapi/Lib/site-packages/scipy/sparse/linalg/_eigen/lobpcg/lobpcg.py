...
def _b_orthonormalize(B, blockVectorV, blockVectorBV=None, verbosityLevel=0):
    ...
    try:
        ...
        blockVectorV = _matmul_inplace(
            blockVectorV, VBV, verbosityLevel=verbosityLevel
        )
        if B is not None:
...
def lobpcg(
    ...
    if retLambdaHistory:
        lambdaHistory = np.zeros(
            (maxiter + 3, sizeX), dtype=blockVectorX.dtype
        )
    if retResidualNormsHistory:
        residualNormsHistory = np.zeros(
            (maxiter + 3, sizeX), dtype=blockVectorX.dtype
        )
    ...
    if (n - sizeY) < (5 * sizeX):
        ...
        try:
            vals, vecs = eigh(
                A, B, subset_by_index=eigvals, check_finite=False
            )
            if largest:
    ...
    while iterationNumber < maxiter:
        ...
        if not restart:
            ...
            try:
                _lambda, eigBlockVector = eigh(
                    gramA, gramB, check_finite=False
                )
            except LinAlgError as e:
