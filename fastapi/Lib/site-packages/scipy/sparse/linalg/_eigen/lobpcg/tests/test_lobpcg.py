...
def ElasticRod(n):
    ...
    k = E * S / le
    A = k * (
        diag(r_[2.0 * ones(n - 1), 1])
        - diag(ones(n - 1), 1)
        - diag(ones(n - 1), -1)
    )
    B = mass * (
        diag(r_[4.0 * ones(n - 1), 2])
        + diag(ones(n - 1), 1)
        + diag(ones(n - 1), -1)
    )
    return A, B
    ...
    for A in [A_a, A_s, A_lo]:
        for B in [B_a, B_s, B_lo]:
            for M in [M_a, M_s, M_lo]:
                eigvals, vecs = lobpcg(
                    A, X, B, M=M, Y=Y, maxiter=40, largest=False
                )

                assert_allclose(
                    eigvals, np.arange(1 + m_excluded, 1 + m_excluded + m)
                )
...
def test_hermitian():
    ...
    for s, k, gen, dh, dx, db in itertools.product(
        sizes, ks, gens, gens, gens, gens
    ):
        ...
        for wx, vx in zip(w, v.T):
            # Check eigenvector
            assert_allclose(
                np.linalg.norm(H.dot(vx) - B.dot(vx) * wx)
...
@pytest.mark.xfail(
    _IS_32BIT and sys.platform == "win32",
...
@pytest.mark.parametrize(
    "arr_type", [np.array, sparse.csr_matrix, sparse.coo_matrix]
)
