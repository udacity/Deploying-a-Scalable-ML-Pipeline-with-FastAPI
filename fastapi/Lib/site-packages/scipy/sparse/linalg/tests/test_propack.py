...
for dtype_flavour in TOLS.keys():
    ...
    _dtypes.append(
        pytest.param(dtype_flavour, marks=marks, id=dtype_flavour.__name__)
    )
...
def generate_matrix(constructor, n, m, f, dtype=float, rseed=0, **kwargs):
    ...
    if is_complex_type(dtype):
        M = (-5 + 10 * rng.rand(n, m) - 5j + 10j * rng.rand(n, m)).astype(
            dtype
        )
    else:
...
def check_svdp(n, m, constructor, dtype, k, irl_mode, which, f=0.8):
    ...
    u2, sigma2, vt2, _ = _svdp(
        Msp, k=k, which=which, irl_mode=irl_mode, tol=tol
    )
