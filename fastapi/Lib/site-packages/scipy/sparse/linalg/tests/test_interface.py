...
class TestLinearOperator:
    ...
    def test_matvec(self):
            ...
            return [
                {
                    ...
                    "rmatvec": lambda x: np.dot(A.T.conj(), x).reshape(
                        A.shape[1]
                    ),
                },
                {
        ...
        for matvecs in get_matvecs(self.A):
            ...
            assert_equal(
                (2j * A).rmatmat(X), np.dot((2j * self.A).T.conj(), X)
            )
            assert_equal(
                (A * 2j).rmatmat(X), np.dot((self.A * 2j).T.conj(), X)
            )
            ...
            assert_(
                isinstance(A.matvec(np.array([[1], [2], [3]])), np.ndarray)
            )
        ...
        for matvecsA, matvecsB in product(
            get_matvecs(self.A), get_matvecs(self.B)
        ):
            ...
            assert_equal(
                (2j * A * B).rmatmat(X), np.dot((2j * AtimesB).T.conj(), X)
            )
        ...
        for matvecsC in get_matvecs(self.C):
            ...
            assert_equal(
                (C**2).rmatmat(X), np.dot((np.dot(self.C, self.C)).T, X)
            )
    ...
    def test_matmul(self):
        D = {
            ...
            "rmatvec": lambda x: np.dot(self.A.T.conj(), x).reshape(
                self.A.shape[1]
            ),
        ...
        assert_equal(
            operator.matmul(A, b.reshape(-1, 1)), A * b.reshape(-1, 1)
        )
        assert_equal(operator.matmul(A, B), A * B)
        assert_equal(operator.matmul(b, A.H), b * A.H)
        assert_equal(
            operator.matmul(b.reshape(1, -1), A.H), b.reshape(1, -1) * A.H
        )
        assert_equal(operator.matmul(B, A.H), B * A.H)
