...
def do_solve(**kw):
    ...
    with suppress_warnings() as sup:
        ...
        x0, flag = lgmres(
            A, b, x0=zeros(A.shape[0]), inner_m=6, rtol=1e-14, **kw
        )
    count_0 = count[0]
...
class TestLGMRES:
    ...
    def test_outer_v(self):
        ...
        x1, count_1 = do_solve(
            outer_k=6, outer_v=outer_v, prepend_outer_v=True
        )
        ...
        outer_v = []
        x0, count_0 = do_solve(
            outer_k=6, outer_v=outer_v, store_outer_Av=False
        )
    ...
    def test_arnoldi(self):
        ...
        with suppress_warnings() as sup:
            sup.filter(DeprecationWarning, ".*called without specifying.*")
            x0, flag0 = lgmres(
                A, b, x0=zeros(A.shape[0]), inner_m=15, maxiter=1
            )
            x1, flag1 = gmres(
                A, b, x0=zeros(A.shape[0]), restart=15, maxiter=1
            )
