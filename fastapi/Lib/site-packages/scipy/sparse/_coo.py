...
class _coo_base(_data_matrix, _minmax_mixin):
            ...
            if isshape(arg1, allow_1d=is_array):
                ...
                if shape is None:
                    if any(len(idx) == 0 for idx in coords):
                        raise ValueError(
                            "cannot infer dimensions from zero "
                            "sized index arrays"
                        )
                    shape = tuple(
                        operator.index(np.max(idx)) + 1 for idx in coords
                    )
    ...
    def _getnnz(self, axis=None):
        if axis is None or (axis == 0 and self.ndim == 1):
            ...
            if self.data.ndim != 1 or any(
                idx.ndim != 1 for idx in self.coords
            ):
        ...
        if self.ndim > 2:
            raise NotImplementedError(
                "per-axis nnz for COO arrays with >2 "
                "dimensions is not supported"
            )
        return np.bincount(
            downcast_intp_index(self.coords[1 - axis]),
    ...
    def _check(self):
        ...
        self.coords = tuple(
            np.asarray(idx, dtype=idx_dtype) for idx in self.coords
        )
    ...
    def _with_data(self, data, copy=True):
        ...
        if copy:
            ...
            coords = self.coords
        return self.__class__(
            (data, coords), shape=self.shape, dtype=data.dtype
        )
    ...
    def _matmul_multivector(self, other):
        ...
        for i, other_col in enumerate(other.T):
            coo_matvec(
                self.nnz, row, col, self.data, other_col, result[i : i + 1]
            )
