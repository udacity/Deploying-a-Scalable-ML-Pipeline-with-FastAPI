...
def test_getfullargspec_no_self():
    ...
    assert_equal(
        argspec, FullArgSpec(["pool"], None, None, (1,), [], None, {})
    )
    argspec = getfullargspec_no_self(p.__call__)
    assert_equal(
        argspec,
        FullArgSpec(["func", "iterable"], None, None, None, [], None, {}),
    )
...
class TestValidateInt:
    ...
    def test_validate_int_below_min(self):
        with pytest.raises(
            ValueError, match="n must be an integer not " "less than 0"
        ):
            _validate_int(-1, "n", 0)
...
class TestLazywhere:
    ...
    def test_basic(self, n_arrays, rng_seed, dtype, p, data, xp):
        mbs = npst.mutually_broadcastable_shapes(
            num_shapes=n_arrays + 1, min_side=0
        )
