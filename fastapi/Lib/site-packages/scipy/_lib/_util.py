...
if TYPE_CHECKING:
    SeedType = Optional[
        Union[IntNumber, np.random.Generator, np.random.RandomState]
    ]
    GeneratorType = TypeVar(
        "GeneratorType",
        bound=Union[np.random.Generator, np.random.RandomState],
    )
...
def _rng_html_rewrite(func):
    ...
    def _wrapped(*args, **kwargs):
        ...
        lines = [
            re.sub(pattern, "np.random.default_rng()", line) for line in res
        ]
        return lines
