...
def cross(
    x1: ndarray, x2: ndarray, /, xp, *, axis: int = -1, **kwargs
) -> ndarray:
...
def svd(
    x: ndarray, /, xp, *, full_matrices: bool = True, **kwargs
) -> SVDResult:
    ...
    *,
    ...
    if axis is None:
        ...
        _x = xp.transpose(x, newshape).reshape(
            (
                math.prod([x.shape[i] for i in axis]),
...
def trace(
    x: ndarray, /, xp, *, offset: int = 0, dtype=None, **kwargs
) -> ndarray:
    if dtype is None:
