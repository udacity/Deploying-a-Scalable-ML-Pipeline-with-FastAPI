...
def result_type(*arrays_and_dtypes: Union[array, Dtype]) -> Dtype:
    ...
    if len(arrays_and_dtypes) > 2:
        return result_type(
            arrays_and_dtypes[0], result_type(*arrays_and_dtypes[1:])
        )
...
def std(
    ...
    **kwargs,
    ...
    if axis is None:
        ...
        # (https://github.com/pytorch/pytorch/issues/71209)
        res = torch.std(
            x, tuple(range(x.ndim)), correction=_correction, **kwargs
        )
        res = _axis_none_keepdims(res, x.ndim, keepdims)
        return res
    return torch.std(
        x, axis, correction=_correction, keepdims=keepdims, **kwargs
    )
...
def var(
    ...
    **kwargs,
    ...
    if axis is None:
        ...
        # (https://github.com/pytorch/pytorch/issues/71209)
        res = torch.var(
            x, tuple(range(x.ndim)), correction=correction, **kwargs
        )
        res = _axis_none_keepdims(res, x.ndim, keepdims)
        return res
    return torch.var(
        x, axis, correction=correction, keepdims=keepdims, **kwargs
    )
...
def arange(
    ...
    **kwargs,
    ...
    return torch.arange(
        start, stop, step, dtype=dtype, device=device, **kwargs
    )
...
def linspace(
    ...
    **kwargs,
    ...
    return torch.linspace(
        start, stop, num, dtype=dtype, device=device, **kwargs
    )
...
def take(
    x: array, indices: array, /, *, axis: Optional[int] = None, **kwargs
) -> array:
    if axis is None:
        if x.ndim != 1:
