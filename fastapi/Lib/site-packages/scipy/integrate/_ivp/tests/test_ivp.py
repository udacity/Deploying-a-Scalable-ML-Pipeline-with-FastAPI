...
def test_integration_sparse_difference():
    ...
    for method in ["BDF", "Radau"]:
        res = solve_ivp(
            fun_medazko, t_span, y0, method=method, jac_sparsity=sparsity
        )
...
def test_integration_stiff(method):
    ...
    res = solve_ivp(
        fun_robertson, tspan, y0, rtol=rtol, atol=atol, method=method
    )
...
def test_events():
    ...
    for method in ["RK23", "RK45", "DOP853", "Radau", "BDF", "LSODA"]:
        ...
        assert np.allclose(
            sol_rational(res.t_events[1][0]),
            res.y_events[1][0],
            rtol=1e-3,
            atol=1e-6,
        )
        assert np.allclose(
            sol_rational(res.t_events[2][0]),
            res.y_events[2][0],
            rtol=1e-3,
            atol=1e-6,
        )
...
def test_t_eval():
    ...
    for t_span in ([5, 9], [5, 1]):
        ...
        res = solve_ivp(
            fun_rational, t_span, y0, rtol=rtol, atol=atol, t_eval=t_eval
        )
    ...
    res = solve_ivp(
        fun_rational, [5, 9], y0, rtol=rtol, atol=atol, t_eval=t_eval
    )
    assert_equal(res.t, t_eval)
    assert_(res.t_events is None)
    assert_(res.success)
    ...
    res = solve_ivp(
        fun_rational, [5, 1], y0, rtol=rtol, atol=atol, t_eval=t_eval
    )
    assert_equal(res.t, t_eval)
    assert_(res.t_events is None)
    assert_(res.success)
...
def test_OdeSolution():
    ...
    assert_equal(
        sol([12, -5, 10, -3, 6, 1, 4]), np.array([[-1, 1, -1, 1, -1, 1, -1]])
    )
...
def test_num_jac_sparse():
    ...
    assert_allclose(
        J_num_dense, J_num_sparse.toarray(), rtol=1e-12, atol=1e-14
    )
