...
class TestQuadrature:
    ...
    def test_romb_gh_3731(self):
        ...
        with suppress_warnings() as sup:
            ...
            val3 = romberg(
                lambda x: np.cos(0.2 * x), x.min(), x.max(), divmax=4
            )
    ...
    def test_newton_cotes(self):
        ...
        wts, errcoff = newton_cotes(n, 1)
        assert_almost_equal(
            wts, n * np.array([7.0, 32.0, 12.0, 32.0, 7.0]) / 90.0
        )
    ...
    def test_simpson(self):
        ...
        for i in range(3):
            ...
            for _ in it:
                ...
                integral = (
                    x[tuple(idx)][-1] ** 3 / 3 - x[tuple(idx)][0] ** 3 / 3
                )
    ...
    def test_simpson_2d_integer_no_x(self, droplast):
        ...
        y = np.array(
            [[2, 2, 4, 4, 8, 8, -4, 5], [4, 4, 2, -4, 10, 22, -2, 10]]
        )
        if droplast:
            y = y[:, :-1]
...
class TestQMCQuad:
    ...
    def basic_test(self, n_points=2**8, n_estimates=8, signs=np.ones(2)):
        ...
        atol = (
            special.stdtrit(n_estimates - 1, 0.995) * res.standard_error
        )  # 99% CI
...
def cumulative_simpson_nd_reference(
    y, *, x=None, dx=None, initial=None, axis=-1
):
    ...
    if y.shape[axis] < 3:
        if initial is None:
            ...
            return initial + cumulative_trapezoid(
                y, x=x, dx=dx, axis=axis, initial=0
            )
    ...
    initial = (
        np.moveaxis(initial, axis, -1) if np.ndim(initial) > 1 else initial
    )
...
class TestCumulativeSimpson:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                "`initial` must either be a scalar or...",
                dict(initial=np.arange(5)),
            ),
            (
                "`dx` must either be a scalar or...",
                dict(x=None, dx=np.arange(5)),
            ),
        ],
    )
    ...
    def test_cumulative_simpson_against_simpson(self, y):
        ...
        theoretical_difference = (
            self._get_theoretical_diff_between_simps_and_cum_simps(y, x)
        )
        np.testing.assert_allclose(
            res[..., 1:], ref[..., 1:] + theoretical_difference[..., 1:]
        )
