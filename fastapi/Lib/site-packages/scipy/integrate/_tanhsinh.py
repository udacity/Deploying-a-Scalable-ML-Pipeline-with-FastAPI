...
def _nsum(
    ...
    return _RichResult(
        sum=S, error=E, status=status, success=status == 0, nfev=nfev
    )
...
def _integral_bound(f, a, b, step, args, constants):
    ...
    tol = (
        _logsumexp((tol, rtol + lb.integral))
        if log
        else tol + rtol * lb.integral
    )
    i_skip = (
        lb.status < 0
    )  # avoid unnecessary f_evals if integral is divergent
    tol[i_skip] = np.nan
    status = lb.status
