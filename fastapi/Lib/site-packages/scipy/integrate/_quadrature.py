...
def _printresmat(function, interval, resmat):
    ...
    for i in range(len(resmat)):
        print(
            "%6d %9f" % (2**i, (interval[1] - interval[0]) / (2.0**i)), end=" "
        )
        for j in range(i + 1):
...
def romberg(
    ...
    if np.isinf(a) or np.isinf(b):
        raise ValueError(
            "Romberg integration only available " "for finite limits."
        )
...
def newton_cotes(rn, equal=0):
    ...
    if (rn[0] != 0) or (rn[-1] != N):
        raise ValueError(
            "The sample positions must start at 0" " and end at N"
        )
    yi = rn / float(N)
    ti = 2 * yi - 1
    nvec = np.arange(N + 1)
...
def qmc_quad(
    func, a, b, *, n_estimates=8, n_points=1024, qrng=None, log=False
):
    """
    ...
    def std(estimates, m=None, ddof=0, log=False):
        ...
        if log:
            ...
            return np.real(
                0.5 * (logsumexp(2 * diff) - np.log(n_estimates - ddof))
            )
        else:
