...
def compute_jac_indices(n, m, k):
    ...
    j_col = np.tile(np.arange(n), n * (m - 1)) + np.repeat(
        np.arange(m - 1) * n, n**2
    )
    ...
    j = np.hstack(
        (j_col, j_col + n, j_bc, j_bc + (m - 1) * n, j_p_col, j_p_bc)
    )
...
def collocation_fun(fun, y, p, x, h):
    ...
    """
    f = fun(x, y, p)
    y_middle = 0.5 * (y[:, 1:] + y[:, :-1]) - 0.125 * h * (
        f[:, 1:] - f[:, :-1]
    )
    f_middle = fun(x[:-1] + 0.5 * h, y_middle, p)
    col_res = (
        y[:, 1:] - y[:, :-1] - h / 6 * (f[:, :-1] + f[:, 1:] + 4 * f_middle)
    )
...
def solve_bvp(
    ...
    while True:
        ...
        rms_res = estimate_rms_residuals(
            fun_wrapped, sol, x, h, p, r_middle, f_middle
        )
