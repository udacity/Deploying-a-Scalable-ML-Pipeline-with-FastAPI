...
class InterpolatedUnivariateSpline(UnivariateSpline):
    ...
    def __init__(
        self, x, y, w=None, bbox=[None] * 2, k=3, ext=0, check_finite=False
    ):
        ...
        # _data == x,y,w,xb,xe,k,s,n,t,c,fp,fpint,nrdata,ier
        self._data = dfitpack.fpcurf0(
            x, y, k, w=w, xb=bbox[0], xe=bbox[1], s=0
        )
        self._reset_class()
...
class _BivariateSplineBase:
    ...
    def _from_tck(cls, tck):
        ...
        if len(tck) != 5:
            raise ValueError(
                "tck should be a 5 element tuple of tx," " ty, c, kx, ky"
            )
        self.tck = tck[:3]
        self.degrees = tck[3:]
        return self
    ...
    def partial_derivative(self, dx, dy):
        ...
        if dx == 0 and dy == 0:
            ...
            if not (dx >= 0 and dy >= 0):
                raise ValueError(
                    "order of derivative must be positive or" " zero"
                )
...
class SmoothBivariateSpline(BivariateSpline):
    ...
    def __init__(
        self, x, y, z, w=None, bbox=[None] * 4, kx=3, ky=3, s=None, eps=1e-16
    ):
...
class LSQBivariateSpline(BivariateSpline):
    ...
    def __init__(
        self, x, y, z, tx, ty, w=None, bbox=[None] * 4, kx=3, ky=3, eps=None
    ):
...
class RectSphereBivariateSpline(SphereBivariateSpline):
    ...
    def __init__(
        ...
        if pole_continuity[1] is False and pole_flat[1] is True:
            raise ValueError(
                "if pole_continuity is False, so must be " "pole_flat"
            )
