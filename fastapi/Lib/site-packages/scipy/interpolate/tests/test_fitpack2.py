...
class TestUnivariateSpline:
    ...
    def test_out_of_range_regression(self):
        ...
        for cls in [UnivariateSpline, InterpolatedUnivariateSpline]:
            ...
            for ext in [1, "zeros"]:
                ...
                assert_allclose(
                    cls(x, y, ext=ext)(xp), xp_zeros**3, atol=1e-16
                )
            for ext in [2, "raise"]:
        ...
        for ext in [-1, "unknown"]:
            ...
            assert_raises(ValueError, spl, xp, **dict(ext=ext))
            assert_raises(
                ValueError, UnivariateSpline, **dict(x=x, y=y, ext=ext)
            )
    ...
    def test_lsq_fpchec(self):
        ...
        bbox = (-1, 101)
        assert_raises(
            ValueError, LSQUnivariateSpline, xs, ys, knots, bbox=bbox
        )
    ...
    def test_array_like_input(self):
        ...
        assert_allclose(
            spl1([0.1, 0.5, 0.9, 0.99]), spl2([0.1, 0.5, 0.9, 0.99])
        )
...
class TestLSQSphereBivariateSpline:
    def setup_method(self):
        ...
        ntheta, nphi = 70, 90
        theta = (
            linspace(0.5 / (ntheta - 1), 1 - 0.5 / (ntheta - 1), ntheta) * pi
        )
    ...
    def test_linear_constant(self):
        ...
        assert_array_almost_equal(
            self.lut_lsq(self.new_lats, self.new_lons), self.data
        )

    def test_empty_input(self):
        assert_array_almost_equal(self.lut_lsq([], []), np.zeros((0, 0)))
        assert_array_almost_equal(
            self.lut_lsq([], [], grid=False), np.zeros((0,))
        )
...
class TestRectBivariateSpline:
    ...
    def test_partial_derivative_method(self):
        ...
        lut = RectBivariateSpline(x, y, z)
        assert_array_almost_equal(
            lut.partial_derivative(1, 0)(x, y, grid=False), dx
        )
        assert_array_almost_equal(
            lut.partial_derivative(0, 1)(x, y, grid=False), dy
        )
        assert_array_almost_equal(
            lut.partial_derivative(1, 1)(x, y, grid=False), dxdy
        )
...
class TestRectSphereBivariateSpline:
    ...
    def test_derivatives_grid(self):
        ...
        assert_allclose(
            lut(x, y, dtheta=1),
            _numdiff_2d(lut, x, y, dx=1),
            rtol=1e-4,
            atol=1e-4,
        )
        assert_allclose(
            lut(x, y, dphi=1),
            _numdiff_2d(lut, x, y, dy=1),
            rtol=1e-4,
            atol=1e-4,
        )
        assert_allclose(
        ...
        assert_array_equal(
            lut(x, y, dtheta=1), lut.partial_derivative(1, 0)(x, y)
        )
        assert_array_equal(
            lut(x, y, dphi=1), lut.partial_derivative(0, 1)(x, y)
        )
        assert_array_equal(
    ...
    def test_derivatives(self):
        ...
        assert_allclose(
            lut(x, y, dtheta=1, dphi=1, grid=False),
            _numdiff_2d(
                lambda x, y: lut(x, y, grid=False), x, y, dx=1, dy=1, eps=1e-6
            ),
            rtol=1e-3,
            atol=1e-3,
        )
