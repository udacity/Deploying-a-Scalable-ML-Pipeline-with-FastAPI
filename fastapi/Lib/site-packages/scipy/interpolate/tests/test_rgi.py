...
class TestRegularGridInterpolator:
    ...
    def test_derivatives(self):
        ...
        assert_allclose(
            interp(sample, nu=(0, 1, 0, 0)), [10, 10, 10], atol=1e-15
        )
    ...
    def test_length_one_axis(self):
        ...
        assert_allclose(
            interp(np.array([[1, 1.4], [1, 5.3], [1, 10]])),
            [2.4, 6.3, 11],
            atol=1e-14,
        )
        ...
        assert_allclose(
            interp(np.array([1.1, 2.4])), interp.fill_value, atol=1e-14
        )
        ...
        assert_allclose(
            interp([[1.5, 0.3], [1.9, 11.5]]), [1.3, 12.5], atol=1e-15
        )
...
class TestInterpN:
    ...
    def test_spline_2d(self):
        ...
        assert_array_almost_equal(
            interpn((x, y), z, xi, method="splinef2d"),
            lut.ev(xi[:, 0], xi[:, 1]),
        )
    ...
    def test_non_scalar_values_splinef2d(self):
        ...
        assert_raises(
            ValueError, interpn, points, values, sample, method="splinef2d"
        )
    ...
    def test_length_one_axis(self):
        ...
        res = interpn(
            ([1], [2, 3, 4]), values, xi, bounds_error=False, fill_value=None
        )
    ...
    def test_descending_points(self):
        ...
        pts = (
            0.1,
            0.3,
            np.transpose(np.linspace(0, 30, 4)),
            np.linspace(0, 0.3, 4),
        )
