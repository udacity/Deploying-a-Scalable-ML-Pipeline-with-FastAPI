...
class TestInterp1D:
    ...
    def test_validation(self):
        ...
        interp1d(
            self.x10, self.y210, kind="linear", axis=-1, fill_value=(-1, -1)
        )
        interp1d(
            self.x2, self.y210, kind="linear", axis=0, fill_value=np.ones(10)
        )
        interp1d(
            self.x2,
            self.y210,
    ...
    def test_init(self):
        ...
        assert_(
            not interp1d(self.x10, self.y10, bounds_error=False).bounds_error
        )
        assert_(np.isnan(interp1d(self.x10, self.y10).fill_value))
        assert_equal(
            interp1d(self.x10, self.y10, fill_value=3.0).fill_value, 3.0
        )
        assert_equal(
            interp1d(self.x10, self.y10, fill_value=(1.0, 2.0)).fill_value,
            (1.0, 2.0),
        )
    ...
    def _check_linear(self, kind):
        ...
        extrapolator = interp1d(
            self.x10, self.y10, kind=kind, fill_value="extrapolate"
        )
        assert_allclose(
            extrapolator([-1.0, 0, 9, 11]), [-1, 0, 9, 11], rtol=1e-14
        )
    ...
    def test_nearest(self):
        ...
        assert_allclose(
            extrapolator([-1.0, 0, 9, 11]), [0, 0, 9, 9], rtol=1e-14
        )

        opts = dict(
            kind="nearest", fill_value="extrapolate", bounds_error=True
        )
        assert_raises(ValueError, interp1d, self.x10, self.y10, **opts)
    ...
    def test_nearest_up(self):
        ...
        opts = dict(
            kind="nearest-up", fill_value="extrapolate", bounds_error=True
        )
        assert_raises(ValueError, interp1d, self.x10, self.y10, **opts)
    ...
    def test_previous(self):
        ...
        assert_allclose(
            extrapolator([-1.0, 0, 9, 11]), [np.nan, 0, 9, 9], rtol=1e-14
        )
        ...
        assert_allclose(
            interpolator2DAxis0([-2, 5, 12]),
            [[np.nan, np.nan], [10, 11], [18, 19]],
        )

        opts = dict(
            kind="previous", fill_value="extrapolate", bounds_error=True
        )
        assert_raises(ValueError, interp1d, self.x10, self.y10, **opts)
    ...
    def test_next(self):
        ...
        assert_allclose(
            extrapolator([-1.0, 0, 9, 11]), [0, 0, 9, np.nan], rtol=1e-14
        )
    ...
    def _bounds_check(self, kind="linear"):
        ...
        raises_bounds_error = interp1d(
            self.x10, self.y10, bounds_error=True, kind=kind
        )
    ...
    def test_single_value(self, kind):
        ...
        f = interp1d(
            [1.5], [6], kind=kind, bounds_error=False, fill_value=(2, 10)
        )
...
class TestAkima1DInterpolator:
    ...
    def test_extrapolate_attr(self):
        ...
        assert_allclose(
            ak_false(x_ext), ak_none(x_ext), equal_nan=True, atol=1e-15
        )
        ...
        assert_allclose(
            ak_false(x_ext, extrapolate=True), ak_true(x_ext), atol=1e-15
        )
...
class TestPPolyCommon:
    ...
    def test_shape(self):
        ...
        for cls in (PPoly, BPoly):
            ...
            assert_raises(
                ValueError, p, np.array([[0.1, 0.2], [0.4]], dtype=object)
            )
...
class TestPPoly:
    ...
    def test_descending(self):
        ...
        for m in [10, 20, 30]:
            ...
            for a, b in np.random.uniform(-10, 20, (5, 2)):
                ...
                assert_allclose(
                    pa_i(b) - pa_i(a), pd_i(b) - pd_i(a), rtol=1e-13
                )
    ...
    def test_derivative(self):
        ...
        for dx in range(0, 10):
            assert_allclose(
                pp(xi, dx), pp.derivative(dx)(xi), err_msg="dx=%d" % (dx,)
            )
    ...
    def test_integrate_periodic(self):
        ...
        assert_allclose(
            P.integrate(3.5 + 12, 5 + 12), I(2) - I(1) + I(4) - I(3.5)
        )
        assert_allclose(
        ...
        for extrapolate in (True, False):
            for order in range(0, 20):
                ...
                for y in [0, np.random.random()]:
                    r = pp.solve(
                        y, discontinuity=False, extrapolate=extrapolate
                    )
                    ...
                    for i in range(2):
                        for j in range(3):
                            ...
                            if rr.size > 0:
                                ...
                                cmpval = pp(rr, nu=1, extrapolate=extrapolate)[
                                    :, i, j
                                ]
    ...
    def test_descending(self):
        ...
        for m in [10, 20, 30]:
            ...
            for a, b in np.random.uniform(-10, 20, (5, 2)):
                ...
                assert_allclose(
                    pa_i(b) - pa_i(a), pd_i(b) - pd_i(a), rtol=1e-12
                )
...
class TestBPolyCalculus:
    def test_derivative(self):
        ...
        assert_allclose(
            [bp(0.4, nu=1), bp(0.4, nu=2), bp(0.4, nu=3)],
            [-6 * (1 - 0.4), 6.0, 0.0],
        )
        assert_allclose(
            [bp(1.7, nu=1), bp(1.7, nu=2), bp(1.7, nu=3)], [0.7, 1.0, 0]
        )
    ...
    def test_der_antider(self):
        ...
        assert_allclose(
            bp.antiderivative().derivative()(xx),
            bp(xx),
            atol=1e-12,
            rtol=1e-12,
        )
    ...
    def test_antider_ppoly(self):
        ...
        assert_allclose(
            bp.antiderivative(2)(xx),
            pp.antiderivative(2)(xx),
            atol=1e-12,
            rtol=1e-12,
        )
    ...
    def test_integrate(self):
        ...
        pp = PPoly.from_bernstein_basis(bp)
        assert_allclose(
            bp.integrate(0, 1), pp.integrate(0, 1), atol=1e-12, rtol=1e-12
        )
...
class TestNdPPoly:
    ...
    def test_integrate_2d(self):
        ...
        for ranges in [
            ...
            assert_allclose(
                ig, ig2, rtol=1e-5, atol=1e-5, err_msg=repr(ranges)
            )
...
def _ppoly_eval_2(coeffs, breaks, xnew, fill=np.nan):
    ...
    values = np.array(
        [np.dot(V[k, :], pp[:, indxs[k]]) for k in range(len(xx))]
    )
    res[mask] = values
    res.shape = saveshape
    return res
