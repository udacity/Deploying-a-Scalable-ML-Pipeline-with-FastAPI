...
def check_shape(
    ...
    target_shape = (
        (deriv_shape or ()) + y.shape[:axis] + x_shape + y.shape[axis:][1:]
    )
    ...
    if x_shape and y.size > 0:
        if interpolator_cls is CubicHermiteSpline:
            interpolator_cls(
                list(x), list(y), list(dydx), axis=axis, **extra_args
            )(list(xi))
        else:
            interpolator_cls(list(x), list(y), axis=axis, **extra_args)(
                list(xi)
            )
...
class TestKrogh:
    ...
    def test_ndim_derivatives(self):
        ...
        ys = np.stack(
            (poly1(self.xs), poly2(self.xs), poly3(self.xs)), axis=-1
        )
    ...
    def test_vector(self):
        ...
        assert_almost_equal(
            ...
            np.transpose(
                np.asarray([p.derivatives(test_xs) for p in Pi]), (1, 2, 0)
            ),
        )
    ...
    def test_wrapper(self):
        ...
        assert_almost_equal(
            P(self.test_xs), ki(self.xs, self.ys, self.test_xs)
        )
        assert_almost_equal(
    ...
    def test_int_inputs(self):
        ...
        assert_allclose(
            abs((f(x) - offset_cdf) / f.derivative(x, 1)), 0, atol=1e-10
        )
    ...
    def test_derivatives_complex(self):
        ...
        cmplx2 = KroghInterpolator(x, y.real).derivatives(
            0
...
class TestBarycentric:
    ...
    def test_low_derivatives(self):
        ...
        for i in range(D.shape[0]):
            assert_allclose(
                self.true_poly.deriv(i)(self.test_xs), D[i], atol=1e-12
            )
...
class TestCubicSpline:
    ...
    def check_correctness(
        S, bc_start="not-a-knot", bc_end="not-a-knot", tol=1e-14
    ):
        ...
        assert_allclose(
            c[1, 1:], 3 * c[0, :-1] * dxi + c[1, :-1], rtol=tol, atol=tol
        )
