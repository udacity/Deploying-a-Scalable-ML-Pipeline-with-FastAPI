...
class TestBSpline:
    ...
    def test_ctor(self):
        ...
        with np.errstate(invalid="ignore"):
            assert_raises(
                ValueError, BSpline, **dict(t=[1, np.nan], c=[1.0], k=0)
            )
        ...
        assert_raises(
            ValueError, BSpline, **dict(t=[0, 1, 2, 3, 4], c=[1.0, 1.0], k=2)
        )
        ...
        assert_raises(
            ...
            BSpline,
            **dict(
                t=[0.0, 0.0, 1.0, 2.0, 3.0, 4.0], c=[1.0, 1.0, 1.0], k="cubic"
            ),
        )
        assert_raises(
            TypeError,
    ...
    def test_extrap(self):
        ...
        assert_allclose(
            b(xx[mask], extrapolate=True), b(xx[mask], extrapolate=False)
        )
    ...
    def test_derivative_jumps(self):
        ...
        x1 = np.asarray([1, 6])
        assert_(
            not np.all(np.allclose(b(x1 - 1e-10, nu=1), b(x1 + 1e-10, nu=1)))
        )

        # 2nd derivative is not guaranteed to be continuous either
        assert_(
            not np.all(np.allclose(b(x - 1e-10, nu=2), b(x + 1e-10, nu=2)))
        )
    ...
    def test_basis_element_quadratic(self):
        ...
        assert_allclose(
            b(xx), np.where(xx < 1, xx * xx, (2.0 - xx) ** 2), atol=1e-14
        )
    ...
    def test_integral(self):
        ...
        assert_allclose(
            b.integrate(1.5 + 12, 3 + 12), i(1) - i(0) + i(2) - i(1.5)
        )
        assert_allclose(
    ...
    def test_axis(self, axis):
        ...
        assert_equal(
            b(xp).shape, sh[:pos_axis] + list(xp.shape) + sh[pos_axis + 1 :]
        )
    ...
    def test_design_matrix_t_shapes(self):
        ...
        assert_allclose(
            des_matr, [[0.25, 0.58333333, 0.16666667, 0.0]], atol=1e-14
        )
    ...
    def test_from_power_basis_complex(self, bc_type):
        ...
        assert_equal(
            bspl.c.dtype, (bspl_new_real.c + 1j * bspl_new_imag.c).dtype
        )
        assert_allclose(
            bspl.c, bspl_new_real.c + 1j * bspl_new_imag.c, atol=1e-15
        )
...
class TestInsert:
    ...
    @pytest.mark.parametrize(
        "xv",
        [0, 0.1, 2.0, 4.0, 4.5, 5.5, 6.0, 6.1, 7.0],  # l.h. edge  # r.h. edge
    )
...
class TestInterop:
    ...
    def test_splev(self):
        ...
        assert_allclose(
            [splev(x, b) for x in xnew], b(xnew), atol=1e-15, rtol=1e-15
        )
    ...
    def test_insert(self):
        ...
        assert_allclose(
            np.asarray(splev(xx, tck_n2)).transpose(2, 0, 1),
            bn2(xx),
            atol=1e-15,
        )
...
class TestInterp:
    ...
    def test_periodic(self):
        ...
        for i in range(1, 5):
            assert_allclose(
                b(self.xx[0], nu=i), b(self.xx[-1], nu=i), atol=1e-11
            )
        # tests for axis=-1
        b = make_interp_spline(
            self.xx, self.yy, k=5, bc_type="periodic", axis=-1
        )
        assert_allclose(b(self.xx), self.yy, atol=1e-14, rtol=1e-14)
        for i in range(1, 5):
            assert_allclose(
    ...
    def test_quintic_derivs(self):
        ...
        assert_allclose(
            [b(x[-1], 1), b(x[-1], 2)], [val for (nu, val) in der_r]
        )
    ...
    def test_knots_not_data_sites(self):
        ...
        b = make_interp_spline(
            self.xx, self.yy, k, t, bc_type=([(2, 0)], [(2, 0)])
        )

        assert_allclose(b(self.xx), self.yy, atol=1e-14, rtol=1e-14)
        assert_allclose(
            [b(self.xx[0], 2), b(self.xx[-1], 2)], [0.0, 0.0], atol=1e-14
        )
    ...
    def test_string_aliases(self):
        ...
        b1 = make_interp_spline(
            self.xx, yy, k=3, bc_type=("natural", "clamped")
        )
        ...
        b1 = make_interp_spline(
            self.xx, yy, k=3, bc_type=("clamped", "natural")
        )
        assert_allclose(b1.c, b2.c, atol=1e-15)
    ...
    def test_woodbury(self):
        ...
        for k in range(3, 32, 2):
            ...
            assert_allclose(
                _woodbury_algorithm(d, ur, ll, b, k),
                np.linalg.solve(a, b),
                atol=1e-14,
            )
...
def data_file(basename):
    return os.path.join(
        os.path.abspath(os.path.dirname(__file__)), "data", basename
    )
...
class TestNdBSpline:
    ...
    def test_2D_separable(self):
        ...
        assert_allclose(
            [bspline2(xy, t2, c2, k) for xy in xi], target, atol=1e-14
        )
    ...
    def test_2D_separable_2(self):
        ...
        assert_allclose(
            bspl2_4(xi) - np.asarray(target)[:, None], 0, atol=5e-14
        )
        ...
        assert_allclose(
            result,
            [[val_single, val_single], [val_single, val_single]],
            atol=1e-14,
        )
        ...
        assert (
            bspl2_22(xi).shape == np.shape(xi)[:-1] + bspl2_22.c.shape[ndim:]
        )
        assert_allclose(
            bspl2_22(xi) - np.asarray(target)[:, None, None], 0, atol=5e-14
        )
    ...
    def test_2D_derivative(self):
        ...
        der = bspl2(xi, nu=(1, 0))
        assert_allclose(
            der, [3 * x**2 * (y**2 + 2 * y) for x, y in xi], atol=1e-14
        )

        der = bspl2(xi, nu=(1, 1))
        assert_allclose(
            der, [3 * x**2 * (2 * y + 2) for x, y in xi], atol=1e-14
        )

        der = bspl2(xi, nu=(0, 0))
        assert_allclose(
            der, [x**3 * (y**2 + 2 * y) for x, y in xi], atol=1e-14
        )
    ...
    def test_3D_random(self):
        ...
        c = rng.uniform(
            size=(tx.size - k - 1, ty.size - k - 1, tz.size - k - 1)
        )
...
class TestMakeND:
    ...
    def test_2D_separable_trailing_dims(self):
        ...
        assert_allclose(
            result.reshape(6, 6, 2, 2),
            target.reshape((6, 6, 2, 2)),
            atol=1e-14,
        )
