...
class interp1d(_Interpolator1D):
    ...
    def __init__(
        ...
        if kind in ("linear", "nearest", "nearest-up", "previous", "next"):
            ...
            if order > 1:
                ...
                if mask.any():
                    ...
                    xx = np.linspace(
                        np.nanmin(self.x), np.nanmax(self.x), len(self.x)
                    )
                    rewrite_nan = True
            ...
            self._spline = make_interp_spline(
                xx, yy, k=order, check_finite=False
            )
            if rewrite_nan:
        ...
        if len(self.x) < minval:
            raise ValueError(
                "x and y arrays must have at " "least %d entries" % minval
            )
    ...
    def fill_value(self, fill_value):
        ...
        if _do_extrapolate(fill_value):
            ...
            broadcast_shape = (
                self.y.shape[: self.axis] + self.y.shape[self.axis + 1 :]
            )
            ...
            if isinstance(fill_value, tuple) and len(fill_value) == 2:
                ...
                below_above = [
                    _check_broadcast_up_to(
                        fill_value, broadcast_shape, "fill_value"
                    )
                ] * 2
...
class _PPolyBase:
    ...
    def extend(self, c, x):
        ...
        if self.x[-1] >= self.x[0]:
            if not x[-1] >= x[0]:
                raise ValueError(
                    "`x` is in the different order " "than `self.x`."
                )
            ...
            if x[0] >= self.x[-1]:
                ...
                raise ValueError(
                    "`x` is neither on the left or on the right "
                    "from `self.x`."
                )
        else:
            if not x[-1] <= x[0]:
...
class PPoly(_PPolyBase):
    ...
    def antiderivative(self, nu=1):
        ...
        _ppoly.fix_continuity(
            c.reshape(c.shape[0], c.shape[1], -1), self.x, nu - 1
        )
...
class BPoly(_PPolyBase):
    ...
    def from_power_basis(cls, pp, extrapolate=None):
        ...
        for a in range(k + 1):
            factor = (
                pp.c[a] / comb(k, k - a) * dx[(slice(None),) + rest] ** (k - a)
            )
    ...
    def from_derivatives(cls, xi, yi, orders=None, extrapolate=None):
        ...
        for i in range(m):
            ...
            b = BPoly._construct_from_derivatives(
                xi[i], xi[i + 1], y1[:n1], y2[:n2]
            )
            if len(b) < k:
    ...
    def _construct_from_derivatives(xa, xb, ya, yb):
        ...
        for q in range(0, nb):
            c[-q - 1] = (
                yb[q] / spec.poch(n - q, q) * (-1) ** q * (xb - xa) ** q
            )
            for j in range(0, q):
...
class NdPPoly:
    ...
    def __init__(self, c, x, extrapolate=None):
        ...
        if any(np.any(v[1:] - v[:-1] < 0) for v in self.x):
            raise ValueError("x-coordinates are not in increasing order")
        if any(
            a != b.size - 1 for a, b in zip(c.shape[ndim : 2 * ndim], self.x)
        ):
