...
class BSpline:
    ...
    **Implementation details**
    ...
    def __init__(self, t, c, k, extrapolate=True, axis=0):
        ...
        if n < self.k + 1:
            raise ValueError(
                "Need at least %d knots for degree %d" % (2 * k + 2, k)
            )
    ...
    def __call__(self, x, nu=0, extrapolate=None):
        ...
        if extrapolate == "periodic":
            ...
            x = self.t[self.k] + (x - self.t[self.k]) % (
                self.t[n] - self.t[self.k]
            )
            extrapolate = False
    ...
    def derivative(self, nu=1):
        ...
        return self.construct_fast(
            *tck, extrapolate=self.extrapolate, axis=self.axis
        )
    ...
    def antiderivative(self, nu=1):
        ...
        return self.construct_fast(
            *tck, extrapolate=extrapolate, axis=self.axis
        )
    ...
    def integrate(self, a, b, extrapolate=None):
        ...
        if extrapolate == "periodic":
            ...
            if n_periods > 0:
                ...
                integral = np.zeros(
                    (1, prod(self.c.shape[1:])), dtype=self.c.dtype
                )
...
def _make_periodic_spline(x, y, t, k, axis):
    ...
    if n <= k:
        ...
        return BSpline.construct_fast(
            t, c, k, extrapolate="periodic", axis=axis
        )
...
def make_interp_spline(
    x, y, k=3, t=None, bc_type=None, axis=0, check_finite=True
):
    ...
    if bc_type == "periodic" and not np.allclose(y[0], y[-1], atol=1e-15):
        raise ValueError(
            "First and last points does not match while "
    ...
    if t.size < x.size + k + 1:
        raise ValueError(
            "Got %d knots, need at least %d." % (t.size, x.size + k + 1)
        )
    ...
    lu, piv, c, info = gbsv(
        kl, ku, ab, rhs, overwrite_ab=True, overwrite_b=True
    )
