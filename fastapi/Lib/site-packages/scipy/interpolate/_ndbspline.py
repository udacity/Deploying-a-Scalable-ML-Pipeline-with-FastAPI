...
class NdBSpline:
    ...
    def __init__(self, t, c, k, *, extrapolate=None):
        ...
        for d in range(ndim):
            ...
            if td.ndim != 1:
                raise ValueError(
                    f"Knot vector in dimension {d} must be"
                    f" one-dimensional."
                )
            if n < kd + 1:
                raise ValueError(
            ...
            if (np.diff(td) < 0).any():
                raise ValueError(
                    f"Knots in dimension {d} must be in a"
            ...
            if self.c.ndim < ndim:
                raise ValueError(
                    f"Coefficients must be at least" f" {d}-dimensional."
                )
            if self.c.shape[d] != n:
                raise ValueError(
...
def make_ndbspl(points, values, k=3, *, solver=ssl.gcrotmk, **solver_args):
    ...
    t = tuple(
        _not_a_knot(np.asarray(points[d], dtype=float), k[d])
        for d in range(ndim)
    )
