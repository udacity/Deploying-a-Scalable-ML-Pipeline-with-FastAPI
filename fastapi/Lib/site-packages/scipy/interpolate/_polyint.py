...
class _Interpolator1D:
    ...
    def _set_yi(self, yi, xi=None, axis=None):
        ...
        if xi is not None and shape[axis] != len(xi):
            raise ValueError(
                "x and y arrays must be equal in length along "
                "interpolation axis."
            )

        self._y_axis = axis % yi.ndim
        self._y_extra_shape = (
            yi.shape[: self._y_axis] + yi.shape[self._y_axis + 1 :]
        )
        self.dtype = None
...
class BarycentricInterpolator(_Interpolator1DWithDerivatives):
    ...
    def __init__(self, xi, yi=None, axis=0, *, wi=None, random_state=None):
        ...
        if wi is not None:
            ...
            for i in range(self.n):
                ...
                if prod == 0.0:
                    raise ValueError(
                        "Interpolation points xi must be" " distinct."
                    )
                self.wi[i] = 1.0 / prod
    ...
    def _evaluate_derivatives(self, x, der=None, all_lower=True):
        ...
        return self._diff_baryint._evaluate_derivatives(
            x, der - 1, all_lower=False
        )
