...
def splprep(
    ...
    if ier <= 0 and not quiet:
        warnings.warn(
            RuntimeWarning(
                _iermess[ier][0]
                + "\tk=%d n=%d m=%d fp=%f s=%f" % (k, len(t), m, fp, s)
            ),
            stacklevel=2,
        )
...
def splrep(
    ...
    if not (1 <= k <= 5):
        raise TypeError(
            "Given degree of the spline (k=%d) is not supported. "
            "(1<=k<=5)" % k
        )
    if m <= k:
        raise TypeError("m > k must hold")
    ...
    if task <= 0:
        if per:
            _curfit_cache["wrk"] = empty(
                (m * (k + 1) + nest * (8 + 5 * k),), float
            )
        else:
            _curfit_cache["wrk"] = empty(
                (m * (k + 1) + nest * (7 + 3 * k),), float
            )
    ...
    if not per:
        n, c, fp, ier = dfitpack.curfit(
            task, x, y, w, t, wrk, iwrk, xb, xe, k, s
        )
    else:
...
def splev(x, tck, der=0, ext=0):
    ...
    if parametric:
        return list(
            map(
                lambda c, x=x, t=t, k=k, der=der: splev(
                    x, [t, c, k], der, ext
                ),
                c,
            )
...
def sproot(tck, mest=10):
    ...
    if parametric:
        ...
        if ier == 1:
            warnings.warn(
                RuntimeWarning("The number of zeros exceeds mest"),
                stacklevel=2,
            )
            return z[:m]
...
def bisplrep(
    ...
    if ierm > 0 and not full_output:
        if ier in [1, 2, 3, 4, 5]:
            ...
            warnings.warn(
                RuntimeWarning(_iermess2[ierm][0] + _mess), stacklevel=2
            )
        else:
            try:
...
def bisplev(x, y, tck, dx=0, dy=0):
    ...
    if dx != 0 or dy != 0:
        _int_overflow(
            (tx.size - kx - 1) * (ty.size - ky - 1), MemoryError, msg=msg
        )
