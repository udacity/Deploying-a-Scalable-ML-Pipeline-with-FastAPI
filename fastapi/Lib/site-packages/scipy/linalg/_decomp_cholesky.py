...
def _cholesky(
    a, lower=False, overwrite_a=False, clean=True, check_finite=True
):
...
def cho_solve(c_and_lower, b, overwrite_b=False, check_finite=True):
    ...
    if info != 0:
        raise ValueError(
            "illegal value in %dth argument of internal potrs" % -info
        )
    return x
...
def cho_solve_banded(cb_and_lower, b, overwrite_b=False, check_finite=True):
    ...
    if info < 0:
        raise ValueError(
            "illegal value in %dth argument of internal pbtrs" % -info
        )
    return x
