...
def test_reflection_coeffs():
    ...
    for i in range(2, 10):
        reflection_coeffs_d.append(
            solve_toeplitz(y_d[: (i - 1)], b=y_d[1:i])[-1]
        )
        reflection_coeffs_z.append(
            solve_toeplitz(y_z[: (i - 1)], b=y_z[1:i])[-1]
        )
...
def test_empty(dt_c, dt_b):
    ...
    assert (
        x.dtype
        == solve_toeplitz(
            np.array([2, 1], dtype=dt_c), np.ones(2, dtype=dt_b)
        ).dtype
    )
