...
class TestFBLAS2Simple:
    ...
    def test_syr_her(self):
        ...
        for p, rtol in zip("sd", [1e-7, 1e-14]):
            ...
            assert_allclose(
                f(1.0, y, incx=-2, offx=2, n=4), resx_reverse, rtol=rtol
            )
        ...
        for p, rtol in zip("cz", [1e-7, 1e-14]):
            ...
            # negative increments imply reversed vectors in blas
            assert_allclose(
                f(1.0, w, incx=-3, offx=1, n=4), resz_reverse, rtol=rtol
            )
        ...
        for p, rtol in zip("cz", [1e-7, 1e-14]):
            ...
            # negative increments imply reversed vectors in blas
            assert_allclose(
                f(1.0, w, incx=-3, offx=1, n=4), rehz_reverse, rtol=rtol
            )
    ...
    def test_syr2(self):
        ...
        for p, rtol in zip("sd", [1e-7, 1e-14]):
            ...
            assert_raises(
                Exception, f, 1.0, x, y, a=np.zeros((2, 2), "d", "F")
            )
    ...
    def test_her2(self):
        ...
        for p, rtol in zip("cz", [1e-7, 1e-14]):
            ...
            assert_allclose(f(1.0, x, y, n=3), resxy[:3, :3], rtol=rtol)
            assert_allclose(
                f(1.0, x, y, lower=True), resxy.T.conj(), rtol=rtol
            )
            ...
            # negative increments imply reversed vectors in blas
            assert_allclose(
                f(1.0, u, v, incx=-3, offx=1, incy=-3, offy=1),
                resxy_reverse,
                rtol=rtol,
            )
    ...
    def test_gbmv(self):
        ...
        for ind, dtype in enumerate(DTYPES):
            ...
            y1 = func(
                m=m, n=n, ku=ku, kl=kl, alpha=alpha, a=Ab, x=x, y=y, beta=beta
            )
    ...
    def test_tpmv(self):
        ...
        for ind, dtype in enumerate(DTYPES):
            ...
            A = (
                triu(rand(n, n))
                if ind < 2
                else triu(rand(n, n) + rand(n, n) * 1j)
            )
...
class TestBLAS3Symm:
    ...
    def test_symm(self):
        for f in _get_func("symm"):
            ...
            res = f(
                a=self.a.T, b=self.b, lower=1, c=self.c, alpha=1.0, beta=1.0
            )
            assert_array_almost_equal(res, self.t)

            res = f(
                a=self.a, b=self.b.T, side=1, c=self.c.T, alpha=1.0, beta=1.0
            )
    ...
    def test_summ_wrong_side(self):
        ...
        if f is not None:
            assert_raises(
                Exception,
                f,
                **{"a": self.a, "b": self.b, "alpha": 1, "side": 1}
            )
...
class TestBLAS3Syrk:
    def setup_method(self):
        ...
        self.t = np.array(
            [[1.0, 0.0, 2.0], [0.0, 4.0, -6.0], [2.0, -6.0, 13.0]]
        )
    ...
    def test_syrk_wrong_c(self):
        ...
        if f is not None:
            assert_raises(
                Exception,
                f,
                **{"a": self.a, "alpha": 1.0, "c": np.ones((5, 8))}
            )
...
class TestBLAS3Syr2k:
    def setup_method(self):
        ...
        self.t = np.array(
            [[0.0, -1.0, 3.0], [-1.0, 0.0, 0.0], [3.0, 0.0, 6.0]]
        )
