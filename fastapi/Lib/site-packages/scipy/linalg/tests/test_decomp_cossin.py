...
def test_cossin(dtype_, m, p, q, swap_sign):
    ...
    assert_allclose(
        x, u @ cs @ vh, rtol=0.0, atol=m * 1e3 * np.finfo(dtype_).eps
    )
    ...
    _, cs2, _ = cossin(
        x, p, q, compute_u=False, compute_vh=False, swap_sign=swap_sign
    )
    assert_allclose(cs, cs2, rtol=0.0, atol=10 * np.finfo(dtype_).eps)
...
def test_cossin_mixed_types():
    ...
    assert_allclose(
        x, u @ cs @ vh, rtol=0.0, atol=1e4 * np.finfo(np.complex128).eps
    )
