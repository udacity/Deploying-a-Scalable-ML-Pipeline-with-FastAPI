...
def make_strided(arrs):
    ...
    for a in arrs:
        if a.ndim == 1:
            ...
            raise ValueError(
                "make_strided only works for ndim = 1 or" " 2 arrays"
            )
        ret.append(view)
    return ret
...
def negate_strides(arrs):
    ...
    for a in arrs:
        ...
        if b.ndim == 2:
            ...
            raise ValueError(
                "negate_strides only works for ndim = 1 or" " 2 arrays"
            )
        b[...] = a
        ret.append(b)
    return ret
...
class BaseQRdelete(BaseQRdeltas):
    ...
    def test_sqr_p_col(self):
        ...
        for ndel in range(2, 6):
            for col in range(r.shape[1] - ndel):
                q1, r1 = qr_delete(
                    q, r, col, ndel, which="col", overwrite_qr=False
                )
    ...
    def base_non_simple_strides(
        self, adjust_strides, ks, p, which, overwriteable
    ):
        if which == "row":
        ...
        for type, k in itertools.product(["sqr", "tall", "fat"], ks):
            ...
            if p == 1:
                ...
                if k < 0:
                    s = slice(
                        k,
                        k + p + (a.shape[0] if which == "row" else a.shape[1]),
                    )
