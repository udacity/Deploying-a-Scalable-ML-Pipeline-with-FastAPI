...
class TestLU:
    ...
    def test_old_lu_smoke_tests(self):
        ...
        p, l, u = lu(a)
        result_lu = np.array(
            [[2.0, 5.0, 6.0], [0.5, -0.5, 0.0], [0.5, 1.0, 0.0]]
        )
        ...
        p, l, u = lu(a)
        result_lu = np.array(
            [[2.0, 5.0j, 6.0], [0.5, 2 - 2.5j, 0.0], [0.5, 1.0, 0.0]]
        )
        assert_allclose(p, np.rot90(np.eye(3)))
        assert_allclose(l, np.tril(result_lu, k=-1) + np.eye(3))
        assert_allclose(u, np.triu(result_lu))
        ...
        assert_allclose(
            l, np.array([[1, 0, 0], [1 / 7, 1, 0], [4 / 7, 0.5, 1]])
        )
        assert_allclose(
            u,
        ...
        assert_allclose(p, np.array([[0, 1, 0], [0, 0, 1], [1, 0, 0]]))
        assert_allclose(
            l, np.array([[1, 0, 0], [1 / 9, 1, 0], [5 / 9, 0.5, 1]])
        )
        assert_allclose(
            u,
            np.array(
                [[9, 10, 12, 12], [0, 8 / 9, 15 / 9, 24 / 9], [0, 0, -0.5, 0]]
            ),
            rtol=0.0,
            atol=1e-14,
        )
        ...
        assert_allclose(
            l,
            np.array(
                [[1.0, 0, 0], [0.1, 1, 0], [0.7, -0.5, 1], [0.4, 0.25, 0.5]]
            ),
        )
        assert_allclose(
            u, np.array([[10, 12, 12], [0, 0.8, 1.8], [0, 0, 1.5]])
        )
        ...
        assert_allclose(
            u, np.array([[10, 12, 12], [0, 0.8, 1.8], [0, 0, 1.5]]) * 1j
        )
