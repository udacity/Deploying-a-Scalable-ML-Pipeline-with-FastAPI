...
def lu_factor(a, overwrite_a=False, check_finite=True):
    ...
    if info < 0:
        raise ValueError(
            "illegal value in %dth argument of "
...
def lu_solve(lu_and_piv, b, trans=0, overwrite_b=False, check_finite=True):
    ...
    if b1.size == 0:
        m = lu_solve(
            (np.eye(2, dtype=lu.dtype), [0, 1]), np.ones(2, dtype=b.dtype)
        )
    ...
    if info == 0:
        return x
    raise ValueError(
        "illegal value in %dth argument of internal gesv|posv" % -info
    )
