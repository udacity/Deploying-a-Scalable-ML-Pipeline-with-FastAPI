...
def svd(
    ...
    if lapack_driver not in ("gesdd", "gesvd"):
        message = (
            f'lapack_driver must be "gesdd" or "gesvd", not "{lapack_driver}"'
        )
    ...
    if info < 0:
        raise ValueError(
            "illegal value in %dth argument of internal gesdd" % -info
        )
    if compute_uv:
        return u, s, v
    else:
...
def svdvals(a, overwrite_a=False, check_finite=True):
    ...
    """
    return svd(
        a, compute_uv=0, overwrite_a=overwrite_a, check_finite=check_finite
    )
