    ...
    *,
    ...
    if b is None:  # Standard problem
        drv, drvlw = get_lapack_funcs(
            (pfx + driver, pfx + driver + "_lwork"), [a1]
        )
...
def _check_select(select, select_range, max_ev, max_len):
    ...
    if select != 0:  # (non-all)
        ...
        if sr.ndim != 1 or sr.size != 2 or sr[1] < sr[0]:
            raise ValueError(
                "select_range must be a 2-element array-like "
...
def eigvals(
    a, b=None, overwrite_a=False, check_finite=True, homogeneous_eigvals=False
):
    """
...
def eigh_tridiagonal(
    ...
    if lapack_driver == "sterf":
        ...
        if not eigvals_only:
            raise ValueError(
                "sterf can only be used when eigvals_only is " "True"
            )
        w, info = func(d, e)
        m = len(w)
        ...
        m, w, iblock, isplit, info = stebz(
            d, e, select, vl, vu, il, iu, tol, order
        )
    else:  # 'stemr'
...
def cdf2rdf(w, v):
    ...
    if v.shape[-1] != n:
        raise ValueError(
            "expected the same number of eigenvalues as " "eigenvectors"
        )
    ...
    for i in idx_stack:
        ...
        assert (
            i[0::2] == i[1::2]
        ).all(), "Conjugate pair spanned different arrays!"
        stack_ind += (i[0::2],)
