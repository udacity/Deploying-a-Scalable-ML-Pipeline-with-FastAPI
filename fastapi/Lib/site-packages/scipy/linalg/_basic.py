...
lapack_cast_dict = {
    x: "".join([y for y in "fdFD" if np.can_cast(x, y)])
...
def solve(
    ...
    if n != b1.shape[0]:
        ...
        if not (n == 1 and b1.size != 0):
            raise ValueError(
                "Input b has to have same number of rows as " "input a"
            )
    ...
    if assume_a == "gen":
        gecon, getrf, getrs = get_lapack_funcs(
            ("gecon", "getrf", "getrs"), (a1, b1)
        )
...
def solve_triangular(
    ...
    raise ValueError(
        "illegal value in %dth argument of internal trtrs" % (-info)
    )
...
def solve_banded(
    ...
    raise ValueError(
        "illegal value in %d-th argument of internal " "gbsv/gtsv" % -info
    )
...
def solveh_banded(
    ...
    if a1.shape[0] == 2:
        ...
        if lower:
            ...
            e = a1[0, 1:].conj()
        d, du, x, info = ptsv(
            d, e, b1, overwrite_ab, overwrite_ab, overwrite_b
        )
    else:
    ...
    if info < 0:
        raise ValueError(
            "illegal value in %dth argument of internal " "pbsv" % -info
        )
    return x
...
def solve_circulant(
    c, b, singular="raise", tol=None, caxis=-1, baxis=0, outaxis=0
):
...
def inv(a, overwrite_a=False, check_finite=True):
    ...
    if info < 0:
        raise ValueError(
            "illegal value in %d-th argument of internal "
            "getrf|getri" % -info
        )
    return inv_a
...
def lstsq(
    ...
    if driver in ("gelss", "gelsd"):
        if driver == "gelss":
            ...
            if real_data:
                ...
                x, s, rank, info = lapack_func(
                    a1, b1, lwork, iwork, cond, False, False
                )
            else:  # complex data
                lwork, rwork, iwork = _compute_lwork(
                    lapack_lwork, m, n, nrhs, cond
                )
                x, s, rank, info = lapack_func(
        ...
        v, x, j, rank, info = lapack_func(
            a1, b1, jptv, cond, lwork, False, False
        )
        if info < 0:
            raise ValueError(
...
def pinvh(
    a, atol=None, rtol=None, lower=True, return_rank=False, check_finite=True
):
    """
...
def matrix_balance(
    A, permute=True, scale=True, separate=False, overwrite_a=False
):
    """
...
def matmul_toeplitz(c_or_cr, x, check_finite=False, workers=None):
    ...
    if np.iscomplexobj(embedded_col) or np.iscomplexobj(x):
        ...
        mat_times_x = ifft(fft_mat * fft_x, axis=0, workers=workers)[
            :T_nrows, :
        ]
    else:
        ...
        mat_times_x = irfft(fft_mat * fft_x, axis=0, workers=workers, n=p)[
            :T_nrows, :
        ]
