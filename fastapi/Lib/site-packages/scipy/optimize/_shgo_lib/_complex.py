...
class Complex:
    ...
    def triangulate(
        self, n=None, symmetry=None, centroid=True, printout=False
    ):
        """
        ...
        if symmetry is None:
            ...
            for i, j in enumerate(symmetry):
                if i is not j:
                    ...
                    if (
                        self.bounds[symmetry[i]]
                        is not self.bounds[symmetry[j]]
                    ):
                        logging.warning(
        ...
        if n is None:
            ...
            try:
                ...
                self.cp = self.cyclic_product(
                    cbounds, origin, supremum, centroid
                )
    ...
    def refine_local_space(self, origin, supremum, bounds, centroid=1):
        ...
        try:
            self.triangulated_vectors.remove(
                (tuple(origin_c), tuple(supremum_c))
            )
        except ValueError:
    ...
    def connect_vertex_non_symm(self, v_x, near=None):
        ...
        for s_i in itertools.combinations(
            range(S_rows.shape[0]), r=self.dim + 1
        ):
            ...
            for i in itertools.combinations(s_i, r=2):
                ...
                if (
                    self.V[tuple(S_rows[i[1]])]
                    not in self.V[tuple(S_rows[i[0]])].nn
                ) and (
                    self.V[tuple(S_rows[i[0]])]
                    not in self.V[tuple(S_rows[i[1]])].nn
                ):
                    valid_simplex = False
                    break
