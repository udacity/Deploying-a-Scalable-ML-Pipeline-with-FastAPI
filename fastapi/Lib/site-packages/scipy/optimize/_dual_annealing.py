...
class VisitingDistribution:
    ...
    def visiting(self, x, step, temperature):
        ...
        if step < dim:
            ...
            x_visit[
                np.fabs(x_visit - self.lower) < self.MIN_VISIT_BOUND
            ] += 1.0e-10
        else:
            ...
            x_visit[index] = (
                np.fmod(b, self.bound_range[index]) + self.lower[index]
            )
            if (
                np.fabs(x_visit[index] - self.lower[index])
                < self.MIN_VISIT_BOUND
            ):
    ...
    def run(self, step, temperature):
        ...
        for j in range(self.energy_state.current_location.size * 2):
            ...
            if self.func_wrapper.nfev >= self.func_wrapper.maxfun:
                return (
                    "Maximum number of function call reached "
                    "during annealing"
                )
...
class LocalSearchWrapper:
    ...
    def __init__(self, search_bounds, func_wrapper, *args, **kwargs):
        ...
        if not self.kwargs:
            n = len(self.lower)
            ls_max_iter = min(
                max(n * self.LS_MAXITER_RATIO, self.LS_MAXITER_MIN),
                self.LS_MAXITER_MAX,
            )
