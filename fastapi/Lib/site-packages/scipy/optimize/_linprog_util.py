...
def _presolve(lp, rr, rr_method, tol=1e-9):
    ...
    if rr_method is not None and rr_method.lower() not in {
        "svd",
        "pivot",
        "id",
    }:
    ...
    if np.any(zero_row):
        if np.any(
            np.logical_and(zero_row, np.abs(b_eq) > tol)
        ):  # test_zero_row_1
...
def _check_result(
    x, fun, status, slack, con, bounds, tol, message, integrality
):
    """
