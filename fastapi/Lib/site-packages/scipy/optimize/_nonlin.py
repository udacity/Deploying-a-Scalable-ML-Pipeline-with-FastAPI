...
def _nonlin_line_search(
    func, x, Fx, dx, search_type="armijo", rdiff=1e-8, smin=1e-2
):
    tmp_s = [0]
    tmp_Fx = [Fx]
    tmp_phi = [norm(Fx) ** 2]
...
class LowRankMatrix:
    ...
    def _matvec(v, alpha, cs, ds):
        axpy, scal, dotc = get_blas_funcs(
            ["axpy", "scal", "dotc"], cs[:1] + [v]
        )
        w = alpha * v
...
class BroydenFirst(GenericBroyden):
    ...
    def __init__(self, alpha=None, reduction_method="restart", max_rank=None):
        ...
        if reduction_method == "svd":
            ...
            raise ValueError(
                "Unknown rank reduction method '%s'" % reduction_method
            )
        ...
        for i in range(n):
            for j in range(n):
                ...
                if i == j and self.w0 != 0:
                    b[i, j] -= (
                        vdot(self.df[i], self.df[i]) * self.w0**2 * self.alpha
                    )
        gamma = solve(b, df_f)
