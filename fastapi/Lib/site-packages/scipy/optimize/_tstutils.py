...
def aps10_fpp(x, n):
    return np.exp(-n * x) * (-n * (-n * (x - 1) + 1) + -n * x) + n * (
        n - 1
    ) * x ** (n - 2)
...
_APS_TESTS_DICTS = [
    dict(zip(_APS_TESTS_KEYS, testcase)) for testcase in _APS_TESTS
]
...
def fun8(x):
    ...
    xi = 0.61489
    return (
        -(3062 * (1 - xi) * xp.exp(-x)) / (xi + (1 - xi) * xp.exp(-x))
        - 1013
        + 1628 / x
    )
...
_CHANDRUPATLA_TESTS_DICTS = [
    dict(zip(_CHANDRUPATLA_TESTS_KEYS, testcase))
