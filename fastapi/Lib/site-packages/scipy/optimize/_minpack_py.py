...
def _check_func(
    checker, argname, thefunc, x0, args, numinputs, output_shape=None
):
...
def _root_hybr(
    ...
    **unknown_options,
    ...
    sol = OptimizeResult(
        x=x, success=(status == 1), status=status, method="hybr"
    )
    sol.update(info)
    try:
    ...
    if transform is None:
        ...
        def func_wrapped(params):
            return solve_triangular(
                transform, func(xdata, *params) - ydata, lower=True
            )
    ...
    **kwargs
    ...
    if not check_finite and nan_policy is not None:
        if nan_policy == "propagate":
            raise ValueError(
                "`nan_policy='propagate'` is not supported "
                "by this function."
            )

        policies = [None, "raise", "omit"]
        x_contains_nan, nan_policy = _contains_nan(
            xdata, nan_policy, policies=policies
        )
        y_contains_nan, nan_policy = _contains_nan(
            ydata, nan_policy, policies=policies
        )
    ...
    if method == "lm":
        ...
        res = least_squares(
            func, p0, jac=jac, bounds=bounds, method=method, **kwargs
        )
