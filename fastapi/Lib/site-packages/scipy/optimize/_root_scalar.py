...
def root_scalar(
    ...
    if meth in ["bisect", "ridder", "brentq", "brenth", "toms748"]:
        ...
        r, sol = methodc(
            f, x0, args=args, fprime=None, fprime2=None, x1=x1, **kwargs
        )
    elif meth in ["newton"]:
        if x0 is None:
            raise ValueError("x0 must not be None for %s" % method)
        ...
        if "xtol" in kwargs:
            kwargs["tol"] = kwargs.pop("xtol")
        r, sol = methodc(
            f, x0, args=args, fprime=fprime, fprime2=None, **kwargs
        )
    elif meth in ["halley"]:
        if x0 is None:
            raise ValueError("x0 must not be None for %s" % method)
        ...
        if "xtol" in kwargs:
            kwargs["tol"] = kwargs.pop("xtol")
        r, sol = methodc(
            f, x0, args=args, fprime=fprime, fprime2=fprime2, **kwargs
        )
    else:
