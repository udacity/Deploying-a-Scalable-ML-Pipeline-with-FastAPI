...
def _wrap_nan_raise(f):
    ...
    def f_raise(x, *args):
        ...
        if np.isnan(fx):
            msg = (
                f"The function value at x={x} is NaN; "
    ...
    if zero_der.any():
        ...
        if fprime is None:
            ...
            if zero_der_nz_dp.any():
                ...
                warnings.warn(
                    f"RMS of {rms:g} reached", RuntimeWarning, stacklevel=3
                )
        ...
        msg = (
            f"{all_or_some:s} failed to converge after {maxiter:d} iterations"
        )
        if failures.all():
            raise RuntimeError(msg)
        warnings.warn(msg, RuntimeWarning, stacklevel=3)
...
def _compute_divided_differences(
    xvals, fvals, N=None, full=True, forward=True
):
