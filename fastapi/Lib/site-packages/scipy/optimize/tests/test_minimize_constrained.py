...
class TestTrustRegionConstr:
    ...
    def test_issue_15093(self):
        ...
        bounds = Bounds(
            np.array([0.0, 0.0]), np.array([1.0, 1.0]), keep_feasible=True
        )

        with suppress_warnings() as sup:
            sup.filter(UserWarning, "delta_grad == 0.0")
            result = minimize(
                method="trust-constr", fun=obj, x0=x0, bounds=bounds
            )
...
@pytest.mark.xfail(
    reason="Known bug in trust-constr; see gh-11649.", strict=True
)
def test_gh11649():
    ...
    res = minimize(
        fun=obj, x0=x0, method="trust-constr", bounds=bnds, constraints=nlcs
    )
    assert res.success
    ...
    ref = minimize(
        fun=obj, x0=x0, method="slsqp", bounds=bnds, constraints=nlcs
    )
...
class TestBoundedNelderMead:
    ...
    def test_rosen_brock_with_bounds(self, bounds, x_opt):
        ...
        with suppress_warnings() as sup:
            ...
            result = minimize(
                prob.fun, [-10, -10], method="Nelder-Mead", bounds=bounds
            )
    ...
    def test_equal_all_bounds(self):
        ...
        with suppress_warnings() as sup:
            ...
            result = minimize(
                prob.fun, [-10, 8], method="Nelder-Mead", bounds=bounds
            )
