...
class TestDifferentialEvolutionSolver:
    ...
    def test_invalid_mutation_values_arent_accepted(self):
        ...
        mutation = 0.5
        solver = DifferentialEvolutionSolver(
            func, self.bounds, mutation=mutation
        )
    ...
    def test_intermediate_result(self):
        ...
        def callback(intermediate_result):
            ...
            assert_equal(
                intermediate_result.x, intermediate_result.population[0]
            )
            assert_equal(
    ...
    def test_args_tuple_is_passed(self):
        ...
        result = differential_evolution(
            quadratic, bounds, args=args, polish=True
        )
    ...
    def test_init_with_invalid_strategy(self):
        ...
        bounds = [(-3, 3)]
        assert_raises(
            ValueError, differential_evolution, func, bounds, strategy="abc"
        )
    ...
    def test_maxiter_stops_solve(self):
        ...
        assert_equal(
            result.message, "Maximum number of iterations has been exceeded."
        )
    ...
    def test_maxfun_stops_solve(self):
        ...
        solver = DifferentialEvolutionSolver(
            rosen, self.bounds, maxfun=1, polish=False
        )
        result = solver.solve()
    ...
    def test_quadratic(self):
        ...
        solver = DifferentialEvolutionSolver(
            self.quadratic, [(-100, 100)], tol=0.02
        )
        solver.solve()
    ...
    def test_iteration(self):
        ...
        # if popsize is 3, then the overall generation has size (6,)
        solver = DifferentialEvolutionSolver(
            rosen, self.bounds, popsize=3, maxfun=12
        )
        x, fun = next(solver)
    ...
    def test_convergence(self):
        solver = DifferentialEvolutionSolver(
            rosen, self.bounds, tol=0.2, polish=False
        )
        solver.solve()
    ...
    def test_population_initiation(self):
        ...
        unscaled_population = np.clip(
            solver._unscale_parameters(population), 0, 1
        )
    ...
    def test_impossible_constraint(self):
        ...
        solver.constraint_violation[[0, l], :] = solver.constraint_violation[
            [l, 0], :
        ]
    ...
    def test_accept_trial(self):
        ...
        assert (
            fn(1.0, True, np.array([0.0]), 0.1, True, np.array([0.0])) is False
        )
        ...
        assert fn(
            0.1, False, np.array([0.5, 0.5]), 1.0, False, np.array([1.0, 1.0])
        )
        assert fn(
            0.1, False, np.array([0.5, 0.5]), 1.0, False, np.array([1.0, 0.50])
        )
        assert not (
            fn(
                1.0,
    ...
    def test_constraint_wrapper(self):
        ...
        pc = _ConstraintWrapper(
            LinearConstraint(csr_matrix(A), -np.inf, 0), x0
        )
        assert (pc.violation(x0) > 0).any()
        assert (pc.violation([-10, 2, -10, 4]) == 0).all()
    ...
    def test_integrality(self):
        ...
        LBFGSB = minimize(
            func2, res2.x[1], args=(5, dist, x), bounds=[(0, 0.95)]
        )
    ...
    def test_vectorized(self):
        ...
        res1 = differential_evolution(
            rosen, bounds, updating="deferred", seed=1
        )
        res2 = differential_evolution(
