...
class TestSLSQP:
    ...
    def test_unbounded_approximated(self):
        ...
        res = fmin_slsqp(
            self.fun, [-1.0, 1.0], args=(-1.0,), iprint=0, full_output=1
        )
    ...
    def test_scalar_constraints(self):
        ...
        x = fmin_slsqp(
            lambda z: z**2, [3.0], ieqcons=[lambda z: z[0] - 1], iprint=0
        )
    ...
    def test_inconsistent_inequalities(self):
        ...
        bounds = ((-5, 5), (-5, 5))
        cons = (
            dict(type="ineq", fun=ineqcons1),
            dict(type="ineq", fun=ineqcons2),
        )
        res = minimize(
            cost, x0, method="SLSQP", bounds=bounds, constraints=cons
        )
    ...
    def test_parameters_stay_within_bounds(self):
        ...
        x0 = np.array(
            bounds.lb + (bounds.ub - bounds.lb) * np.random.random(n_inputs)
        )
