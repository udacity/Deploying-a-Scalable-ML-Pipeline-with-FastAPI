...
class TestChandrupatlaMinimize:
    ...
    def test_maxiter_callback(self):
        ...
        res = _chandrupatla_minimize(
            self.f, *bracket, args=(loc,), maxiter=maxiter
        )
        ...
        res2 = _chandrupatla_minimize(
            self.f, *bracket, args=(loc,), callback=callback
        )
    ...
    def test_dtype(self, loc, dtype):
        ...
        res = _chandrupatla_minimize(
            f, dtype(-3), dtype(1), dtype(5), args=(loc,)
        )
    ...
    def test_special_cases(self):
        ...
        res = _chandrupatla_minimize(
            f, 0, 1, np.pi, xatol=0, xrtol=0, fatol=0, frtol=0
        )
        assert res.success
...
class TestChandrupatla(TestScalarRootFinders):
    ...
    def test_vectorization(self, shape, xp):
        ...
        # input shapes.
        p = (
            np.linspace(-0.05, 1.05, 12).reshape(shape)
            if shape
            else np.float64(0.6)
        )
        p_xp = xp.asarray(p)
        ...
        res = _chandrupatla_root(
            f, xp.asarray(-5.0), xp.asarray(5.0), args=args_xp
        )
        ...
        ref_success = xp.reshape(
            xp.asarray(ref_success, dtype=xp_test.bool), shape
        )
    ...
    def test_maxiter_callback(self, xp):
        ...
        for key in res.keys():
            if key == "status":
                xp_assert_equal(
                    res[key], xp.asarray(eim._ECONVERR, dtype=xp.int32)
                )
                xp_assert_equal(
                    res2[key], xp.asarray(eim._ECALLBACK, dtype=xp.int32)
                )
    ...
    def test_nit_expected(self, case, xp):
        ...
        xp_assert_close(
            res.fun,
            xp.asarray(f(root), dtype=xp.float64),
            rtol=1e-8,
            atol=2e-3,
        )
    ...
    def test_special_cases(self, xp):
        ...
        a, b = xp.asarray([0.1, 0.0, 0.0, 0.1]), xp.asarray(
            [0.9, 1.0, 0.9, 1.0]
        )
