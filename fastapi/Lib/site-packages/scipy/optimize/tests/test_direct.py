...
class TestDIRECT:
    ...
    def test_direct(self, locally_biased):
        res = direct(
            self.sphere, self.bounds_sphere, locally_biased=locally_biased
        )
    ...
    def test_direct_callback(self, locally_biased):
        ...
        # test accuracy
        assert_allclose(
            res_callback.x, self.optimum_sphere_pos, rtol=1e-3, atol=1e-3
        )
        assert_allclose(
            res_callback.fun, self.optimum_sphere, atol=1e-5, rtol=1e-5
        )
    ...
    def test_failure_maxfun(self, locally_biased):
        ...
        message = (
            "Number of function evaluations done is "
    ...
    def test_bounds_variants(self, locally_biased):
        ...
        res_old_bounds = direct(
            self.sphere, bounds_old, locally_biased=locally_biased
        )
        res_new_bounds = direct(
            self.sphere, bounds_new, locally_biased=locally_biased
        )
    ...
    def test_maxfun_wrong_type(self, maxfun):
        ...
        with pytest.raises(ValueError, match=error_msg):
            direct(
                self.styblinski_tang, self.bounds_stylinski_tang, maxfun=maxfun
            )
    ...
    def test_negative_maxiter(self):
        ...
        with pytest.raises(ValueError, match=error_msg):
            direct(
                self.styblinski_tang, self.bounds_stylinski_tang, maxiter=-1
            )
