...
def run_test(
    ...
    if test.expected_funl is not None:
        np.testing.assert_allclose(
            res.funl, test.expected_funl, atol=test_atol
        )
    return
...
class TestShgoArguments:
    ...
    def test_args_gh14589(self):
        ...
        #16506"""
        res = shgo(
            func=lambda x, y, z: x * z + y, bounds=[(0, 3)], args=(1, 2)
        )
    ...
    def test_15_min_every_iter(self):
        ...
        run_test(
            test1_1, n=1, iters=7, options=options, sampling_method="sobol"
        )
    ...
    def test_20_constrained_args(self):
        ...
        def g1_modified(x, i):
            return (
                i * 2.3 * x[0] + i * 5.6 * x[1] + 11.1 * x[2] + 1.3 * x[3] - 5
            )  # >=0
    ...
    def test_21_2_derivative_options(self, derivative):
        ...
        ref = minimize(
            objective, x0=[0], bounds=bounds, **minimizer_kwargs, **options
        )
    ...
    def test_21_3_hess_options_rosen(self):
        ...
        res = shgo(
            rosen, bounds, minimizer_kwargs=minimizer_kwargs, options=options
        )
...
def test_vector_constraint():
    ...
    res = shgo(
        rosen, [(0, 10), (0, 10)], constraints=oldc, sampling_method="sobol"
    )
