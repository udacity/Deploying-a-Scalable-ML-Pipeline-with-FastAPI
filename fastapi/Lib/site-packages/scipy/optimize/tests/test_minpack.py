...
class TestFSolve:
    ...
    def test_wrong_shape_fprime_callable(self):
        ...
        assert_raises(
            TypeError, optimize.fsolve, func, x0=[0, 1], fprime=deriv_func
        )
...
class TestLeastSq:
    ...
    def test_wrong_shape_func_function(self):
        ...
        x0 = [1.5, 2.0]
        assert_raises(
            TypeError, optimize.leastsq, dummy_func, x0, args=((1,),)
        )
    ...
    def test_wrong_shape_Dfun_callable(self):
        ...
        deriv_func = ReturnShape((2, 2))
        assert_raises(
            TypeError, optimize.leastsq, func, x0=[0, 1], Dfun=deriv_func
        )
    ...
    def test_float32(self):
        ...
        assert_(
            (func(p1, x, y) ** 2).sum() < 1e-4 * (func(p0, x, y) ** 2).sum()
        )
...
class TestCurveFit:
    ...
    def test_one_argument(self):
        ...
        res = curve_fit(
            func, self.x, self.y, full_output=1, check_finite=False
        )
    ...
    def test_pcov(self):
        ...
        with suppress_warnings() as sup:
            ...
            popt, pcov = curve_fit(
                f_flat, xdata, ydata, p0=[2, 0], sigma=sigma
            )
    ...
    def test_indeterminate_covariance(self):
        ...
        assert_warns(
            OptimizeWarning, curve_fit, lambda x, a, b: a * x, xdata, ydata
        )
    ...
    def test_None_x(self):  # Added in GH10196
        popt, pcov = curve_fit(
            lambda _, a: a * np.arange(10), None, 2 * np.arange(10)
        )
    ...
    def test_bounds(self):
        ...
        for method in [None, "trf", "dogbox"]:
            popt, pcov = curve_fit(
                f, xdata, ydata, bounds=bounds, method=method
            )
    ...
    def test_jac(self):
        ...
        for method in ["trf", "dogbox"]:
            for scheme in ["2-point", "3-point", "cs"]:
                popt, pcov = curve_fit(
                    f, xdata, ydata, jac=scheme, method=method
                )
    ...
    def test_maxfev_and_bounds(self):
        ...
        y = 2 * x
        popt1, _ = curve_fit(
            lambda x, p: p * x, x, y, bounds=(0, 3), maxfev=100
        )
        popt2, _ = curve_fit(
            lambda x, p: p * x, x, y, bounds=(0, 3), max_nfev=100
        )
    ...
    def test_curvefit_scalar_sigma(self, absolute_sigma):
        ...
        x, y = self.x, self.y
        _, pcov1 = curve_fit(
            func, x, y, sigma=2, absolute_sigma=absolute_sigma
        )
    ...
    def test_broadcast_y(self):
        ...
        for method in ["lm", "trf", "dogbox"]:
            ...
            popt1, pcov1 = curve_fit(
                fit_func, xdata=xdata, ydata=0, method=method
            )
