...
class BaseMixin:
    ...
    def test_dense_bounds(self):
        ...
        for lsq_solver in self.lsq_solvers:
            res = lsq_linear(
                A, b, (lb, ub), method=self.method, lsq_solver=lsq_solver
            )
            assert_allclose(res.x, lstsq(A, b, rcond=-1)[0])
        ...
        for lsq_solver in self.lsq_solvers:
            ...
            assert_allclose(
                res.x, np.array([0.0, -4.084174437334673]), atol=1e-6
            )
            assert_allclose(res.unbounded_sol[0], unbounded_sol)
        ...
        for lsq_solver in self.lsq_solvers:
            ...
            assert_allclose(
                res.x, np.array([0.448427311733504, 0]), atol=1e-15
            )
            assert_allclose(res.unbounded_sol[0], unbounded_sol)
    ...
    def test_convergence_small_matrix(self):
        A = np.array(
            [[49.0, 41.0, -32.0], [-19.0, -32.0, -8.0], [-13.0, 10.0, 69.0]]
        )
