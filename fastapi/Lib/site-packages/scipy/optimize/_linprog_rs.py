...
def _phase_one(
    ...
    for basis_column in basis[basis >= n]:
        ...
        try:
            ...
            index = np.argmax(
                basis_finder[:, :n][pertinent_row, eligible_columns]
            )
...
def _get_more_basis_columns(A, basis):
    ...
    if basis.size > 0 and np.linalg.matrix_rank(B[:, : len(basis)]) < len(
        basis
    ):
...
def _generate_auxiliary_problem(A, b, x0, tol):
    ...
    arows = nonzero_constraints[
        np.logical_not(np.isin(nonzero_constraints, rows))
    ]
    n_aux = len(arows)
...
def _select_singleton_columns(A, b):
    ...
    unique_row_indices, first_columns = np.unique(
        row_indices, return_index=True
    )
