    ...
    while i <= n_corrections:
        ...
        if i == 1:
            if ip:  # if the correction is to get "initial point"
                # Reference [4] Eq. 8.23
                rhatxs = (
                    (1 - alpha) * gamma * mu - x * z - alpha**2 * d_x * d_z
                )
                rhattk = (
                    (1 - alpha) * gamma * mu
                    - tau * kappa
        ...
        while not solved:
            try:
                ...
                # [4] Equation 8.29
                u, v = _sym_solve(
                    Dinv, A, rhatd - (1 / x) * rhatxs, rhatp, solve
                )
    ...
    while go:
        ...
        try:
            ...
            if ip:  # initial point
                ...
                # [4] Section 4.3
                alpha = _get_step(
                    x, d_x, z, d_z, tau, d_tau, kappa, d_kappa, alpha0
                )
                # [4] Equation 8.9
                x, y, z, tau, kappa = _do_step(
                    x, y, z, tau, kappa, d_x, d_y, d_z, d_tau, d_kappa, alpha
...
def _linprog_ip(
    ...
    **unknown_options
    ...
    if sparse and lstsq:
        warn(
            "Option combination 'sparse':True and 'lstsq':True "
            "is not recommended.",
            OptimizeWarning,
            stacklevel=3,
        )
