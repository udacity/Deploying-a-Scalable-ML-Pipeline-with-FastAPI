...
def approx_derivative(
    ...
    if as_linear_operator and not (
        np.all(np.isinf(lb)) and np.all(np.isinf(ub))
    ):
        raise ValueError(
    ...
    def fun_wrapped(x):
        ...
        if f.ndim > 1:
            raise RuntimeError(
                "`fun` return value has " "more than 1 dimension."
            )
        return f
    ...
    if as_linear_operator:
        ...
        return _linear_operator_difference(
            fun_wrapped, x0, f0, rel_step, method
        )
    else:
        ...
        if sparsity is None:
            return _dense_difference(
                fun_wrapped, x0, f0, h, use_one_sided, method
            )
        else:
...
def _sparse_difference(
    fun, x0, f0, h, use_one_sided, structure, groups, method
):
    m = f0.size
    n = x0.size
    row_indices = []
...
def check_derivative(
    ...
    if issparse(J_to_test):
        ...
        return np.max(
            np.abs(abs_err_data) / np.maximum(1, np.abs(J_diff_data))
        )
    else:
        J_diff = approx_derivative(
            fun, x0, bounds=bounds, args=args, kwargs=kwargs
        )
