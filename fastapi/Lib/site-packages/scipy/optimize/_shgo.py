...
class SHGO:
    def __init__(
        ...
        if isinstance(sampling_method, str) and sampling_method not in methods:
            raise ValueError(
                (
                    "Unknown sampling_method specified." " Valid methods: {}"
                ).format(", ".join(methods))
            )
            ...
            if sampling_method in ["halton", "sobol"]:
                if sampling_method == "sobol":
                    ...
                    self.qmc_engine = qmc.Sobol(
                        d=self.dim, scramble=False, seed=0
                    )
                else:
                    self.sampling_method = "halton"
                    self.qmc_engine = qmc.Halton(
                        d=self.dim, scramble=True, seed=0
                    )
    ...
    def init_options(self, options):
        ...
        for opt in ["jac", "hess", "hessp"]:
            if opt in self.minimizer_kwargs["options"]:
                self.minimizer_kwargs[opt] = self.minimizer_kwargs[
                    "options"
                ].pop(opt)
    ...
    def finite_ev(self):
        ...
        if self.disp:
            logging.info(
                f"Sampling evaluations done = {self.n_sampled} "
                f"/ {self.maxev}"
            )
    ...
    def finite_time(self):
        if self.disp:
            logging.info(
                f"Time elapsed = {time.time() - self.init} "
                f"/ {self.maxtime}"
            )
    ...
    def minimize(self, x_min, ind=None):
        ...
        if self.LMC[x_min].lres is not None:
            logging.info(
                f"Found self.LMC[x_min].lres = " f"{self.LMC[x_min].lres}"
            )
