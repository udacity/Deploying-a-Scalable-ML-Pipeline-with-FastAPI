...
def check_fpu_mode(request):
    ...
    if old_mode != new_mode:
        warnings.warn(
            f"FPU mode changed from {old_mode:#x} to {new_mode:#x} during "
            "the test",
...
if SCIPY_ARRAY_API and isinstance(SCIPY_ARRAY_API, str):
    ...
    try:
        ...
        if _pep440.parse(array_api_strict.__version__) < _pep440.Version(
            "2.0"
        ):
...
if HAVE_SCPDT:
    ...
    def warnings_errors_and_rng(test=None):
        ...
        dct = {
            ...
            # tutorials
            "linalg.rst": dict(
                message="the matrix subclass is not",
            ...
            with warnings.catch_warnings():
                if test and test.name in known_warnings:
                    warnings.filterwarnings(
                        "ignore", **known_warnings[test.name]
                    )
                    yield
