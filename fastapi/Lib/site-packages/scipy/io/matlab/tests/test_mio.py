...
case_table5.append(
    {
        ...
        "expected": {
            "test3dmatrix": np.transpose(
                np.reshape(list(range(1, 25)), (4, 3, 2))
            )
        },
    }
)
...
def _whos_check_case(name, files, case, classes):
    for file_name in files:
        ...
        expected_whos.sort()
        assert_equal(
            whos, expected_whos, f"{label}: {whos!r} != {expected_whos!r}"
        )
...
def test_read_both_endian():
    ...
    for fname in ("big_endian.mat", "little_endian.mat"):
        ...
        fp.close()
        assert_array_equal(
            d["strings"], np.array([["hello"], ["world"]], dtype=object)
        )
        assert_array_equal(
...
def test_write_opposite_endian():
    ...
    savemat(
        stream,
        {
            "floats": float_arr.byteswap().view(
                float_arr.dtype.newbyteorder()
            ),
...
def test_bad_utf8():
    ...
    res = loadmat(filename)
    assert_equal(
        res["bad_string"], b"\x80 am broken".decode("utf8", "replace")
    )
