...
def to_writeable(source):
    ...
    if (
        narr.dtype.type in (object, np.object_)
        and narr.shape == ()
        and narr == source
    ):
...
class VarWriter5:
    ...
    def write_header(
        self, shape, mclass, is_complex=False, is_logical=False, nzmax=0
    ):
    ...
    def update_matrix_tag(self, start_pos):
        ...
        if byte_count >= 2**32:
            raise MatWriteError(
                "Matrix too large to save with Matlab " "5 format"
            )
    ...
    def _write_items(self, arr):
        ...
        self.write_element(
            np.array(fieldnames, dtype="S%d" % (length)), mdtype=miINT8
        )
        A = np.atleast_2d(arr).flatten("F")
        for el in A:
            for f in fieldnames:
...
class MatFile5Writer:
    ...
    def write_file_header(self):
        ...
        hdr["description"] = (
            f"MATLAB 5.0 MAT-file Platform: {os.name}, "
            f"Created on: {time.asctime()}"
        )
        hdr["version"] = 0x0100
        hdr["endian_test"] = np.ndarray(
            shape=(), dtype="S2", buffer=np.uint16(0x4D49)
        )
