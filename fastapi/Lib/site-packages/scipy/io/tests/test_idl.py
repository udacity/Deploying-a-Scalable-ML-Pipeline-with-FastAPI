...
class TestIdict:
    ...
    def test_idict(self):
        ...
        s = readsav(
            path.join(DATA_PATH, "scalar_byte.sav"),
            idict=custom_dict,
            verbose=False,
        )
...
class TestScalars:
    ...
    def test_complex32(self):
        s = readsav(
            path.join(DATA_PATH, "scalar_complex32.sav"), verbose=False
        )
    ...
    def test_complex64(self):
        s = readsav(
            path.join(DATA_PATH, "scalar_complex64.sav"), verbose=False
        )
        assert_identical(
            s.c64,
...
class TestCompressed(TestScalars):
    ...
    def test_compressed(self):
        s = readsav(
            path.join(DATA_PATH, "various_compressed.sav"), verbose=False
        )
...
class TestArrayDimensions:
    ...
    def test_1d(self):
        s = readsav(
            path.join(DATA_PATH, "array_float32_1d.sav"), verbose=False
        )
        assert_equal(s.array1d.shape, (123,))

    def test_2d(self):
        s = readsav(
            path.join(DATA_PATH, "array_float32_2d.sav"), verbose=False
        )
        assert_equal(s.array2d.shape, (22, 12))

    def test_3d(self):
        s = readsav(
            path.join(DATA_PATH, "array_float32_3d.sav"), verbose=False
        )
        assert_equal(s.array3d.shape, (11, 22, 12))

    def test_4d(self):
        s = readsav(
            path.join(DATA_PATH, "array_float32_4d.sav"), verbose=False
        )
        assert_equal(s.array4d.shape, (4, 5, 8, 7))

    def test_5d(self):
        s = readsav(
            path.join(DATA_PATH, "array_float32_5d.sav"), verbose=False
        )
        assert_equal(s.array5d.shape, (4, 3, 4, 6, 5))

    def test_6d(self):
        s = readsav(
            path.join(DATA_PATH, "array_float32_6d.sav"), verbose=False
        )
        assert_equal(s.array6d.shape, (3, 6, 4, 5, 3, 4))

    def test_7d(self):
        s = readsav(
            path.join(DATA_PATH, "array_float32_7d.sav"), verbose=False
        )
        assert_equal(s.array7d.shape, (2, 1, 2, 3, 4, 3, 2))

    def test_8d(self):
        s = readsav(
            path.join(DATA_PATH, "array_float32_8d.sav"), verbose=False
        )
...
class TestStructures:
    ...
    def test_scalars_replicated(self):
        s = readsav(
            path.join(DATA_PATH, "struct_scalars_replicated.sav"),
            verbose=False,
        )
        ...
        assert_identical(
            s.scalars_rep.f, np.repeat(np.complex64(-1.0 + 3j), 5)
        )

    def test_scalars_replicated_3d(self):
        s = readsav(
            path.join(DATA_PATH, "struct_scalars_replicated_3d.sav"),
            verbose=False,
        )
        assert_identical(
            s.scalars_rep.a, np.repeat(np.int16(1), 24).reshape(4, 3, 2)
        )
        assert_identical(
            s.scalars_rep.b, np.repeat(np.int32(2), 24).reshape(4, 3, 2)
        )
        assert_identical(
    ...
    def test_arrays_replicated_3d(self):
        s = readsav(
            path.join(DATA_PATH, "struct_arrays_replicated_3d.sav"),
            verbose=False,
        )
    ...
    def test_arrays_corrupt_idl80(self):
        ...
        with suppress_warnings() as sup:
            ...
            s = readsav(
                path.join(DATA_PATH, "struct_arrays_byte_idl80.sav"),
                verbose=False,
            )
...
class TestPointerArray:
    ...
    def test_8d(self):
        ...
        assert_(
            np.all(vect_id(s.array8d) == id(s.array8d[0, 0, 0, 0, 0, 0, 0, 0]))
        )
...
class TestPointerStructures:
    ...
    def test_scalars(self):
        ...
        assert_identical(
            s.pointers.g, np.array(np.float32(4.0), dtype=np.object_)
        )
        assert_identical(
            s.pointers.h, np.array(np.float32(4.0), dtype=np.object_)
        )
        assert_(id(s.pointers.g[0]) == id(s.pointers.h[0]))

    def test_pointers_replicated(self):
        s = readsav(
            path.join(DATA_PATH, "struct_pointers_replicated.sav"),
            verbose=False,
        )
        assert_identical(
    ...
    def test_pointers_replicated_3d(self):
        s = readsav(
            path.join(DATA_PATH, "struct_pointers_replicated_3d.sav"),
            verbose=False,
        )
        s_expect = (
            np.repeat(np.float32(4.0), 24).reshape(4, 3, 2).astype(np.object_)
        )
    ...
    def test_arrays_replicated(self):
        s = readsav(
            path.join(DATA_PATH, "struct_pointer_arrays_replicated.sav"),
            verbose=False,
        )
...
class TestTags:
    ...
    def test_description(self):
        s = readsav(
            path.join(DATA_PATH, "scalar_byte_descr.sav"), verbose=False
        )
        assert_identical(s.i8u, np.uint8(234))
