...
def _read_body_array(cursor):
    ...
    vals = np.zeros(
        cursor.header.shape, dtype=_field_to_dtype.get(cursor.header.field)
    )
...
def _read_body_coo(cursor, generalize_symmetry=True):
    ...
    data = np.zeros(
        cursor.header.nnz, dtype=_field_to_dtype.get(cursor.header.field)
    )
...
def _get_read_cursor(source, parallelism=None):
    ...
    if is_path:
        ...
        if path.endswith(".gz"):
            ...
            return (
                _fmm_core.open_read_file(path, parallelism),
                ret_stream_to_close,
            )
    ...
    if hasattr(source, "read"):
        ...
        return (
            _fmm_core.open_read_stream(source, parallelism),
            ret_stream_to_close,
        )
    else:
...
def _get_write_cursor(
    ...
    try:
        ...
        # It's a file path
        return _fmm_core.open_write_file(
            str(target), h, parallelism, precision
        )
    except TypeError:
        pass
...
def mmwrite(
    target, a, comment=None, field=None, precision=None, symmetry="AUTO"
):
    r"""
    ...
    if symmetry == "AUTO":
        if ALWAYS_FIND_SYMMETRY or (
            hasattr(a, "shape") and max(a.shape) < 100
        ):
            symmetry = None
        else:
            symmetry = "general"
