...
def kmeans(
    obs, k_or_guess, iter=20, thresh=1e-5, check_finite=True, *, seed=None
):
    """
    ...
    if size(guess) != 1:
        if size(guess) < 1:
            raise ValueError(
                "Asked for 0 clusters. Initial book was %s" % guess
            )
...
def kmeans2(
    ...
    *,
    ...
    if int(iter) < 1:
        raise ValueError(
            "Invalid iter (%s), " "must be a positive integer." % iter
        )
    try:
    ...
    if minit == "matrix" or size(code_book) > 1:
        ...
        if nc < 1:
            raise ValueError(
                "Cannot ask kmeans2 for %d clusters"
