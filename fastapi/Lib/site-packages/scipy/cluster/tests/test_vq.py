...
class TestVq:
    ...
    def test_py_vq(self, xp):
        ...
        xp_assert_equal(
            label1, xp.asarray(LABEL1, dtype=xp.int64), check_dtype=False
        )
    ...
    @skip_xp_backends(
        np_only=True, reasons=["`_vq` only supports NumPy backend"]
    )
    def test_vq(self, xp):
        initc = np.concatenate([[X[0]], [X[1]], [X[2]]])
    ...
    def test_vq_large_nfeat(self, xp):
        ...
        xp_assert_equal(
            codes1, xp.asarray(codes0, dtype=xp.int64), check_dtype=False
        )
...
class TestKMean:
    ...
    def test_kmeans_large_thres(self, xp):
        ...
        xp_assert_close(
            res[1], xp.asarray(2.3999999999999999, dtype=xp.float64)[()]
        )
    ...
    def test_kmeans2_kpp_high_dim(self, xp):
        ...
        data = np.vstack(
            [
                ...
                np.random.multivariate_normal(
                    centers[1], np.eye(n_dim), size=size
                ),
            ]
        )
    ...
    def test_kmeans_diff_convergence(self, xp):
        ...
        xp_assert_close(
            res[1], xp.asarray(1.0666666666666667, dtype=xp.float64)[()]
        )
