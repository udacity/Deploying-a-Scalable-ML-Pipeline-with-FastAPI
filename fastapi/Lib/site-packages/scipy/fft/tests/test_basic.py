...
class TestFFT:
    ...
    @skip_xp_backends(
        np_only=True, reasons=["significant overhead for some backends"]
    )
        ...
        for n in [size(x), 2 * size(x)]:
            for norm in [None, "backward", "ortho", "forward"]:
                xp_assert_close(
                    ...
                    fft.fft(
                        xp.asarray(x, dtype=xp.complex128), n=n, norm=norm
                    )[: (n // 2 + 1)],
                )
            xp_assert_close(
    ...
    @pytest.mark.parametrize(
        "op", [fft.fftn, fft.ifftn, fft.rfftn, fft.irfftn]
    )
    ...
    def test_axes_subset_with_shape_standard(self, op, xp):
        ...
        for a in axes:
            ...
            op_tr = op(
                xp_test.permute_dims(x, axes=a), s=shape[:2], axes=(0, 1)
            )
