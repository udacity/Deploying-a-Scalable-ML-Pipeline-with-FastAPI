...
def test_identity_1d(
    forward, backward, type, n, axis, norm, orthogonalize, xp
):
...
def test_identity_1d_overwrite(
    forward, backward, type, dtype, axis, norm, overwrite_x
):
    # Test the identity f^-1(f(x)) == x
...
def test_identity_nd(
    forward, backward, type, shape, axes, norm, orthogonalize, xp
):
    # Test the identity f^-1(f(x)) == x
...
@pytest.mark.parametrize(
    "func", [dct, idct, dctn, idctn, dst, idst, dstn, idstn]
)
