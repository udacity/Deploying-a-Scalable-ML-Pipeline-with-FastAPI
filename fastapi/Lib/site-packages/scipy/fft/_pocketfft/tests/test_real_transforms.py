...
fftpack_test_dir = join(
    dirname(__file__), "..", "..", "..", "fftpack", "tests"
)
...
class TestDCT:
    ...
    def test_axis(self, rdt, type, size):
        ...
        for j in range(nt):
            assert_array_almost_equal(
                y[:, j], dct(x[:, j], type=type), decimal=dec
            )
...
@pytest.mark.parametrize(
    "func", [dct, dctn, idct, idctn, dst, dstn, idst, idstn]
)
