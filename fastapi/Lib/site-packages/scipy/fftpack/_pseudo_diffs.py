...
def sc_diff(x, a, b, period=None, _cache=_cache):
    ...
    tmp = asarray(x)
    if iscomplexobj(tmp):
        return sc_diff(tmp.real, a, b, period) + 1j * sc_diff(
            tmp.imag, a, b, period
        )
    if period is not None:
        a = a * 2 * pi / period
...
def ss_diff(x, a, b, period=None, _cache=_cache):
    ...
    tmp = asarray(x)
    if iscomplexobj(tmp):
        return ss_diff(tmp.real, a, b, period) + 1j * ss_diff(
            tmp.imag, a, b, period
        )
    if period is not None:
        a = a * 2 * pi / period
...
def cc_diff(x, a, b, period=None, _cache=_cache):
    ...
    tmp = asarray(x)
    if iscomplexobj(tmp):
        return cc_diff(tmp.real, a, b, period) + 1j * cc_diff(
            tmp.imag, a, b, period
        )
    if period is not None:
        a = a * 2 * pi / period
...
def shift(x, a, period=None, _cache=_cache):
    ...
    overwrite_x = _datacopied(tmp, x)
    return convolve.convolve_z(
        tmp, omega_real, omega_imag, overwrite_x=overwrite_x
    )
