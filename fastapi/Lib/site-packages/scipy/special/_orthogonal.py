...
class orthopoly1d(np.poly1d):
    ...
    def __init__(
        ...
        equiv_weights = [
            weights[k] / wfunc(roots[k]) for k in range(len(roots))
        ]
        mu = sqrt(hn)
        if monic:
            evf = eval_func
...
def roots_jacobi(n, alpha, beta, mu=False):
    ...
    def bn_func(k):
        return (
            ...
            * np.where(
                k == 1, 1.0, np.sqrt(k * (k + a + b) / (2.0 * k + a + b - 1))
            )
        )
...
def roots_hermite(n, mu=False):
    ...
    if n <= 150:
        ...
        return _gen_roots_and_weights(
            m, mu0, an_func, bn_func, f, df, True, mu
        )
    else:
...
def roots_gegenbauer(n, alpha, mu=False):
    ...
    if alpha <= 170:
        mu0 = (np.sqrt(np.pi) * _ufuncs.gamma(alpha + 0.5)) / _ufuncs.gamma(
            alpha + 1
        )
    else:
...
def gegenbauer(n, alpha, monic=False):
    ...
    base._scale(factor)
    base.__dict__["_eval_func"] = lambda x: _ufuncs.eval_gegenbauer(
        float(n), alpha, x
    )
    return base
...
def roots_legendre(n, mu=False):
    ...
    def df(n, x):
        return (
            -n * x * _ufuncs.eval_legendre(n, x)
            + n * _ufuncs.eval_legendre(n - 1, x)
        ) / (1 - x**2)
