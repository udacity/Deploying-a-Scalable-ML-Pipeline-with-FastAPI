...
def test_hyp0f1_gh5764():
    ...
    FuncData(
        lambda v, z: sc.hyp0f1(v.real, z), dataset, (0, 1), 2, rtol=1e-13
    ).check()
...
def test_erf_complex():
    ...
    try:
        ...
        x2, y2 = np.meshgrid(
            np.logspace(-80, 0.8, 31), np.logspace(-80, 0.8, 11)
        )
        points = (
            np.r_[x1.ravel(), x2.ravel()] + 1j * np.r_[y1.ravel(), y2.ravel()]
        )
...
def test_digamma_roots():
    ...
    dx = np.r_[
        -0.24, -np.logspace(-1, -15, 10), 0, np.logspace(-15, -1, 10), 0.24
    ]
    dy = dx.copy()
    dx, dy = np.meshgrid(dx, dy)
    dz = dx + 1j * dy
...
def test_wrightomega_branch():
    ...
    dataset = np.asarray(
        [(z0, complex(_mpmath_wrightomega(z0, 25))) for z0 in z]
    )
...
class TestSystematic:
    ...
    def test_airyai(self):
        ...
        assert_mpmath_equal(
            lambda z: sc.airy(z)[0], mpmath.airyai, [Arg(-1e3, 1e3)]
        )

    def test_airyai_complex(self):
        assert_mpmath_equal(
            lambda z: sc.airy(z)[0], mpmath.airyai, [ComplexArg()]
        )
    ...
    @pytest.mark.xfail(
        run=False, reason="some cases in hyp2f1 not fully accurate"
    )
    ...
    def test_cospi(self):
        eps = np.finfo(float).eps
        assert_mpmath_equal(
            _cospi, mpmath.cospi, [Arg()], nan_ok=False, rtol=2 * eps
        )
    ...
    def test_digamma_complex(self):
        ...
        def param_filter(z):
            return np.where(
                (z.real < 0) & (np.abs(z.imag) < 1.12), False, True
            )
    ...
    def test_e1_complex(self):
        ...
        assert_mpmath_equal(
            ...
            (
                ...
                + np.r_[0, np.logspace(-3, 2, 61), -np.logspace(-3, 2, 11)]
                * 1j
            ).ravel(),
            rtol=1e-11,
        )
    ...
    def test_ellipeinc(self):
        assert_mpmath_equal(
            sc.ellipeinc, mpmath.ellipe, [Arg(-1e3, 1e3), Arg(b=1.0)]
        )
    ...
    def test_ellipf(self):
        assert_mpmath_equal(
            sc.ellipkinc, mpmath.ellipf, [Arg(-1e3, 1e3), Arg()]
        )
    ...
    def test_erf_complex(self):
        assert_mpmath_equal(
            sc.erf, lambda z: mpmath.erf(z), [ComplexArg()], n=200
        )
    ...
    @pytest.mark.xfail(
        run=False, reason="issues at intermediately large orders"
    )
    ...
    def test_legenp_complex_2(self):
        ...
        def legenp(n, m, z):
            if abs(z) < 1e-15:
                # mpmath has bad performance here
                return np.nan
            return exception_to_nan(mpmath.legenp)(
                int(n.real), int(m.real), z, type=2
            )
    ...
    def test_legenp_complex_3(self):
        ...
        def legenp(n, m, z):
            if abs(z) < 1e-15:
                ...
                return np.nan
            return exception_to_nan(mpmath.legenp)(
                int(n.real), int(m.real), z, type=3
            )
    ...
    @pytest.mark.xfail(
        run=False, reason="apparently picks wrong function at |z| > 1"
    )
    ...
    def test_legenq_complex(self):
        ...
        def legenq(n, m, z):
            if abs(z) < 1e-15:
                # mpmath has bad performance here
                return np.nan
            return exception_to_nan(mpmath.legenq)(
                int(n.real), int(m.real), z, type=2
            )
    ...
    def test_pcfv(self):
        ...
        assert_mpmath_equal(
            pcfv,
            lambda v, x: time_limited()(exception_to_nan(mpmath.pcfv))(
                v, x, **HYPERKW
            ),
            [Arg(), Arg()],
            n=1000,
