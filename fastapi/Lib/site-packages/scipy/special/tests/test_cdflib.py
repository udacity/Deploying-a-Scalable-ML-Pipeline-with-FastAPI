...
class _CDFData:
    ...
    def get_param_filter(self):
        ...
        filters = []
        for rtol, atol, spec in zip(
            self.endpt_rtol, self.endpt_atol, self.argspec
        ):
...
class TestCDFlib:
    ...
    def test_nrdtrimn(self):
        _assert_inverts(
            ...
            [
                ProbArg(),  # CDF value p
                Arg(
                    0.1, np.inf, inclusive_a=False, inclusive_b=False
                ),  # sigma
                Arg(-1e10, 1e10),
            ],  # x
            rtol=1e-5,
    ...
    def test_stdtridf(self):
        _assert_inverts(
            sp.stdtridf, _student_t_cdf, 0, [ProbArg(), Arg()], rtol=1e-7
        )
...
def test_nonfinite(func, numargs):
    ...
    args_choices = [
        (float(x), np.nan, np.inf, -np.inf) for x in rng.random(numargs)
    ]
