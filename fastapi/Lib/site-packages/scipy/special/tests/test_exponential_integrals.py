...
class TestExpi:
    ...
    @pytest.mark.parametrize(
        "result",
        [sc.expi(complex(-1, 0)), sc.expi(complex(-1, -0.0)), sc.expi(-1)],
    )
    ...
    def test_near_branch_cut(self):
        ...
        assert_allclose(
            lim_from_above.real, lim_from_below.real, atol=0, rtol=1e-15
        )
        assert_allclose(
            lim_from_above.imag, -lim_from_below.imag, atol=0, rtol=1e-15
        )
