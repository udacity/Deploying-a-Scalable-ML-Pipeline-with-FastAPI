...
def _nonneg_int_or_fail(n, var_name, strict=True):
    try:
        ...
        raise err.__class__(
            f"{var_name} must be a non-negative integer"
        ) from err
    return n
...
def mathieu_even_coef(m, q):
    ...
    if km > 251:
        warnings.warn(
            "Too many predicted coefficients.", RuntimeWarning, stacklevel=2
        )
    kd = 1
    m = int(floor(m))
    if m % 2:
...
def lpmn(m, n, z):
    ...
    if z.ndim == 0:
        ...
        _lpmn(
            ...
            m_signbit,
            out=(
                np.moveaxis(p, (0, 1), (-2, -1)),
                np.moveaxis(pd, (0, 1), (-2, -1)),
            ),
...
def perm(N, k, exact=False):
    ...
    if exact:
        ...
        if non_integral:
            raise ValueError(
                "Non-integer `N` and `k` with `exact=True` is not "
                "supported."
            )
        val = 1
...
def factorial(n, exact=False):
    ...
    if np.ndim(n) == 0 and not isinstance(n, np.ndarray):
        ...
        if n is None or np.isnan(n):
            ...
            np.issubdtype(type(n), np.integer)
    ...
    if exact and not np.issubdtype(n.dtype, np.integer):
        msg = (
            "factorial with `exact=True` does not "
...
def stirling2(N, K, *, exact=False):
    ...
    nk_pairs = list(
        set(
            [(n.take(0), k.take(0)) for n, k in np.nditer([N, K], ["refs_ok"])]
        )
    )
    heapify(nk_pairs)
