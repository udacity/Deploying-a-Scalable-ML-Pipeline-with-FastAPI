...
def series_small_a():
    ...
    expression = Sum(
        x**k / factorial(k) / gamma(a * k + b), (k, 0, S.Infinity)
    )
    ...
    for n in range(0, order + 1):
        ...
        x_part = term.subs(polygamma(0, b), 1).replace(
            polygamma, lambda *args: 0
        )
...
def series_small_a_small_b():
    ...
    for i in range(len(C)):
        ...
        s += f"\nC[{i}] = "
        s += str(
            C[i].subs({M_EG: EulerGamma, M_PI: pi, M_Z3: zeta(3)}).evalf(17)
        )
...
def asymptotic_series():
    ...
    class coef_C(sympy.Function):
        ...
        def eval(cls, m, rho, beta):
            ...
            v = symbols("v")
            expression = (1 - v) ** (-beta) * g(2 * m, rho, v) ** (
                -m - Rational(1, 2)
            )
