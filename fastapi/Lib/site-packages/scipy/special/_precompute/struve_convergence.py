...
def do_plot(is_h=True):
    ...
    plt.hold(1)
    plt.contourf(
        vs, zs, np.log10(err_p).T, levels=levels, colors=["r", "r"], alpha=0.1
    )
    plt.contourf(
        vs, zs, np.log10(err_a).T, levels=levels, colors=["b", "b"], alpha=0.1
    )
    plt.contourf(
        vs, zs, np.log10(err_b).T, levels=levels, colors=["g", "g"], alpha=0.1
    )
