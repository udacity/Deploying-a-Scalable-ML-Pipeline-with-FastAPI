...
class TestODR:
    ...
    def test_explicit(self):
        ...
        assert_array_almost_equal(
            ...
            np.array(
                [
                    1.2646548050648876e03,
                    -5.4018409956678255e01,
        ...
        assert_array_almost_equal(
            out.sd_beta,
            np.array(
                [1.0349270280543437, 1.583997785262061, 0.0063321988657267]
            ),
        )
        assert_array_almost_equal(
            out.cov_beta,
    ...
    def multi_fcn(self, B, x):
        ...
        r = (B[0] - B[1]) * np.power(
            np.sqrt(
                np.power(1.0 + omega * ctheta, 2) + np.power(omega * stheta, 2)
            ),
            -B[4],
        )
        ret = np.vstack(
            [B[1] + r * np.cos(B[4] * phi), r * np.sin(B[4] * phi)]
        )
        return ret
    ...
    def test_multi(self):
        multi_mod = Model(
            self.multi_fcn,
            meta=dict(
                name="Sample Multi-Response Model", ref="ODRPACK UG, pg. 56"
            ),
        )
        ...
        multi_dat = Data(
            multi_x, multi_y, wd=1e-4 / np.power(multi_x, 2), we=multi_we
        )
        multi_odr = ODR(
            multi_dat,
            multi_mod,
    ...
    def test_pearson(self):
        ...
        p_sx = np.array(
            [0.03, 0.03, 0.04, 0.035, 0.07, 0.11, 0.13, 0.22, 0.74, 1.0]
        )
        p_sy = np.array(
            [1.0, 0.74, 0.5, 0.35, 0.22, 0.22, 0.12, 0.12, 0.1, 0.04]
        )
    ...
    def lorentz(self, beta, x):
        return (
            ...
            * beta[2]
            / np.sqrt(
                np.power(x * x - beta[2] * beta[2], 2.0)
                + np.power(beta[1] * x, 2.0)
            )
        )
    ...
    def test_lorentz(self):
        ...
        assert_array_almost_equal(
            ...
            np.array(
                [
                    1.4306780846149925e03,
                    1.3390509034538309e-01,
                    3.7798193600109009e00,
                ]
            ),
        )
        assert_array_almost_equal(
            ...
            np.array(
                [
                    7.3621186811330963e-01,
                    3.5068899941471650e-04,
    ...
    def test_ifixx(self):
        ...
        fix = np.vstack(
            (np.zeros_like(x1, dtype=int), np.ones_like(x2, dtype=int))
        )
        data = Data(np.vstack((x1, x2)), y=1, fix=fix)
        model = Model(
            lambda beta, x: x[1, :] - beta[0] * x[0, :] ** 2.0, implicit=True
        )
