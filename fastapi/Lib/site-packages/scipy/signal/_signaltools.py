    ...
    if len(a) == 1:
        ...
        if zi is not None:
            ...
            if zi.shape != expected_shape:
                ...
                zi = np.lib.stride_tricks.as_strided(
                    zi, expected_shape, strides
                )
            inputs.append(zi)
...
def _compute_residues(poles, multiplicity, numerator):
    ...
    for pole, mult, factor in zip(poles, multiplicity, denominator_factors):
        if mult == 1:
            residues.append(
                np.polyval(numerator, pole) / np.polyval(factor, pole)
            )
        else:
...
def residuez(b, a, tol=1e-3, rtype="avg"):
    ...
    if a.size == 0:
        ...
        raise ValueError(
            "First coefficient of determinant `a` must be " "non-zero."
        )
...
def resample_poly(
    ...
    h = np.concatenate(
        (
            np.zeros(n_pre_pad, dtype=h.dtype),
            h,
...
def detrend(
    ...
    if type in ["constant", "c"]:
        ...
        if np.any(bp > N):
            raise ValueError(
                "Breakpoints must be less than length "
...
def filtfilt(
    ...
    # method == "pad"
    edge, ext = _validate_pad(
        padtype, padlen, x, axis, ntaps=max(len(a), len(b))
    )
...
def decimate(x, q, n=None, ftype="iir", axis=-1, zero_phase=True):
    ...
    result_type = x.dtype
    if (
        not np.issubdtype(result_type, np.inexact)
