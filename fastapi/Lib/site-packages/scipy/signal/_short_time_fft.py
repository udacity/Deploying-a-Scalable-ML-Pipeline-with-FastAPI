...
class ShortTimeFFT:
    ...
    def stft_detrend(
        ...
        *,
        ...
        if isinstance(detr, str):
            ...
            raise ValueError(
                f"Parameter {detr=} is not a str, function or " + "None!"
            )
        n = x.shape[axis]
    ...
    def istft(
        ...
        *,
        ...
        if S.shape[f_axis] != self.f_pts:
            raise ValueError(
                f"{S.shape[f_axis]=} must be equal to "
    ...
    def _fft_func(self, x: np.ndarray) -> np.ndarray:
        ...
        if self.fft_mode == "centered":
            return fft_lib.fftshift(
                fft_lib.fft(x, self.mfft, axis=-1), axes=-1
            )
    ...
    def _ifft_func(self, X: np.ndarray) -> np.ndarray:
        ...
        if self.fft_mode == "twosided":
            ...
            x = fft_lib.ifft(
                fft_lib.ifftshift(X, axes=-1), n=self.mfft, axis=-1
            )
