...
def freqz(
    b, a=1, worN=512, whole=False, plot=None, fs=2 * pi, include_nyquist=False
):
    """
    ...
    if h is None:  # still need to compute using freqs w
        zm1 = exp(-1j * w)
        h = npp_polyval(zm1, b, tensor=False) / npp_polyval(
            zm1, a, tensor=False
        )
...
def zpk2sos(z, p, k, pairing=None, *, analog=False):
    ...
    *Algorithms*
    ...
    if analog and pairing != "minimal":
        raise ValueError(
            "for analog zpk2sos conversion, " 'pairing must be "minimal"'
        )
...
def normalize(b, a):
    ...
    if num.ndim > 2:
        raise ValueError(
            "Numerator polynomial must be rank-1 or" " rank-2 array."
        )
    if np.all(den == 0):
...
def iirdesign(
    wp, ws, gpass, gstop, analog=False, ftype="ellip", output="ba", fs=None
):
    ...
    try:
        ...
        raise ValueError(
            ("%s does not have order selection. Use " "iirfilter function.")
            % ftype
        ) from e
    ...
    if wp.shape[0] == 2:
        if not (
            (ws[0] < wp[0] and wp[1] < ws[1])
            or (wp[0] < ws[0] and ws[1] < wp[1])
        ):
            raise ValueError(
    ...
    btype = {1: "lowpass", 2: "highpass", 3: "bandstop", 4: "bandpass"}[
        band_type
    ]
...
def iirfilter(
    ...
    try:
        ...
        raise ValueError(
            "'%s' is an invalid bandtype for filter." % btype
        ) from e
    ...
    try:
        ...
        raise ValueError(
            "'%s' is not a valid basic IIR filter." % ftype
        ) from e
    ...
    if typefunc == buttap:
        ...
        if rs is None or rp is None:
            raise ValueError(
                "Both rp and rs must be provided to design an "
                "elliptic filter."
            )
...
def bessel(
    N, Wn, btype="low", analog=False, output="ba", norm="phase", fs=None
):
    """
...
def buttord(wp, ws, gpass, gstop, analog=False, fs=None):
    ...
    try:
        ...
        W0 = 1.0
        warnings.warn(
            "Order is zero...check input parameters.",
            RuntimeWarning,
            stacklevel=2,
        )
    ...
    if filter_type == 1:  # low
        ...
        WN = np.empty(2, float)
        discr = sqrt(
            (passb[1] - passb[0]) ** 2 + 4 * W0**2 * passb[0] * passb[1]
        )
...
def cheb2ord(wp, ws, gpass, gstop, analog=False, fs=None):
    ...
    if filter_type == 1:
        ...
        nat[0] = new_freq / 2.0 * (passb[0] - passb[1]) + sqrt(
            new_freq**2 * (passb[1] - passb[0]) ** 2 / 4.0
            + passb[1] * passb[0]
        )
        ...
        nat[0] = 1.0 / (2.0 * new_freq) * (passb[0] - passb[1]) + sqrt(
            (passb[1] - passb[0]) ** 2 / (4.0 * new_freq**2)
            + passb[1] * passb[0]
        )
...
def ellipap(N, rp, rs):
    ...
    if N % 2:
        newp = np.compress(
            abs(p.imag)
            > EPSILON * np.sqrt(np.sum(p * np.conjugate(p), axis=0).real),
            p,
            axis=-1,
        )
