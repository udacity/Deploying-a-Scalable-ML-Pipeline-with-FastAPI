...
def test_sg_coeffs_even_window_length():
    ...
    for length in window_lengths:
        m = length // 2
        expected_output = [
            h_p_d_closed_form_1(k, m) for k in range(-m + 1, m + 1)
        ][::-1]
        ...
        expected_output = [
            h_p_d_closed_form_2(k, m) for k in range(-m + 1, m + 1)
        ][::-1]
...
def test_sg_filter_interp_edges():
    ...
    y1 = savgol_filter(
        x, window_length, 3, axis=0, mode="interp", deriv=1, delta=delta
    )
    assert_allclose(y1, dx, atol=1e-12)

    y2 = savgol_filter(
        x, window_length, 3, axis=0, mode="interp", deriv=2, delta=delta
    )
    assert_allclose(y2, d2x, atol=1e-12)
