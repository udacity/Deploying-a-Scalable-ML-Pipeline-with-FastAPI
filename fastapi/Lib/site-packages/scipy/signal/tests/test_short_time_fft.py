...
def test_invalid_initializer_parameters():
    ...
    with pytest.raises(
        ValueError,
        match=r"Parameter win must be 1d, " + r"but win.shape=\(2, 2\)!",
    ):
        ShortTimeFFT(np.ones((2, 2)), hop=4, fs=1)
    with pytest.raises(
        ValueError, match="Parameter win must have " + "finite entries"
    ):
    ...
    with pytest.raises(
        ValueError,
        match=r"dual_win.shape=\(5,\) must equal " + r"win.shape=\(4,\)!",
    ):
    ...
    with pytest.raises(
        ...
        ShortTimeFFT(
            np.ones(3), hop=2, fs=1, dual_win=np.array([np.nan, 2, 3])
        )
...
def test_exceptions_properties_methods():
    ...
    with pytest.raises(
        ValueError,
        match="For scaling is None, " + "fft_mode='onesided2X' is invalid.*",
    ):
    ...
    with pytest.raises(
        ValueError,
        match="Parameter detr=nan is not a str, " + "function or None!",
    ):
        # noinspection PyTypeChecker
    ...
    with pytest.raises(
        ValueError, match="f_axis=0 may not be equal to " + "t_axis=0!"
    ):
        SFT.istft(np.zeros((SFT.f_pts, 2)), t_axis=0, f_axis=0)
    with pytest.raises(
        ValueError,
        match=r"S.shape\[f_axis\]=2 must be equal" + " to self.f_pts=5.*",
    ):
        SFT.istft(np.zeros((2, 2)))
    with pytest.raises(
        ValueError,
        match=r"S.shape\[t_axis\]=1 needs to have" + " at least 2 slices.*",
    ):
    ...
    with pytest.raises(
        ValueError,
        match=r"\(k1=1\) - \(k0=0\) = 1 has to " + "be at least.* length 4!",
    ):
        SFT.istft(np.zeros((SFT.f_pts, 3)), k0=0, k1=1)

    with pytest.raises(
        ValueError,
        match=r"Parameter axes_seq='invalid' " + r"not in \['tf', 'ft'\]!",
    ):
        # noinspection PyTypeChecker
...
def test_exceptions_fft_mode_complex_win(m: FFT_MODE_TYPE):
    ...
    """
    with pytest.raises(
        ValueError, match=f"One-sided spectra, i.e., fft_mode='{m}'.*"
    ):
    ...
    SFT = ShortTimeFFT(
        np.ones(8), hop=4, fs=1, scale_to="psd", fft_mode="onesided"
    )
    with pytest.raises(
        ValueError, match="Complex-valued `x` not allowed for self.*"
    ):
...
def test_dual_win_roundtrip():
    ...
    kw = dict(
        hop=4, fs=1, fft_mode="twosided", mfft=8, scale_to=None, phase_shift=2
    )
...
def test_f(fft_mode: FFT_MODE_TYPE, f):
    ...
    SFT = ShortTimeFFT(
        np.ones(5), hop=4, fs=5, fft_mode=fft_mode, scale_to="psd"
    )
...
def test_impulse_roundtrip(i):
    ...
    assert_allclose(
        Sx1[:, q1_le:], Sx[:, q1_le - Sx1.shape[1] :], err_msg=f"{i=}"
    )

    Sx01 = np.hstack(
        (Sx0[:, :q0_ub], Sx0[:, q0_ub:] + Sx1[:, :q1_le], Sx1[:, q1_le:])
    )
...
def test_asymmetric_window_roundtrip(hop: int):
    ...
    assert_allclose(
        x1,
        x1,
        err_msg="Roundtrip for asymmetric window with " + f" {hop=} failed!",
    )
...
def test_tutorial_stft_legacy_spectrogram():
    ...
    Sz3 = SFT.stft(
        z, p0=0, p1=(N - noverlap) // SFT.hop, k_offset=nperseg // 2
    )
...
def test_roundtrip_windows(window, n: int, nperseg: int, noverlap: int):
    ...
    assert_allclose(
        x32, x32_1, err_msg="Roundtrip for 32 Bit float values failed"
    )
...
def test_roundtrip_complex_window(signal_type):
    ...
    z1 = SFT.istft(Sz, k1=len(z))
    assert_allclose(
        z, z1, err_msg="Roundtrip for complex-valued window failed"
    )
...
def test_stft_padding_roundtrip(
    ...
    x1 = SFT.istft(Sx, k1=N)
    assert_allclose(
        x1, x, err_msg=f"Failed real roundtrip with '{padding}' padding"
    )
    ...
    assert_allclose(
        z1,
        z,
        err_msg="Failed complex roundtrip with " + f" '{padding}' padding",
    )
...
def test_energy_conservation(N_x: int, w_size: int, t_step: int, f_c: float):
    ...
    SFT = ShortTimeFFT(
        window, t_step, fs=1000, fft_mode="onesided2X", scale_to="psd"
    )
