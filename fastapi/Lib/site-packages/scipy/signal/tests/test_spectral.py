...
class TestPeriodogram:
    ...
    @pytest.mark.skipif(
        sys.maxsize <= 2**32, reason="On some 32-bit tolerance issue"
    )
    ...
    def test_window_external(self):
        ...
        assert_raises(
            ValueError, periodogram, x, 10, win_err
...
class TestWelch:
    ...
    def test_real_onesided_odd(self):
        ...
        q = np.array(
            [0.12477455, 0.23430933, 0.17072113, 0.17072113, 0.17072113]
        )
        assert_allclose(p, q, atol=1e-7, rtol=1e-7)
    ...
    def test_real_spectrum(self):
        ...
        assert_allclose(f, np.linspace(0, 0.5, 5))
        q = np.array(
            [0.015625, 0.02864583, 0.04166667, 0.04166667, 0.02083333]
        )
        assert_allclose(p, q, atol=1e-7, rtol=1e-7)
    ...
    def test_detrend_external(self):
        x = np.arange(10, dtype=np.float64) + 0.04
        f, p = welch(
            x, nperseg=10, detrend=lambda seg: signal.detrend(seg, type="l")
        )
        assert_allclose(p, np.zeros_like(p), atol=1e-15)
    ...
    def test_window_long_or_nd(self):
        assert_raises(
            ValueError, welch, np.zeros(4), 1, np.array([1, 1, 1, 1, 1])
        )
        assert_raises(
            ValueError, welch, np.zeros(4), 1, np.arange(6).reshape((2, 3))
        )
    ...
    def test_real_onesided_odd_32(self):
        ...
        assert_allclose(f, np.arange(5.0) / 9.0)
        q = np.array(
            [0.12477458, 0.23430935, 0.17072113, 0.17072116, 0.17072113], "f"
        )
        assert_allclose(p, q, atol=1e-7, rtol=1e-7)
        assert_(p.dtype == q.dtype)
    ...
    def test_average(self):
        ...
        assert_raises(
            ValueError, welch, x, nperseg=8, average="unrecognised-average"
        )
...
class TestCSD:
    ...
    def test_detrend_external(self):
        x = np.arange(10, dtype=np.float64) + 0.04
        f, p = csd(
            x, x, nperseg=10, detrend=lambda seg: signal.detrend(seg, type="l")
        )
        assert_allclose(p, np.zeros_like(p), atol=1e-15)
    ...
    def test_bad_noverlap(self):
        assert_raises(
            ValueError, csd, np.zeros(4), np.ones(4), 1, "hann", 2, 7
        )

    def test_nfft_too_short(self):
        assert_raises(
            ValueError, csd, np.ones(12), np.zeros(12), nfft=3, nperseg=4
        )
    ...
    def test_copied_data(self):
        ...
        _, p_copied = csd(
            x, y, nperseg=8, average="mean", return_onesided=False
        )
        assert_allclose(p_same, p_copied)

        _, p_same = csd(
            x, x, nperseg=8, average="median", return_onesided=False
        )
        _, p_copied = csd(
            x, y, nperseg=8, average="median", return_onesided=False
        )
        assert_allclose(p_same, p_copied)
...
class TestSTFT:
    def test_input_validation(self):
        ...
        with pytest.warns(
            UserWarning,
            match="NOLA condition failed, " + "STFT may not be invertible",
        ):
        ...
        with chk_VE(
            "Unknown value for mode foo, must be one of: "
            + r"\{'psd', 'stft'\}"
        ):
...
class TestSampledSpectralRepresentations:
    ...
    def test_windowed_DFT(self):
        ...
        assert_allclose(
            self.tau * Xw[self.l_a] / c_amp, self.a_ref * self.tau / 2
        )
        ...
        X_ESD = (
            self.tau * self.T * abs(Xw / c_rms) ** 2
        ...
        kw = dict(
            fs=1 / self.T, window=w, detrend=False, return_onesided=False
        )
        ...
        assert_allclose(
            P_mag, abs(Xw / c_amp) ** 2, atol=float_res * max(P_mag)
        )
