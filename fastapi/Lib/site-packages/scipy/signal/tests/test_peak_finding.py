...
def _gen_gaussians_even(sigmas, total_length):
    ...
    center_locs = np.linspace(
        delta, total_length - delta, num=num_peaks
    ).astype(int)
...
class TestRidgeLines:
    ...
    def test_single_pass(self):
        ...
        length = 12
        line = _gen_ridge_line(
            [0, 25], test_matr.shape, length, distances, gaps
        )
...
class TestPeakProminences:
    ...
    def test_wlen(self):
        ...
        for wlen, i in [
            ...
            assert_equal(
                peak_prominences(x, peak, wlen), [3.0 - i, 0 + i, 6 - i]
            )
...
class TestPeakWidths:
    ...
    def test_intersection_rules(self):
        ...
        assert_allclose(
            peak_widths(x, peaks=[5], rel_height=0),
            [(0.0,), (3.0,), (5.0,), (5.0,)],
        )
