...
class TestDLTI:
    ...
    def test_dlsim(self):
        ...
        yout_truth = np.array(
            [[-0.001, -0.00073, 0.039446, 0.0915387, 0.13195948]]
        ).T
    ...
    def test_dlsim_simple2d(self):
        ...
        expected = np.array([lambda1, lambda2]) ** np.arange(float(n)).reshape(
            -1, 1
        )
        assert_array_equal(yout, expected)
        assert_array_equal(xout, expected)
    ...
    def test_more_step_and_impulse(self):
        ...
        imp = np.array([lambda1, lambda2]) ** np.arange(-1, n + 1).reshape(
            -1, 1
        )
        imp[0, :] = 0.0
