...
class TestOAConvolve:
    ...
    @pytest.mark.parametrize(
        "shape_a_0, shape_b_0", gen_oa_shapes([50, 47, 6, 4, 1])
    )
    ...
    def test_1d_noaxes(
        ...
        monkeypatch.setattr(
            signal._signaltools, "fftconvolve", fftconvolve_err
        )
...
class TestAllFreqConvolves:
    ...
    def test_invalid_flags(self, convapproach):
        ...
        with assert_raises(
            ValueError,
            match="axes must be a scalar or " "iterable of integers",
        ):
...
class TestWiener:
    ...
    def test_basic(self):
        g = array(
            [[5, 6, 4, 3], [3, 5, 6, 2], [2, 3, 5, 6], [1, 6, 9, 7]], "d"
        )
        h = array(
            [
    ...
    def test_resample_methods(self, method, ext, padtype):
        ...
        for rate_to in rates_to:
            ...
            if method == "fft":
                ...
                y_resamps = signal.resample_poly(
...
class TestOrderFilt:
    ...
    def test_basic(self):
        assert_array_equal(
            signal.order_filter([1, 2, 3], [1, 0, 1], 1), [2, 3, 2]
        )
...
class _TestLinearFilter:
    ...
    def test_bad_size_zi(self):
        ...
        self.base_bad_size_zi(
            [1, 1, 1], [1, 1], x2, 0, [[0, 1, 2, 3], [4, 5, 6, 7]]
        )
...
def test_lfilter_notimplemented_input():
    ...
    assert_raises(
        NotImplementedError, lfilter, [2, 3], [4, 5], [1, 2, 3, 4, 5]
    )
...
class TestCorrelateComplex:
    ...
    def test_rank3(self, dt):
        ...
        y_r = (correlate(a.real, b.real) + correlate(a.imag, b.imag)).astype(
            dt
        )
    ...
    def test_rank0(self, dt):
        ...
        y_r += 1j * np.array(
            -correlate(a.real, b.imag) + correlate(a.imag, b.real)
        )
...
class TestCorrelate2d:
    ...
    def test_invalid_shapes(self):
        ...
        assert_raises(
            ValueError, signal.correlate2d, *(a, b), **{"mode": "valid"}
        )
        assert_raises(
            ValueError, signal.correlate2d, *(b, a), **{"mode": "valid"}
        )
...
def filtfilt_gust_opt(b, a, x):
    ...
    if warnflag > 0:
        raise RuntimeError(
            "minimization failed in filtfilt_gust_opt: "
            "warnflag=%d" % warnflag
        )
    z0f = opt[:m]
    z0b = opt[m:]
...
class TestDecimate:
    ...
    def _test_phaseshift(self, method, zero_phase):
        ...
        for rate_to in rates_to:
            ...
            assert_allclose(
                np.angle(h_resps.conj() * h_resamps)[subnyq],
                0,
                atol=1e-3,
                rtol=1e-3,
            )
    ...
    def test_complex_iir_dlti(self):
        ...
        # input
        u = np.exp(2j * np.pi * fcentre * t) + 0.5 * np.exp(
            -2j * np.pi * fcentre * t
        )
...
class TestPartialFractionExpansion:
    ...
    def test_compute_factors(self):
        ...
        factors, poly = _compute_factors(
            [1, 2, 3], [3, 2, 1], include_powers=True
        )
    ...
    def test_residue_general(self):
        ...
        r, p, k = residue(
            [3, -1.1, 0.88, -2.396, 1.348], [1, -0.7, -0.14, 0.048]
        )
    ...
    def test_invres(self):
        ...
        b, a = invres(
            [-1, 2, 1j, 3 - 1j, 4, -2], [-1, 2 - 1j, 2 - 1j, 3, 3, 3], []
        )
        assert_almost_equal(
            b,
...
class TestSOSFilt:
    ...
    def test_rank2(self, dt):
        ...
        y_r2_a0 = np.array(
            [[0, 2, 4], [6, 4, 2], [0, 2, 4], [6, 4, 2]], dtype=dt
        )
    ...
    def test_initial_conditions(self, dt):
        ...
        assert_raises(
            ValueError, sosfilt, sos, x, zi=zi_nd[:, :, :, [0, 1, 1]]
        )
...
class TestUniqueRoots:
    ...
    def test_complex_repeat(self):
        ...
        assert_almost_equal(
            unique,
            [-1.0 + 0.025j, -0.925 + 0.15j, 0.0, 0.475 + 0.525j],
            decimal=15,
        )
        assert_equal(multiplicity, [2, 2, 1, 2])
