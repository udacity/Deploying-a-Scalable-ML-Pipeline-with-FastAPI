    ...
    def __new__(cls, *system, **kwargs):
        ...
        if cls is dlti:
            ...
            if N == 2:
                ...
                return StateSpaceDiscrete.__new__(
                    StateSpaceDiscrete, *system, **kwargs
                )
            else:
                raise ValueError(
    ...
    *system: arguments
    ...
    def to_discrete(self, dt, method="zoh", alpha=None):
        ...
        return TransferFunction(
            *cont2discrete(
                (self.num, self.den), dt, method=method, alpha=alpha
            )[:-1],
            dt=dt,
        )
...
class ZerosPolesGain(LinearTimeInvariant):
    ...
    *system : arguments
    ...
    def to_ss(self):
        ...
        """
        return StateSpace(
            *zpk2ss(self.zeros, self.poles, self.gain), **self._dt_dict
        )
...
class StateSpace(LinearTimeInvariant):
    ...
    *system: arguments
    ...
    def to_tf(self, **kwargs):
        ...
        """
        return TransferFunction(
            *ss2tf(self._A, self._B, self._C, self._D, **kwargs),
            **self._dt_dict,
        )
    ...
    def to_zpk(self, **kwargs):
        ...
        """
        return ZerosPolesGain(
            *ss2zpk(self._A, self._B, self._C, self._D, **kwargs),
            **self._dt_dict,
        )
...
def lsim(system, U, T, X0=None, interp=True):
    ...
    if isinstance(system, lti):
        ...
        raise AttributeError(
            "lsim can only be used with continuous-time " "systems."
        )
    else:
    ...
    no_input = (
        U is None
        or (isinstance(U, (int, float)) and U == 0.0)
        or not np.any(U)
    )
...
def step(system, X0=None, T=None, N=None):
    ...
    if isinstance(system, lti):
        ...
        raise AttributeError(
            "step can only be used with continuous-time " "systems."
        )
    else:
        sys = lti(*system)._as_ss()
    if N is None:
...
def freqresp(system, w=None, n=10000):
    ...
    if sys.inputs != 1 or sys.outputs != 1:
        raise ValueError(
            "freqresp() requires a SISO (single input, single "
            "output) system."
        )
...
def _valid_inputs(A, B, poles, method, rtol, maxiter):
    ...
    if len(poles) < A.shape[0]:
        raise ValueError(
            "number of poles is %d but you should provide %d"
            % (len(poles), A.shape[0])
        )
...
def _YT_real(ker_pole, Q, transfer_matrix, i, j):
    ...
    # step 2 page 19
    m = np.dot(
        np.dot(ker_pole[i].T, np.dot(u, v.T) - np.dot(v, u.T)), ker_pole[j]
    )
    ...
    transfer_matrix_ij = np.dot(
        np.dot(ker_pole_mu_nu, ker_pole_mu_nu.T),
...
def _YT_complex(ker_pole, Q, transfer_matrix, i, j):
    ...
    m = np.dot(
        np.dot(
            np.conj(ker_pole_ij.T),
            np.dot(u, np.conj(u).T) - np.dot(np.conj(u), u.T),
        ),
        ker_pole_ij,
    )
    ...
    if not np.allclose(
        np.abs(e_val[e_val_idx[-1]]), np.abs(e_val[e_val_idx[-2]])
    ):
    ...
    if not np.allclose(transfer_matrix_i_j, 0):
        transfer_matrix_i_j = transfer_matrix_i_j / np.linalg.norm(
            transfer_matrix_i_j
        )
    ...
    while nb_try < maxiter and not stop:
        ...
        for i, j in update_order:
            if i == j:
                ...
                transfer_matrix_not_i_j = np.delete(
                    transfer_matrix, (i, j), axis=1
                )
...
def place_poles(A, B, poles, method="YT", rtol=1e-3, maxiter=30):
    ...
    if B.shape[0] == rankB:
        ...
        for j in range(B.shape[0]):
            ...
            transfer_matrix_j = transfer_matrix_j / np.linalg.norm(
                transfer_matrix_j
            )
...
def dlsim(system, u, t=None, x0=None):
    ...
    for i in range(0, out_samples - 1):
        xout[i + 1, :] = np.dot(system.A, xout[i, :]) + np.dot(
            system.B, u_dt[i, :]
        )
        yout[i, :] = np.dot(system.C, xout[i, :]) + np.dot(
            system.D, u_dt[i, :]
        )

    # Last point
    yout[out_samples - 1, :] = np.dot(
        system.C, xout[out_samples - 1, :]
...
def dfreqresp(system, w=None, n=10000, whole=False):
    ...
    if system.inputs != 1 or system.outputs != 1:
        raise ValueError(
            "dfreqresp requires a SISO (single input, single "
            "output) system."
        )
