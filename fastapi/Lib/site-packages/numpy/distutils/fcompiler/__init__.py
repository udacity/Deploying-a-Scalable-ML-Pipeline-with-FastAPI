...
class FCompiler(CCompiler):
    ...
    def _compile(self, obj, src, ext, cc_args, extra_postargs, pp_opts):
        ...
        if extra_flags:
            log.info(
                "using compile options from source: %r" % " ".join(extra_flags)
            )
    ...
    def module_options(self, module_dirs, module_build_dir):
        ...
        if self.module_dir_switch is not None:
            ...
            print(
                "XXX: module_build_dir=%r option ignored" % (module_build_dir)
            )
        ...
        if self.module_include_switch is not None:
            ...
            print(
                "XXX: Fix module_include_switch for ", self.__class__.__name__
            )
        return options
...
def get_default_fcompiler(
    ...
    platform."""
    matching_compiler_types = available_fcompilers_for_platform(
        osname, platform
    )
    log.info(
        "get_default_fcompiler: matching types: '%s'", matching_compiler_types
    )
...
def show_fcompilers(dist=None):
    ...
    compilers_ni = [
        ("fcompiler=" + fc, None, fcompiler_class[fc][2])
