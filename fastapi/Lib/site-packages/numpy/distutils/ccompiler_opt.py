...
class _Config:
    ...
    conf_features = dict(
        ...
        ## Advanced SIMD
        ASIMD=dict(
            interest=4, implies="NEON_FP16 NEON_VFPV4", implies_detect=False
        ),
...
class _Distutils:
    ...
    def dist_info(self):
        ...
        cc_info = getattr(
            self._ccompiler,
            "compiler",
            getattr(self._ccompiler, "compiler_so", ""),
        )
    ...
    def _dist_test_spawn(cmd, display=None):
        ...
        _Distutils.dist_error(
            "Command",
            cmd,
            "failed with exit status %d output -> \n%s" % (s, o),
        )
    ...
    def __init__(self, cache_path=None, *factors):
        ...
        if cache_path:
            if os.path.exists(cache_path):
                ...
                elif not hasattr(cache_mod, "hash") or not hasattr(
                    cache_mod, "data"
                ):
...
class _CCompiler:
    ...
    def _cc_normalize_unix(self, flags):
        def ver_flags(f):
            ...
            ver = float(
                "0"
                + "".join(re.findall(self._cc_normalize_arch_ver, tokens[0]))
            )
        ...
        for i, cur_flag in enumerate(reversed(flags)):
            ...
            upper_flags = flags[-i:]
            filtered = list(
                filter(self._cc_normalize_unix_frgx.search, lower_flags)
            )
            # gather subflags
...
class _Feature:
    ...
    def feature_extra_checks(self, name):
        ...
        self.dist_log(
            "Testing extra checks for feature '%s'" % name, extra_checks
        )
        ...
        if not_available:
            self.dist_log(
                "testing failed for checks", not_available, stderr=True
            )
        return available
...
class _Parse:
    ...
    def __init__(self, cpu_baseline, cpu_dispatch):
        ...
        for group_name, tokens in self.conf_target_groups.items():
            ...
            if not tokens or not tokens.strip():
                ...
                continue
            has_baseline, features, extra_flags = self._parse_target_tokens(
                tokens
            )
    ...
    def _parse_arg_features(self, arg_name, req_features):
        ...
        tokens = list(
            filter(None, re.split(self._parse_regex_arg, req_features))
        )
    ...
    def _parse_token_group(
        self, token, has_baseline, final_targets, extra_flags
    ):
...
class CCompilerOpt(_Config, _Distutils, _Cache, _CCompiler, _Feature, _Parse):
    ...
    def __init__(
        ...
        _Cache.__init__(
            self, cache_path, self.dist_info(), cpu_baseline, cpu_dispatch
        )
    ...
    def generate_dispatch_header(self, header_path):
        ...
        with open(header_path, "w") as f:
            ...
            for name in baseline_names:
                baseline_pre += (
                    self.feature_c_preprocessor(name, tabs=1) + "\n"
                )
    ...
    def report(self, full=False):
        ...
        baseline_rows.append(
            (
                "Enabled",
                (" ".join(baseline_names) if baseline_names else "none"),
            )
        )
        baseline_flags = self.cpu_baseline_flags()
        baseline_rows.append(
        ...
        dispatch_rows.append(
            (
                "Enabled",
                (" ".join(dispatch_names) if dispatch_names else "none"),
            )
        )
        ########## Generated ##########
        # TODO:
        ...
        if not full or not target_sources:
            ...
            for tar in self.feature_sorted(target_sources):
                sources = target_sources[tar]
                pretty_name = (
                    tar if isinstance(tar, str) else "(%s)" % " ".join(tar)
                )
                flags = " ".join(self.feature_flags(tar))
                implies = " ".join(
                    self.feature_sorted(self.feature_implies(tar))
                )
