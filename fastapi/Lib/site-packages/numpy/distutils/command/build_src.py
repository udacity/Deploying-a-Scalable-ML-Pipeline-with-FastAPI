...
class build_src(build_ext.build_ext):
    ...
    def _build_npy_pkg_config(self, info, gd):
        ...
        if self.inplace == 1:
            ...
            generated_dir = os.path.join(
                self.build_src, template_dir, install_dir
            )
    ...
    def build_library_sources(self, lib_name, build_info):
        ...
        if h_files:
            log.info(
                "%s - nothing done with h_files = %s", self.package, h_files
            )
    ...
    def f2py_sources(self, sources, extension):
        ...
        for source in sources:
            ...
            if ext == ".pyf":  # F2PY interface file
                ...
                if os.path.isfile(source):
                    ...
                    log.debug(
                        "  source %s does not exist: skipping f2py'ing."
                        % (source)
                    )
                    name = ext_name
                    skip_f2py = 1
                    ...
                    if not os.path.isfile(target_file):
                        ...
                        log.info(
                            "   Yes! Using %r as up-to-date target."
                            % (target_file)
                        )
        ...
        if f2py_sources:
            ...
            if (
                ...
                f2py2e.run_main(
                    f2py_options + ["--build-dir", target_dir, source]
                )
            else:
                log.debug(
        ...
        for source in sources:
            ...
            if ext == ".i":  # SWIG interface file
                ...
                if os.path.isfile(source):
                    ...
                    log.warn(
                        "  source %s does not exist: skipping swig'ing."
                        % (source)
                    )
                    name = ext_name[1:]
                    skip_swig = 1
        ...
        for source in swig_sources:
            ...
            if self.force or newer_group(depends, target, "newer"):
                log.info(
                    "%s: %s"
                    % (
                        os.path.basename(swig) + (is_cpp and "++" or ""),
                        source,
                    )
                )
                self.spawn(
                ...
                log.debug(
                    "  skipping '%s' swig interface (up-to-date)" % (source)
                )
