...
class build_clib(old_build_clib):
    ...
    def finalize_options(self):
        if self.parallel:
            try:
                ...
                raise ValueError(
                    "--parallel/-j argument must be an integer"
                ) from e
    ...
    def run(self):
        ...
        self.compiler.customize(
            self.distribution, need_cxx=self.have_cxx_sources()
        )
    ...
    def build_a_library(self, build_info, lib_name, libraries):
        ...
        requiref90 = (
            not not fmodule_sources or build_info.get("language", "c") == "f90"
        )
        ...
        lib_file = compiler.library_filename(
            lib_name, output_dir=self.build_clib
        )
        ...
        if c_sources:
            ...
            objects += compiler.compile(
                ...
                debug=self.debug,
                extra_postargs=(
                    extra_postargs + copt_baseline_flags + extra_cflags
                ),
            )
        ...
        if cxx_sources:
            ...
            cxx_objects = cxx_compiler.compile(
                ...
                debug=self.debug,
                extra_postargs=(
                    extra_postargs + copt_baseline_flags + extra_cxxflags
                ),
            )
            ...
            if requiref90 and self._f_compiler.module_dir_switch is None:
                ...
                for f in glob("*.mod"):
                    ...
                    try:
                        ...
                        log.warn(
                            "failed to move %r to %r" % (f, module_build_dir)
                        )
