...
class system_info:
        ...
        for lib in libs:
            ...
            if r_dirs:
                ...
                if i is not None:
                    ...
                    log.info(
                        "Runtime library %s was not found. Ignoring" % (lib)
                    )
    ...
    def get_paths(self, section, key):
        ...
        for d in dirs:
            if len(d) > 0 and not os.path.isdir(d):
                warnings.warn(
                    "Specified path %s is invalid." % d, stacklevel=2
                )
                continue
    ...
    def check_libs(self, lib_dirs, libs, opt_libs=[]):
        ...
        if not info:
            log.info(
                "  libraries %s not found in %s", ",".join(libs), lib_dirs
            )
        return info
    ...
    def __init__(self):
        ...
        if tcsdsroot is None:
            ...
            system_info.__init__(
                ...
                default_include_dirs=[
                    os.path.join(tcsdsroot, "clang-comp/include")
                ],
            )
...
class atlas_info(system_info):
    ...
    def calc_info(self):
        ...
        if lapack is not None:
            ...
            dict_append(info, **atlas)
            dict_append(
                info, define_macros=[("ATLAS_WITH_LAPACK_ATLAS", None)]
            )
            self.set_info(**info)
            return
        else:
...
def get_atlas_version(**config):
    ...
    if atlas_version is None:
        ...
        dict_append(
            info,
            define_macros=[("ATLAS_INFO", _c_string_literal(atlas_version))],
        )
...
class openblas_info(blas_info):
    ...
    def _calc_info(self):
        ...
        if self.symbol_prefix:
            info["define_macros"] += [
                ("BLAS_SYMBOL_PREFIX", self.symbol_prefix)
            ]
    ...
    def check_symbols(self, info):
        ...
        prototypes = "\n".join(
            "void %s%s%s();"
            % (self.symbol_prefix, symbol_name, self.symbol_suffix)
...
class accelerate_info(system_info):
    ...
    def calc_info(self):
        ...
        if sys.platform == "darwin" and not os.getenv(
            "_PYTHON_HOST_PLATFORM", None
        ):
    ...
    def calc_info(self):
        ...
        for d in include_dirs:
            if self.combine_paths(
                d, os.path.join(self.modulename, "arrayobject.h")
            ):
                inc_dir = d
                break
        if inc_dir is not None:
...
class f2py_info(system_info):
    def calc_info(self):
        ...
        self.set_info(
            sources=[os.path.join(f2py_dir, "fortranobject.c")],
...
class boost_python_info(system_info):
    ...
    def calc_info(self):
        ...
        for d in src_dirs:
            if os.path.isfile(
                os.path.join(d, "libs", "python", "src", "module.cpp")
            ):
                src_dir = d
                break
        if not src_dir:
...
class agg2_info(system_info):
    ...
    def calc_info(self):
        ...
        if sys.platform == "win32":
            agg2_srcs = glob(
                os.path.join(
                    src_dir, "src", "platform", "win32", "agg_win32_bmp.cpp"
                )
            )
        else:
    ...
    def calc_info(self):
        ...
        if version:
            ...
            if self.version_macro_name:
                macros.append(
                    (
                        self.version_macro_name
                        + "_%s" % (version.replace(".", "_")),
                        None,
                    )
                )
        if self.release_macro_name:
            ...
            if release:
                macros.append(
                    (
                        self.release_macro_name
                        + "_%s" % (release.replace(".", "_")),
                        None,
                    )
                )
