...
def CCompiler_customize_cmd(self, cmd, ignore=()):
    ...
    log.info(
        "customize %s using %s"
        % (self.__class__.__name__, cmd.__class__.__name__)
    )
    ...
    if need_cxx:
        ...
        if hasattr(self, "compiler") and "cc" in self.compiler[0]:
            ...
            if not hasattr(self, "compiler_cxx"):
                log.warn(
                    "Missing compiler_cxx fix for " + self.__class__.__name__
                )
...
def CCompiler_cxx_compiler(self):
    ...
    cxx.compiler_so = [cxx.compiler_cxx[0]] + sanitize_cxx_flags(
        cxx.compiler_so[1:]
    )
    if (
        sys.platform.startswith(("aix", "os400"))
        and "ld_so_aix" in cxx.linker_so[0]
    ):
