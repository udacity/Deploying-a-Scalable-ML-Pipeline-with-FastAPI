...
class TestTimeConversionFormats:
    ...
    def test_to_datetime_format_YYYYMMDD_ignore(self, cache):
        ...
        result = to_datetime(
            ser, format="%Y%m%d", errors="ignore", cache=cache
        )
        expected = Series(
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                ["201010", pd.NA],
                "%y%m%d",
                DatetimeIndex(["2020-10-10", "NaT"]),
            ),
            (
                ["201010", pd.NA],
                "%d%m%y",
                DatetimeIndex(["2010-10-20", "NaT"]),
            ),
            (
                [None, np.nan, pd.NA],
                None,
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            [
                "2020-10-26 00:00:00+06:00",
                datetime(
                    2020, 1, 1, 18, tzinfo=pytz.timezone("Australia/Melbourne")
                ),
            ],
        ],
    )
...
class TestToDatetimeUnit:
    ...
    def test_unit_array_mixed_nans_large_int(self, cache):
        ...
        expected = Index(
            [1420043460000000000000000, NaT, NaT, NaT, NaT], dtype=object
        )
        tm.assert_index_equal(result, expected)
...
class TestDaysInMonth:
    ...
    def test_day_not_in_month_ignore(self, cache, expected, format):
        result = to_datetime(
            expected, errors="ignore", format=format, cache=cache
        )
        assert result == expected
...
def test_empty_string_datetime(errors, args, format):
    ...
    expected = Series(
        ["2016-03-24", "2016-03-25", NaT], dtype="datetime64[ns]"
    )
...
def test_to_datetime_mixed_not_necessarily_iso8601_raise():
    ...
    with pytest.raises(
        ValueError,
        match="Time data 01-01-2000 is not ISO8601 format, at position 1",
    ):
...
def test_to_datetime_mixed_not_necessarily_iso8601_coerce(errors, expected):
    # https://github.com/pandas-dev/pandas/issues/50411
    result = to_datetime(
        ["2020-01-01", "01-01-2000"], format="ISO8601", errors=errors
    )
    tm.assert_index_equal(result, expected)
...
def test_to_datetime_with_empty_str_utc_false_format_mixed():
    ...
    result = to_datetime(vals, format="mixed")
    expected = Index(
        [Timestamp("2020-01-01 00:00+00:00"), "NaT"], dtype="M8[ns, UTC]"
    )
    tm.assert_index_equal(result, expected)
