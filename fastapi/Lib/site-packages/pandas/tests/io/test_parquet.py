...
class TestParquetFastParquet(Base):
    ...
    @pytest.mark.skipif(
        using_copy_on_write(), reason="fastparquet writes into Index"
    )
    ...
    def test_use_nullable_dtypes_not_supported(self, fp):
        ...
        with tm.ensure_clean() as path:
            df.to_parquet(path)
            with pytest.raises(
                ValueError, match="not supported for the fastparquet"
            ):
                with tm.assert_produces_warning(FutureWarning):
                    read_parquet(
                        path, engine="fastparquet", use_nullable_dtypes=True
                    )
            with pytest.raises(
                ValueError, match="not supported for the fastparquet"
            ):
                read_parquet(
                    path, engine="fastparquet", dtype_backend="pyarrow"
                )
