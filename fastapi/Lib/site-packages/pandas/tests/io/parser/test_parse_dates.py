...
def test_read_csv_with_custom_date_parser(all_parsers):
    ...
    time = pd.TimedeltaIndex(
        [pd.to_timedelta(i, unit="s") for i in time], name="time"
    )
    expected = DataFrame(
        {
            "e": [
...
def test_read_csv_with_custom_date_parser_parse_dates_false(all_parsers):
    ...
    time = Series(
        [41047.00, 41048.00, 41049.00, 41050.00, 41051.00], name="time"
    )
    expected = DataFrame(
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            DataFrame(
                index=DatetimeIndex(["2016-04-15"], name="a"), columns=[]
            ),
        ),
        (
