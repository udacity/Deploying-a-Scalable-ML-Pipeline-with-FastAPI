...
class TestRoundTrip:
    ...
    def test_read_one_empty_col_with_header(self, ext, header, expected):
        ...
        with tm.ensure_clean(ext) as path:
            df.to_excel(
                path, sheet_name="with_header", index=False, header=True
            )
            result = pd.read_excel(
                path, sheet_name=filename, usecols=[0], header=header
            )
...
class TestExcelWriter:
    ...
    def test_roundtrip(self, frame, path):
        ...
        recons = pd.read_excel(
            path, sheet_name="test1", index_col=0, na_values=["NA"]
        )
        tm.assert_frame_equal(frame, recons)
        ...
        recons = pd.read_excel(
            path, sheet_name="test1", index_col=0, na_values=["88"]
        )
        tm.assert_frame_equal(frame, recons)
    ...
    def test_to_excel_output_encoding(self, ext):
        ...
        with tm.ensure_clean(
            "__tmp_to_excel_float_format__." + ext
        ) as filename:
    ...
    def test_excel_010_hemstring(
        ...
        def roundtrip(data, header=True, parser_hdr=0, index=True):
            data.to_excel(
                path, header=header, merge_cells=merge_cells, index=index
            )
