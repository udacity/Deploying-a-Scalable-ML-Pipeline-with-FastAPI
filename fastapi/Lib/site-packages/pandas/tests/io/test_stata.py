...
class TestStata:
    ...
    @pytest.mark.parametrize(
        "file", ["stata8_113", "stata8_115", "stata8_117"]
    )
    ...
    def test_invalid_variable_label_encoding(self, version, mixed_frame):
        ...
        with tm.ensure_clean() as path:
            with pytest.raises(
                ValueError,
                match="Variable labels must contain only characters",
            ):
                mixed_frame.to_stata(
                    path, variable_labels=variable_labels, version=version
                )
...
def test_compression(
    compression, version, use_dict, infer, compression_to_extension
):
...
def test_chunked_categorical(version):
    ...
    with tm.ensure_clean() as path:
        ...
        with StataReader(
            path, chunksize=2, order_categoricals=False
        ) as reader:
...
def test_precision_loss():
    ...
    with tm.ensure_clean() as path:
        with tm.assert_produces_warning(
            PossiblePrecisionLoss,
            match="Column converted from int64 to float64",
        ):
            df.to_stata(path, write_index=False)
        reread = read_stata(path)
