...
def test_count(any_string_dtype):
    ...
    expected_dtype = (
        np.float64 if any_string_dtype in object_pyarrow_numpy else "Int64"
    )
...
def test_ismethods(method, expected, any_string_dtype):
    ...
    expected_dtype = (
        "bool" if any_string_dtype in object_pyarrow_numpy else "boolean"
    )
...
def test_isnumeric_unicode_missing(method, expected, any_string_dtype):
    ...
    expected_dtype = (
        "object" if any_string_dtype in object_pyarrow_numpy else "boolean"
    )
    expected = Series(expected, dtype=expected_dtype)
    result = getattr(ser.str, method)()
    tm.assert_series_equal(result, expected)
...
def test_len(any_string_dtype):
    ...
    result = ser.str.len()
    expected_dtype = (
        "float64" if any_string_dtype in object_pyarrow_numpy else "Int64"
    )
...
def test_index(
    ...
    expected_dtype = (
        np.int64 if any_string_dtype in object_pyarrow_numpy else "Int64"
    )
