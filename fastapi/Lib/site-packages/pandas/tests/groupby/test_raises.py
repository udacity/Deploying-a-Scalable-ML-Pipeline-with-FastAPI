...
def test_groupby_raises_string_np(
    ...
    _call_and_check(
        klass, msg, how, gb, groupby_func_np, (), warn_msg=warn_msg
    )
...
def test_groupby_raises_datetime(
    ...
    klass, msg = {
        ...
        "cumsum": (
            TypeError,
            "datetime64 type does not support cumsum operations",
        ),
        "diff": (None, ""),
        "ffill": (None, ""),
        "fillna": (None, ""),
...
def test_subsetting_columns_axis_1_raises():
    ...
    with pytest.raises(
        ValueError, match="Cannot subset columns when using axis=1"
    ):
        gb["b"]
