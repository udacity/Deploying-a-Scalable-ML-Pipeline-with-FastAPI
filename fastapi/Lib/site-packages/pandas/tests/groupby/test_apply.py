...
def test_apply_trivial(using_infer_string):
    ...
    expected = pd.concat(
        [df.iloc[1:], df.iloc[1:]], axis=1, keys=["float64", dtype]
    )
...
def test_apply_datetime_issue(group_column_dtlike, using_infer_string):
    ...
    dtype = "string" if using_infer_string else "object"
    expected = DataFrame(
        ["spam"], Index(["foo"], dtype=dtype, name="a"), columns=[42]
    )
    tm.assert_frame_equal(result, expected)
...
def test_include_groups(include_groups):
    ...
    expected = DataFrame(
        {"a": [2, 2], "b": [7, 5]}, index=Index([1, 2], name="a")
    )
...
def test_builtins_apply(keys, f):
    ...
    assert_msg = (
        f"invalid frame shape: {result.shape} (expected ({ngroups}, 3))"
    )
    assert result.shape == (ngroups, 3), assert_msg

    npfunc = lambda x: getattr(np, fname)(
        x, axis=0
    )  # numpy's equivalent function
    msg = "DataFrameGroupBy.apply operated on the grouping columns"
    with tm.assert_produces_warning(DeprecationWarning, match=msg):
    ...
    tm.assert_series_equal(
        getattr(result, fname)(axis=0), getattr(df, fname)(axis=0)
    )
