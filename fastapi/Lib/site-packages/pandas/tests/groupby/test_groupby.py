...
def test_as_index_select_column():
    ...
    expected = Series(
        [2, 6, 6],
        name="B",
        index=MultiIndex.from_tuples([(0, 0), (0, 1), (1, 2)]),
    )
    tm.assert_series_equal(result, expected)
...
def test_set_group_name(df, grouper, using_infer_string):
    ...
    def freduce(group):
        ...
        if (
            using_infer_string
            and grouper == "A"
            and is_string_dtype(group.dtype)
        ):
...
def test_rolling_wrong_param_min_period():
    ...
    result_error_msg = (
        r"__init__\(\) got an unexpected keyword argument 'min_period'"
    )
...
def test_groupby_none_column_name():
    ...
    expected = DataFrame(
        {"b": [2, 5], "c": [9, 13]}, index=Index([1, 2], name=None)
    )
    tm.assert_frame_equal(result, expected)
