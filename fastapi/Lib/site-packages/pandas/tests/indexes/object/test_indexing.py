...
class TestGetIndexer:
    ...
    def test_get_indexer_strings_raises(self, using_infer_string):
        ...
        if using_infer_string:
            ...
            with pytest.raises(pa.lib.ArrowNotImplementedError, match=msg):
                index.get_indexer(
                    ["a", "b", "c", "d"], method="pad", tolerance=2
                )
...
class TestGetIndexerNonUnique:
    def test_get_indexer_non_unique_nas(
        ...
        if using_infer_string and (
            ...
            request.applymarker(
                pytest.mark.xfail(reason="NAs are cast to NaN")
            )
...
class TestSliceLocs:
    @pytest.mark.parametrize(
        ...
        [
            "object",
            pytest.param(
                "string[pyarrow_numpy]", marks=td.skip_if_no("pyarrow")
            ),
        ],
    )
    @pytest.mark.parametrize(
    ...
    def test_slice_locs_negative_step(
        self, in_slice, expected, dtype, request
    ):
        if (
            ...
            request.applymarker(
                pytest.mark.xfail(
                    reason="https://github.com/apache/arrow/issues/40642"
                )
            )
