...
class TestSeriesReplace:
    ...
    @pytest.mark.xfail(
        using_pyarrow_string_dtype(), reason="can't fill 0 in string"
    )
    ...
    def test_pandas_replace_na(self):
        # GH#43344
        ser = pd.Series(
            ["AA", "BB", "CC", "DD", "EE", "", pd.NA], dtype="string"
        )
        regex_mapping = {
            "AA": "CC",
            "BB": "CC",
        ...
        exp = pd.Series(
            ["CC", "CC", "CC-REPL", "DD", "CC", "", pd.NA], dtype="string"
        )
    ...
    @pytest.mark.xfail(
        using_pyarrow_string_dtype(), reason="can't fill 1 in string"
    )
