...
class TestDataFrameAnalytics:
    ...
    def test_mode_empty_df(self):
        ...
        result = df.mode()
        expected = DataFrame(
            [], columns=["a", "b"], index=Index([], dtype=np.int64)
        )
        tm.assert_frame_equal(result, expected)
    ...
    def test_sum_mixed_datetime(self):
        ...
        with pytest.raises(
            TypeError, match="does not support reduction 'sum'"
        ):
            df.sum()
    ...
    def test_idxmax_mixed_dtype(self):
        ...
        df = DataFrame(
            {1: [0, 2, 1], 2: range(3)[::-1], 3: dti.copy(deep=True)}
        )
    ...
    def test_any_all_np_func(self, func, data, expected):
        ...
        if any(isinstance(x, CategoricalDtype) for x in data.dtypes):
            ...
            with tm.assert_produces_warning(
                warn, match=msg, check_stacklevel=False
            ):
                # GH#34479
                result = func(data)
