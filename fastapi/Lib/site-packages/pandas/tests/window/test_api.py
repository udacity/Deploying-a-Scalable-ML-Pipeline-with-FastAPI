...
def test_agg(step):
    ...
    with tm.assert_produces_warning(
        FutureWarning, match="using Rolling.[mean|std]"
    ):
...
@pytest.mark.parametrize(
    "func",
    [["min"], ["mean", "max"], {"b": "sum"}, {"b": "prod", "c": "median"}],
)
...
def test_agg_apply(raw):
    ...
    with tm.assert_produces_warning(
        FutureWarning, match="using Rolling.[sum|std]"
    ):
...
def test_agg_consistency(step):
    ...
    with tm.assert_produces_warning(
        FutureWarning, match="using Rolling.[sum|mean]"
    ):
