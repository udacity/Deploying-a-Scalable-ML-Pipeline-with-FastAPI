...
class TestTZLocalizeToUTC:
    def test_tz_localize_to_utc_ambiguous_infer(self):
        ...
        with pytest.raises(
            pytz.AmbiguousTimeError, match="2011-11-06 01:00:00"
        ):
            tz_localize_to_utc(
                vals, pytz.timezone("US/Eastern"), ambiguous="infer"
            )

        with pytest.raises(
            pytz.AmbiguousTimeError, match="are no repeated times"
        ):
            tz_localize_to_utc(
                vals[:1], pytz.timezone("US/Eastern"), ambiguous="infer"
            )
