...
def test_concat_series(to_concat_dtypes, result_dtype):
    ...
    result = pd.concat(
        [pd.Series([0, 1, pd.NA], dtype=t) for t in to_concat_dtypes]
    )
...
def test_concat_series_with_numpy(to_concat_dtypes, result_dtype):
    ...
    expected = pd.Series([0, 1, pd.NA, 0, 1], dtype=object).astype(
        result_dtype
    )
    tm.assert_series_equal(result, expected)
    ...
    expected = pd.Series([0, 1, 0, 1, pd.NA], dtype=object).astype(
        result_dtype
    )
    tm.assert_series_equal(result, expected)
