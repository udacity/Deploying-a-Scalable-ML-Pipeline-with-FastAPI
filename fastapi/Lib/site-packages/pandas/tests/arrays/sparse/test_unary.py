...
class TestUnaryMethods:
    ...
    def test_neg_operator(self):
        arr = SparseArray(
            [-1, -2, np.nan, 3], fill_value=np.nan, dtype=np.int8
        )
        res = -arr
    ...
    def test_invert_operator(self):
        arr = SparseArray(
            [False, True, False, True], fill_value=False, dtype=np.bool_
        )
        exp = SparseArray(
            np.invert([False, True, False, True]),
            fill_value=True,
            dtype=np.bool_,
        )
        res = ~arr
        ...
        res = ~arr
        exp = SparseArray(
            [-1, -2, -1, -3, -4, -1], fill_value=-1, dtype=np.int32
        )
        tm.assert_sp_array_equal(exp, res)
