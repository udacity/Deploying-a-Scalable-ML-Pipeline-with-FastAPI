...
def test_resample_how_callables(unit):
    ...
    ind = date_range(start="2014-01-01", periods=len(data), freq="d").as_unit(
        unit
    )
...
def test_resample_anchored_intraday(unit):
    ...
    with tm.assert_produces_warning(FutureWarning, match=msg):
        expected = (
            df.resample("ME", kind="period").mean().to_timestamp(how="end")
        )
...
def test_resample_anchored_intraday2(unit):
    ...
    with tm.assert_produces_warning(FutureWarning, match=msg):
        expected = (
            df.resample("QE", kind="period").mean().to_timestamp(how="end")
        )
    expected.index += Timedelta(1, "ns") - Timedelta(1, "D")
    ...
    with tm.assert_produces_warning(FutureWarning, match=msg):
        expected = (
            df.shift(1, freq="D")
            .resample("QE", kind="period", closed="left")
            .mean()
        )
