...
def _stata_elapsed_date_to_datetime_vec(dates: Series, fmt: str) -> Series:
    ...
    def convert_year_month_safe(year, month) -> Series:
        ...
        if year.max() < MAX_YEAR and year.min() > MIN_YEAR:
            ...
            return Series(
                [datetime(y, m, 1) for y, m in zip(year, month)], index=index
            )
    ...
    def convert_year_days_safe(year, days) -> Series:
        ...
        if year.max() < (MAX_YEAR - 1) and year.min() > MIN_YEAR:
            ...
            index = getattr(year, "index", None)
            value = [
                datetime(y, 1, 1) + timedelta(days=int(d))
    ...
    def convert_delta_safe(base, deltas, unit) -> Series:
        ...
        if unit == "d":
            ...
            if deltas.max() > MAX_MS_DELTA or deltas.min() < MIN_MS_DELTA:
                values = [
                    base + timedelta(microseconds=(int(d) * 1000))
                    for d in deltas
                ]
                return Series(values, index=index)
        else:
    ...
    def parse_dates_safe(
        ...
        if lib.is_np_dtype(dates.dtype, "M"):
            if delta:
                ...
                d["delta"] = (
                    time_delta._values.view(np.int64) // 1000
                )  # microseconds
            if days or year:
