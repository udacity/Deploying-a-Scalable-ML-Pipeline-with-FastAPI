...
class LibController(ABC):
    ...
    def _get_symbol(self, name):
        ...
        return getattr(
            self.dynlib,
            f"{self._symbol_prefix}{name}{self._symbol_suffix}",
            None,
        )
...
class OpenBLASController(LibController):
    ...
    check_symbols = tuple(
        ...
        for prefix, suffix in itertools.product(
            _symbol_prefixes, _symbol_suffixes
        )
    )
    ...
    def _find_affixes(self):
        for prefix, suffix in itertools.product(
            ...
            if hasattr(
                self.dynlib, f"{prefix}openblas_get_num_threads{suffix}"
            ):
...
class BLISController(LibController):
    ...
    def _get_threading_layer(self):
        ...
        if getattr(self.dynlib, "bli_info_get_enable_openmp", lambda: False)():
            ...
            self.dynlib, "bli_info_get_enable_pthreads", lambda: False
        )():
            return "pthreads"
        return "disabled"
...
class MKLController(LibController):
    ...
    def set_num_threads(self, num_threads):
        set_func = getattr(
            self.dynlib, "MKL_Set_Num_Threads", lambda num_threads: None
        )
        return set_func(num_threads)
...
class OpenMPController(LibController):
    ...
    def set_num_threads(self, num_threads):
        set_func = getattr(
            self.dynlib, "omp_set_num_threads", lambda num_threads: None
        )
        return set_func(num_threads)
...
@_format_docstring(
    USER_APIS=list(_ALL_USER_APIS), INTERNAL_APIS=_ALL_INTERNAL_APIS
)
def threadpool_info():
...
class _ThreadpoolLimiter:
    ...
    def _check_params(self, limits, user_api):
        ...
        if (
            isinstance(limits, str)
            and limits == "sequential_blas_under_openmp"
        ):
            (
                limits,
                user_api,
        ...
        if limits is None or isinstance(limits, int):
            ...
            if isinstance(limits, list):
                ...
                limits = {
                    lib_info["prefix"]: lib_info["num_threads"]
...
class ThreadpoolController:
    ...
    def info(self):
        ...
        return [
            lib_controller.info() for lib_controller in self.lib_controllers
        ]
