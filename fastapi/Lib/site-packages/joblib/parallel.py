...
class Parallel(Logger):
    ...
    * Ability to use shared memory efficiently with worker
    ...
    def __init__(
        ...
        if require == "sharedmem" and not getattr(
            backend, "supports_sharedmem", False
        ):
            raise ValueError(
                "Backend %s does not support shared memory" % backend
            )
        ...
        with self._lock:
            ...
            try:
                ...
                try:
                    ...
                    self._jobs.append(batch_tracker)
                    batch_tracker._register_outcome(
                        dict(result=e, status=TASK_ERROR)
                    )
                    return True
