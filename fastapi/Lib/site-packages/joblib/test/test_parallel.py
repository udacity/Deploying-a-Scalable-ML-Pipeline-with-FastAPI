...
def nested_loop(backend):
    Parallel(n_jobs=2, backend=backend)(
        delayed(square)(0.01) for _ in range(2)
    )
...
def test_overwrite_default_backend():
    ...
    try:
        register_parallel_backend(
            "threading", BACKENDS["threading"], make_default=True
        )
...
def test_parameterized_backend_context_manager(monkeypatch, context):
    monkeypatch.setitem(
        BACKENDS, "param_backend", ParameterizedParallelBackend
    )
    assert _active_backend_type() == DefaultBackend
...
def get_nested_pids():
    ...
    return Parallel(n_jobs=2)(
        delayed(sleep_and_return_pid)() for _ in range(2)
    )
...
def test_parallel_return_order_with_return_as_generator_parameter(n_jobs):
    ...
    input_list = [0] * 5
    result = Parallel(
        n_jobs=n_jobs, return_as="generator", backend="threading"
...
def _test_parallel_unordered_generator_returns_fastest_first(backend, n_jobs):
    ...
    result = Parallel(
        n_jobs=n_jobs, return_as="generator_unordered", backend=backend
...
def test_parallel_unordered_generator_returns_fastest_first_with_dask(
    ...
    with distributed.Client(n_workers=2, threads_per_worker=2), context(
        "dask"
    ):
...
def _test_deadlock_with_generator(backend, return_as, n_jobs):
    ...
    with Parallel(
        n_jobs=n_jobs, backend=backend, return_as=return_as
    ) as parallel:
...
def test_multiple_generator_call(backend, return_as, n_jobs):
    ...
    with raises(
        RuntimeError, match="This Parallel instance is already running"
    ):
...
def test_memmapping_leaks(backend, tmpdir):
    ...
    # arrays
    with Parallel(
        n_jobs=2, max_nbytes=1, backend=backend, temp_folder=tmpdir
    ) as p:
...
@parametrize(
    "backend",
    ([None, "threading"] if mp is None else [None, "loky", "threading"]),
)
...
def test_backend_batch_statistics_reset(backend):
    ...
    assert (
        p._backend._effective_batch_size
...
@parametrize("n_jobs", [2, -1])
@parametrize(
    "var_name", ["OPENBLAS_NUM_THREADS", "MKL_NUM_THREADS", "OMP_NUM_THREADS"]
)
@parametrize("context", [parallel_config, parallel_backend])
