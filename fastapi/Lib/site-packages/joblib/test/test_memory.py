...
def test_call_and_shelve_argument_hash(tmpdir):
    ...
    assert len(w) == 1
    assert "The 'argument_hash' attribute has been deprecated" in str(
        w[-1].message
    )
...
def test_call_and_shelve_lazily_load_stored_result(tmpdir):
    ...
    if test_access_time == os.stat(test_access_time_file.strpath).st_atime:
        ...
        pytest.skip(
            "filesystem does not support fine-grained access time " "attribute"
        )
...
def test__get_items(tmpdir):
    ...
    expected_hash_cache_sizes = [
        get_files_size(hash_dir) for hash_dir in hash_dirs
    ]
    hash_cache_sizes = [ci.size for ci in items]
    assert hash_cache_sizes == expected_hash_cache_sizes

    output_filenames = [
        os.path.join(hash_dir, "output.pkl") for hash_dir in hash_dirs
    ]
...
def test_memory_objects_repr(tmpdir):
    ...
    memorized_result_repr = (
        'MemorizedResult(location="{location}", '
...
def test_memory_pickle_dump_load(tmpdir, memory_kwargs):
    ...
    compare(
        memorized_result.store_backend, memorized_result_reloaded.store_backend
    )
    compare(
        memorized_result,
...
class TestCacheValidationCallback:
    ...
    def test_memory_expires_after(self, memory):
        ...
        f = memory.cache(
            self.foo,
            cache_validation_callback=expires_after(seconds=0.3),
            ignore=["d"],
        )
