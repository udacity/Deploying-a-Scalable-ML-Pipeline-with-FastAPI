...
def test_high_dimension_memmap_array_reducing(tmpdir):
    ...
    a = np.memmap(
        filename, dtype=np.float64, shape=(100, 15, 15, 3), mode="w+"
    )
...
def test_pool_with_memmap(factory, tmpdir):
    ...
    try:
        ...
        c = np.memmap(
            filename, dtype=np.float32, shape=(10,), mode="r", offset=5 * 4
        )
...
def test_workaround_against_bad_memmap_with_copied_buffers(factory, tmpdir):
    ...
    try:
        ...
        a = np.asarray(np.arange(6000).reshape((1000, 2, 3)), order="F")[
            :, :1, :
        ]
...
def test_pool_memmap_with_big_offset(factory, retry_no, tmpdir):
    ...
    offset = mmap.ALLOCATIONGRANULARITY + 1
    obj = make_memmap(
        fname, mode="w+", shape=size, dtype="uint8", offset=offset
    )
