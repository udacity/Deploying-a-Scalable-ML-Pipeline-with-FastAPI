...
def test_compress_level_error(wrong_compress):
    ...
    exception_msg = "Non valid compress level given: " '"{0}"'.format(
        wrong_compress
    )
...
def test_memmap_persistence(tmpdir):
    ...
    np.testing.assert_array_equal(
        obj_reloaded.array_float, obj_loaded.array_float
    )
...
def test_memory_usage(tmpdir, compress):
    ...
    for obj in (small_array, big_array):
        ...
        mem_used = memory_used(
            numpy_pickle.dump, obj, obj_filename, compress=compress
        )
    ...
    if pickle_reading_protocol >= pickle_writing_protocol:
        try:
            ...
            expected_nb_deprecation_warnings = (
                1
                if ("_0.9" in filename_base or "_0.8.4" in filename_base)
                else 0
            )
            ...
            user_warnings = [
                w for w in warninfo if issubclass(w.category, UserWarning)
            ]
...
def test_register_compressor(tmpdir):
    ...
    assert (
        _COMPRESSORS[compressor_name].fileobj_factory
...
def test_register_compressor_invalid_fileobj():
    ...
    class InvalidFileObjectWrapper(CompressorWrapper):
        def __init__(self):
            CompressorWrapper.__init__(
                self, obj=InvalidFileObject, prefix=b"prefix"
            )
...
def test_register_compressor_already_registered():
    ...
    with raises(ValueError) as excinfo:
        register_compressor(
            compressor_name, StandardLibGzipCompressorWrapper()
        )
    excinfo.match(
        "Compressor '{}' already registered.".format(compressor_name)
    )

    register_compressor(
        compressor_name, StandardLibGzipCompressorWrapper(), force=True
    )
